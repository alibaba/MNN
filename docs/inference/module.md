...
- Interpreter::HintMode::GEOMETRY_COMPUTE_MASK ：几何计算相关优化开关，1为区域合并，2为复合区域合并，4为使用 loop 算子，8为支持几何计算重计算，16为连续 virtual tensor 使用 backend Ref，需要多个功能开启时把对应值叠加。默认为功能全开。
