...
- **w3 仍是最弱项**：18.8 GB/s，饱和度 ~31%，远低于 w2/w4。和 M-Mac/SD8G3 上 P3 后的趋势一致——8 Elite 的 sdot/i8mm 调度还有空间，可参考内部记录 `memory/w2w3_optimization_lessons.md`。
...
- **跨设备对比**（w8 eff GB/s）：M3 Pro CPU 103 / SD8 Elite (taskset c0, 2t) 61 / iPad M5 Metal ~114（见内部记录 `memory/ipad_m5_bench.md`）。
...
- iPad / iOS 真机上需通过 `tools/ios_llm_benchmark_server.py` 间接触发 (设备无 shell)；相关经验记录在 `memory/ipad_m5_bench.md`。
