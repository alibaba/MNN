// sherpa-mnn/csrc/online-websocket-server.cc
//
...
./bin/sherpa-mnn-online-websocket-server --help

./bin/sherpa-mnn-online-websocket-server \
  --port=6006 \
  --num-work-threads=5 \
...
int32_t main(int32_t argc, char *argv[]) {
  ...
  sherpa_mnn::OnlineWebsocketServerConfig config;
  ...
  sherpa_mnn::OnlineWebsocketServer server(io_conn, io_work, config);
  server.Run(port);
