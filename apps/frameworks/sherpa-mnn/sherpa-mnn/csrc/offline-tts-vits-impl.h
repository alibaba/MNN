// sherpa-mnn/csrc/offline-tts-vits-impl.h
//
...
class OfflineTtsVitsImpl : public OfflineTtsImpl {
 public:
  ...
  GeneratedAudio Generate(
    ...
    std::vector<std::vector<int>> tones;
    ...
    std::vector<std::vector<int>> batch_tones;
 ...
 private:
  ...
  GeneratedAudio Process(const std::vector<std::vector<int>> &tokens,
                         const std::vector<std::vector<int>> &tones,
    ...
    MNN::Express::VARP tones_tensor{nullptr};
    if (!tones.empty()) {
      tones_tensor = MNNUtilsCreateTensor(memory_info, tone_list.data(),
                                              tone_list.size(), x_shape.data(),
                                              x_shape.size());
    }

    MNN::Express::VARP audio{nullptr};
    if (tones.empty()) {
