...
public actor ModelDownloadManager: ModelDownloadManagerProtocol {
    ...
    public init(
        ...
        fileManager = .default
        storage = ModelDownloadStorage()
        session = URLSession(configuration: sessionConfig)
        concurrencyManager = DynamicConcurrencyManager(config: concurrencyConfig)
        downloadSemaphore = AsyncSemaphore(value: config.maxConcurrentDownloads)
        ...
        for task in downloadQueue {
            if task.isChunked {
                ...
                let chunkBytes = task.chunks.reduce(0) { total, chunk in
                    total + (chunk.isCompleted ? (chunk.endOffset - chunk.startOffset + 1) : chunk.downloadedBytes)
                }
