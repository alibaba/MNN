...
int main() {
    std::cout << "Testing download and verification with a small file..." << std::endl;

    // Use a small JSON file that should work
    std::string test_url = "https://huggingface.co/taobao-mnn/SmolVLM-256M-Instruct-MNN/resolve/main/config.json";
    std::string cache_path = "/tmp/download_verification_test";

    try {
        // Create cache directory
        fs::create_directories(cache_path);
        fs::path test_file_path = fs::path(cache_path) / "config.json";
            ...
            bool onDownloadDelta(const std::string* fileName, int64_t downloadedBytes,
                ...
                if (totalBytes > 0) {
                    ...
                    std::cout << "  Progress: " << static_cast<int>(progress) << "% ("
                              << downloadedBytes << " / " << totalBytes << " bytes)" << std::endl;
                }
