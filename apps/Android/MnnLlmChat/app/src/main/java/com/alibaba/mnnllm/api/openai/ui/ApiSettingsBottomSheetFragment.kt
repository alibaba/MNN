...
class ApiSettingsBottomSheetFragment : BottomSheetDialogFragment() {
    ...
    private fun saveSettings() {
        ...
        if (MainSettings.isApiServiceEnabled(context)) {
            if (ApiServiceManager.isApiServiceRunning()) {
                ...
                Toast.makeText(context, "正在重启API服务以应用新配置...", Toast.LENGTH_SHORT).show()
                
                // 延迟启动新服务，确保停止操作完成
                lifecycleScope.launch {
                    kotlinx.coroutines.delay(1500) // 等待1.5秒确保停止完成
                    // 在重新启动前重置状态
                    // 优雅停止服务，不立即重置状态
    ...
    private fun resetToDefaults() {
        ...
        Toast.makeText(context, "配置已恢复默认，正在处理服务状态...", Toast.LENGTH_SHORT).show()
        ...
        if (ApiServiceManager.isApiServiceRunning()) {
            ...
            ApiServiceManager.stopApiService(context)
            lifecycleScope.launch {
                kotlinx.coroutines.delay(1500) // 等待服务完全停止
