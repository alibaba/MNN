...
class AudioChunksPlayer {
    ...
    fun setMarkerSizeListener(size:Int, listener: (Int) -> Unit) {
        if (audioTrack == null || !isInited) {
            Log.w(TAG, "AudioTrack not initialized, cannot set marker listener")
            return
        }
        
        Log.d(TAG, "Setting marker at position: $size")
        ...
        audioTrack!!.setPlaybackPositionUpdateListener(object : AudioTrack.OnPlaybackPositionUpdateListener {
            ...
            override fun onMarkerReached(track: AudioTrack?) {
                ...
                Log.d(TAG, "onMarkerReached ${track?.playbackHeadPosition} totalSize: ${totalSize}")
...
//        )
        Log.d(TAG, "playChunk: ${pcmData.size} totalSize: $totalSize")
        try {
            playerScope.async {
                audioTrack?.write(pcmData, 0, pcmData.size, AudioTrack.WRITE_BLOCKING)
            }.await()
    ...
    fun reset() {
        Log.d(TAG, "reset called totalSize:$totalSize")
        val savedListener = onCompletionListener  // Preserve the listener
        stop()
        totalSize = 0  // Reset totalSize after stopping
        isEndChunkCalled = false
        finalTotalSize = 0
        start()
        onCompletionListener = savedListener  // Restore the listener
    }
    ...
    fun endChunk() {
        Log.d(TAG, "endChunk called, totalSize: $totalSize, isPlaying: $isPlaying")
        if (!isEndChunkCalled) {
            isEndChunkCalled = true
            finalTotalSize = totalSize
            Log.d(TAG, "Final total size set to: $finalTotalSize")
            
            if (finalTotalSize > 0 && audioTrack != null && isInited) {
                setMarkerSizeListener(finalTotalSize) { position ->
                    Log.d(TAG, "Final marker reached at position: $position, finalTotalSize: $finalTotalSize")
                    if (position >= finalTotalSize) {
                        Log.d(TAG, "Audio completion detected, invoking listener")
                        onCompletionListener?.invoke()
                    }
                }
            } else {
                Log.w(TAG, "Cannot set end marker - finalTotalSize: $finalTotalSize, audioTrack: ${audioTrack != null}, isInited: $isInited")
                // If no audio data, immediately call completion
                if (finalTotalSize == 0) {
                    onCompletionListener?.invoke()
                }
            }
        } else {
            Log.d(TAG, "endChunk already called, ignoring")
        }
    }
