...
class AsrService(
    ...
    private val modelDir: String? = null // Allow null to use dynamic path
) {
    ...
    suspend fun initRecognizer() {
        val actualModelDir = modelDir ?: VoiceModelPathUtils.getAsrModelPath(activity)
        ...
        val config = getModelConfigFromDirectory(actualModelDir)?.let {
            OnlineRecognizerConfig(
                ...
                it,
                getOnlineLMConfigFromDirectory(actualModelDir),
