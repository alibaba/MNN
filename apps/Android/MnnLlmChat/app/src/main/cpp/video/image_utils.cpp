...
bool ImageUtils::saveFrameAsImage(const uint8_t* yuvData, int width, int height,
    ...
    if (!yuvData || !basePath || width <= 0 || height <= 0) {
        VIDEO_LOGE(TAG,"saveFrameAsImage: invalid parameters");
        return false;
    }
    
    VIDEO_LOGV(TAG,"saveFrameAsImage: saving %dx%d frame, format=%d", width, height, static_cast<int>(format));
    ...
    if (format == YUVFormat::AUTO) {
        ...
        VIDEO_LOGV(TAG,"saveFrameAsImage: auto-detected format=%d, valid=%d", 
        ...
        VIDEO_LOGV(TAG,"saveFrameAsImage: using specified format=%d", static_cast<int>(format));
    }
    
    if (!formatInfo.isValid) {
        VIDEO_LOGE(TAG,"saveFrameAsImage: invalid YUV format");
        return false;
    }
    
    VIDEO_LOGV(TAG,"saveFrameAsImage: format info - format=%d, size=%dx%d, ySize=%d, uvSize=%d, totalSize=%d", 
    ...
    if (!yuvToRgb(yuvData, formatInfo, rgbData)) {
        VIDEO_LOGE(TAG,"saveFrameAsImage: YUV to RGB conversion failed");
        return false;
    }
    
    VIDEO_LOGV(TAG,"saveFrameAsImage: YUV to RGB conversion successful, RGB size=%zu", rgbData.size());
    ...
    if (saveJPEG) {
        ...
        if (!saveAsJPEG(rgbData.data(), width, height, jpegPath.c_str())) {
            VIDEO_LOGE(TAG,"saveFrameAsImage: failed to save JPEG: %s", jpegPath.c_str());
            success = false;
        } else {
            VIDEO_LOGV(TAG,"saveFrameAsImage: saved JPEG: %s", jpegPath.c_str());
        }
    }
    ...
    if (saveBMP) {
        ...
        if (!saveAsBMP(rgbData.data(), width, height, bmpPath.c_str())) {
            VIDEO_LOGE(TAG,"saveFrameAsImage: failed to save BMP: %s", bmpPath.c_str());
            success = false;
        } else {
            VIDEO_LOGV(TAG,"saveFrameAsImage: saved BMP: %s", bmpPath.c_str());
        }
    }
...
bool ImageUtils::yuvToRgb(const uint8_t* yuvData, const YUVFormatInfo& formatInfo, 
    ...
    if (!yuvData || !formatInfo.isValid) {
        VIDEO_LOGE(TAG,"yuvToRgb: invalid parameters - yuvData=%p, formatInfo.isValid=%d", 
             yuvData, formatInfo.isValid);
        return false;
    }
    
    VIDEO_LOGV(TAG,"yuvToRgb: converting format=%d, size=%dx%d, stride=%d, sliceHeight=%d", 
         static_cast<int>(formatInfo.format), formatInfo.outputWidth, formatInfo.outputHeight,
    ...
    if (hasStridePadding) {
        VIDEO_LOGV(TAG,"yuvToRgb: detected stride padding - stride=%d vs width=%d, sliceHeight=%d vs height=%d",
    ...
    switch (formatInfo.format) {
        ...
        case YUVFormat::NV21:
            ...
            break;
        default:
            VIDEO_LOGE(TAG,"yuvToRgb: unsupported YUV format: %d", static_cast<int>(formatInfo.format));
            return false;
    }
...
bool ImageUtils::yuvToRgbWithStride(const uint8_t* yuvData, const YUVFormatInfo& formatInfo, 
                                   std::vector<uint8_t>& rgbData) {
    if (!yuvData || !formatInfo.isValid) {
        VIDEO_LOGE(TAG,"yuvToRgbWithStride: invalid parameters");
        return false;
    }
    
    VIDEO_LOGV(TAG,"yuvToRgbWithStride: handling stride conversion - stride=%d, sliceHeight=%d, output=%dx%d",
    ...
    VIDEO_LOGV(TAG,"yuvToRgbWithStride: Y plane size=%d, UV plane size=%d", 
    ...
    for (int i = 0; i < formatInfo.outputHeight; i++) {
        for (int j = 0; j < formatInfo.outputWidth; j++) {
            ...
            if (formatInfo.format == YUVFormat::NV12) {
                ...
                VIDEO_LOGE(TAG,"yuvToRgbWithStride: unsupported format with stride: %d", 
            ...
            if (i < 2 && j < 2) {
                VIDEO_LOGV(TAG,"yuvToRgbWithStride: pixel[%d,%d] YUV=(%d,%d,%d)", i, j, yVal, uVal, vVal);
            }
            ...
            if (i < 2 && j < 2) {
                VIDEO_LOGV(TAG,"yuvToRgbWithStride: pixel[%d,%d] final RGB=(%d,%d,%d)", i, j, r, g, b);
            }
        }
    }
    
    VIDEO_LOGV(TAG,"yuvToRgbWithStride: conversion completed, RGB size=%zu", rgbData.size());
    return true;
}
...
void ImageUtils::i420ToRgb(const uint8_t* yuvData, int width, int height, std::vector<uint8_t>& rgbData) {
    if (!yuvData || width <= 0 || height <= 0) {
        VIDEO_LOGE(TAG,"Invalid parameters for i420ToRgb: yuvData=%p, width=%d, height=%d", yuvData, width, height);
        return;
    }
    
    VIDEO_LOGV(TAG,"i420ToRgb: converting %dx%d I420 to RGB", width, height);
    ...
    VIDEO_LOGV(TAG,"i420ToRgb: ySize=%d, uvSize=%d, total expected size=%d", ySize, uvSize, ySize + 2 * uvSize);
    ...
    VIDEO_LOGV(TAG,"i420ToRgb: first Y values: %d, %d, %d, %d", y[0], y[1], y[2], y[3]);
    VIDEO_LOGV(TAG,"i420ToRgb: first U values: %d, %d, %d, %d", u[0], u[1], u[2], u[3]);
    VIDEO_LOGV(TAG,"i420ToRgb: first V values: %d, %d, %d, %d", v[0], v[1], v[2], v[3]);
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            ...
            // Log first few conversions for debugging
            if (i < 2 && j < 2) {
                VIDEO_LOGV(TAG,"i420ToRgb: pixel[%d,%d] YUV=(%d,%d,%d)", i, j, yVal, uVal, vVal);
            }
            ...
            if (i < 2 && j < 2) {
                VIDEO_LOGV(TAG,"i420ToRgb: pixel[%d,%d] RGB calc: c=%d, d=%d, e=%d -> r=%d, g=%d, b=%d", 
                     i, j, c, d, e, r, g, b);
            }
            ...
            // Log first few final RGB values for debugging
            if (i < 2 && j < 2) {
                VIDEO_LOGV(TAG,"i420ToRgb: pixel[%d,%d] final RGB=(%d,%d,%d)", i, j, r, g, b);
            }
        }
    }
    
    VIDEO_LOGV(TAG,"i420ToRgb: conversion completed, RGB size=%zu", rgbData.size());
}

void ImageUtils::nv12ToRgb(const uint8_t* yuvData, int width, int height, std::vector<uint8_t>& rgbData) {
    if (!yuvData || width <= 0 || height <= 0) {
        VIDEO_LOGE(TAG,"Invalid parameters for nv12ToRgb");
        return;
    }
    
    VIDEO_LOGV(TAG,"nv12ToRgb: converting %dx%d NV12 to RGB", width, height);
    ...
    // Log first few YUV values for debugging
    VIDEO_LOGV(TAG,"nv12ToRgb: first Y values: %d, %d, %d, %d", y[0], y[1], y[2], y[3]);
    VIDEO_LOGV(TAG,"nv12ToRgb: first UV values: %d, %d, %d, %d", uv[0], uv[1], uv[2], uv[3]);
    
    // Log more Y values to check for variation
    VIDEO_LOGV(TAG,"nv12ToRgb: Y values at different positions: [0,0]=%d, [0,10]=%d, [10,0]=%d, [10,10]=%d", 
         y[0], y[10], y[10*width], y[10*width+10]);
    
    // Log UV values at different positions
    VIDEO_LOGV(TAG,"nv12ToRgb: UV values at different positions: [0,0]=%d,%d, [0,10]=%d,%d, [10,0]=%d,%d, [10,10]=%d,%d",
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            ...
            // Log first few conversions for debugging
            if (i < 2 && j < 2) {
                VIDEO_LOGV(TAG,"nv12ToRgb: pixel[%d,%d] YUV=(%d,%d,%d)", i, j, yVal, uVal, vVal);
            }
            ...
            // Log first few RGB calculations for debugging
            if (i < 2 && j < 2) {
                VIDEO_LOGV(TAG,"nv12ToRgb: pixel[%d,%d] RGB calc: c=%d, d=%d, e=%d -> r=%d, g=%d, b=%d", 
                     i, j, c, d, e, r, g, b);
            }
            ...
            // Log first few final RGB values for debugging
            if (i < 2 && j < 2) {
                VIDEO_LOGV(TAG,"nv12ToRgb: pixel[%d,%d] final RGB=(%d,%d,%d)", i, j, r, g, b);
            }
        }
    }
    
    VIDEO_LOGV(TAG,"nv12ToRgb: conversion completed, RGB size=%zu", rgbData.size());
}
...
bool ImageUtils::saveRawYUVData(const uint8_t* yuvData, int width, int height, int size, const char* filename) {
    if (!yuvData || !filename || size <= 0) {
        VIDEO_LOGE(TAG,"saveRawYUVData: invalid parameters");
        return false;
    }
    
    VIDEO_LOGV(TAG,"saveRawYUVData: saving %dx%d YUV data (%d bytes) to: %s", width, height, size, filename);
    
    FILE* file = fopen(filename, "wb");
    if (!file) {
        VIDEO_LOGE(TAG,"saveRawYUVData: failed to open file %s", filename);
        return false;
    }
    ...
    if (written == size) {
        VIDEO_LOGV(TAG,"saveRawYUVData: successfully saved %zu bytes to %s", written, filename);
        return true;
    } else {
        VIDEO_LOGE(TAG,"saveRawYUVData: failed to write all data, wrote %zu/%d bytes", written, size);
        return false;
    }
}
...
bool ImageUtils::createTestPattern(int width, int height, const char* filename) {
    VIDEO_LOGV(TAG,"createTestPattern: creating %dx%d test pattern", width, height);
    ...
    VIDEO_LOGV(TAG,"createTestPattern: test pattern created, saving to: %s", filename);
...
void ImageUtils::nv21ToRgb(const uint8_t* yuvData, int width, int height, std::vector<uint8_t>& rgbData) {
    if (!yuvData || width <= 0 || height <= 0) {
        VIDEO_LOGE(TAG,"Invalid parameters for nv21ToRgb");
        return;
    }
...
bool ImageUtils::saveAsJPEG(const uint8_t* rgbData, int width, int height, const char* filename) {
    if (!rgbData || !filename || width <= 0 || height <= 0) {
        VIDEO_LOGE(TAG,"saveAsJPEG: invalid parameters - rgbData=%p, filename=%s, width=%d, height=%d", 
             rgbData, filename, width, height);
        return false;
    }
    
    VIDEO_LOGV(TAG,"saveAsJPEG: saving %dx%d RGB data to: %s", width, height, filename);
    
    // Log first few RGB values for debugging
    if (width * height >= 4) {
        VIDEO_LOGV(TAG,"saveAsJPEG: first RGB values: R=%d,G=%d,B=%d, R=%d,G=%d,B=%d, R=%d,G=%d,B=%d, R=%d,G=%d,B=%d",
    ...
    VIDEO_LOGV(TAG,"saveAsJPEG: calling stbi_write_jpg with quality=90");
    int result = stbi_write_jpg(filename, width, height, 3, rgbData, 90);
    if (result) {
        VIDEO_LOGV(TAG,"saveAsJPEG: successfully saved JPEG: %s (result=%d)", filename, result);
        return true;
    } else {
        VIDEO_LOGE(TAG,"saveAsJPEG: failed to save JPEG: %s (result=%d)", filename, result);
        return false;
    }
}
...
bool ImageUtils::saveAsBMP(const uint8_t* rgbData, int width, int height, const char* filename) {
    FILE* file = fopen(filename, "wb");
    if (!file) {
        VIDEO_LOGE(TAG,"saveAsBMP: failed to open file %s", filename);
        return false;
    }
    ...
    fclose(file);
    VIDEO_LOGV(TAG,"saveAsBMP: saved BMP file: %s", filename);
    return true;
}
...
bool ImageUtils::saveTensorAsJPG(const MNN::Express::VARP& tensor, const char* filename) {
    if (!tensor.get() || !filename) {
        VIDEO_LOGE(TAG,"saveTensorAsJPG: invalid parameters - tensor=%p, filename=%s", tensor.get(), filename);
        return false;
    }
    
    VIDEO_LOGV(TAG,"saveTensorAsJPG: starting to save tensor to: %s", filename);
    
    // Get tensor info
    auto info = tensor->getInfo();
    if (!info) {
        VIDEO_LOGE(TAG,"saveTensorAsJPG: failed to get tensor info");
        return false;
    }
    
    // Check tensor dimensions
    auto dims = info->dim;
    VIDEO_LOGV(TAG,"saveTensorAsJPG: tensor dimensions: [%s]", 
         [&dims]() {
             std::string dimStr;
    ...
    if (dims.size() < 2) {
        VIDEO_LOGE(TAG,"saveTensorAsJPG: tensor must have at least 2 dimensions, got %zu", dims.size());
        return false;
    }
    ...
    if (dims.size() == 3) {
        ...
        width = dims[1];
        channels = dims[2];
        VIDEO_LOGV(TAG,"saveTensorAsJPG: detected HWC format");
    } else if (dims.size() == 4) {
        // Assume NHWC format, take first batch
        height = dims[1];
        width = dims[2];
        channels = dims[3];
        VIDEO_LOGV(TAG,"saveTensorAsJPG: detected NHWC format");
    } else {
        VIDEO_LOGE(TAG,"saveTensorAsJPG: unsupported tensor dimensions: %zu", dims.size());
        return false;
    }
    
    VIDEO_LOGV(TAG,"saveTensorAsJPG: tensor dimensions: %dx%dx%d", height, width, channels);
    VIDEO_LOGV(TAG,"saveTensorAsJPG: tensor data type: %d", static_cast<int>(info->type.code));
    
    // Convert tensor to RGB data
    std::vector<uint8_t> rgbData;
    VIDEO_LOGV(TAG,"saveTensorAsJPG: calling tensorToRgb...");
    if (!tensorToRgb(tensor, height, width, channels, rgbData)) {
        VIDEO_LOGE(TAG,"saveTensorAsJPG: failed to convert tensor to RGB");
        return false;
    }
    
    VIDEO_LOGV(TAG,"saveTensorAsJPG: tensorToRgb completed, RGB data size: %zu", rgbData.size());
    
    // Log first few RGB values for debugging
    if (rgbData.size() >= 12) {
        VIDEO_LOGV(TAG,"saveTensorAsJPG: first RGB values: R=%d,G=%d,B=%d, R=%d,G=%d,B=%d, R=%d,G=%d,B=%d, R=%d,G=%d,B=%d",
             rgbData[0], rgbData[1], rgbData[2], rgbData[3], rgbData[4], rgbData[5],
             rgbData[6], rgbData[7], rgbData[8], rgbData[9], rgbData[10], rgbData[11]);
    }
    
    // Save as JPG using proper JPEG encoding
    VIDEO_LOGV(TAG,"saveTensorAsJPG: calling saveAsJPEG...");
    bool result = saveAsJPEG(rgbData.data(), width, height, filename);
    if (result) {
        VIDEO_LOGV(TAG,"saveTensorAsJPG: successfully saved JPEG: %s", filename);
    } else {
        VIDEO_LOGE(TAG,"saveTensorAsJPG: failed to save JPEG: %s", filename);
    }
    return result;
}
...
bool ImageUtils::tensorToRgb(const MNN::Express::VARP& tensor, int height, int width, int channels,
                           std::vector<uint8_t>& rgbData) {
    if (!tensor.get()) {
        VIDEO_LOGE(TAG,"tensorToRgb: tensor is null");
        return false;
    }
    ...
    auto info = tensor->getInfo();
    if (!info) {
        VIDEO_LOGE(TAG,"tensorToRgb: failed to get tensor info");
        return false;
    }
    
    auto dims = info->dim;
    VIDEO_LOGV(TAG,"tensorToRgb: tensor dimensions: [%s]", 
         [&dims]() {
             std::string dimStr;
             for (size_t i = 0; i < dims.size(); i++) {
                 if (i > 0) dimStr += ", ";
    ...
    // Validate dimensions
    if (dims.size() < 2) {
        VIDEO_LOGE(TAG,"tensorToRgb: tensor must have at least 2 dimensions, got %zu", dims.size());
        return false;
    }
    
    // Calculate expected size
    int expectedSize = height * width * channels;
    VIDEO_LOGV(TAG,"tensorToRgb: expected size: %d (H=%d, W=%d, C=%d)", expectedSize, height, width, channels);
    ...
    if (dims.size() == 3) {
        ...
        if (dims[0] == height && dims[1] == width && dims[2] == channels) {
            ...
            VIDEO_LOGV(TAG,"tensorToRgb: tensor is in HWC format");
        }
        // Check if it's CHW format
        else if (dims[0] == channels && dims[1] == height && dims[2] == width) {
            dimensionsMatch = true;
            VIDEO_LOGV(TAG,"tensorToRgb: tensor is in CHW format");
        }
    }
    
    if (!dimensionsMatch) {
        VIDEO_LOGV(TAG,"tensorToRgb: tensor dimensions don't match expected format, trying to adapt");
    }
    
    // Try to read tensor data as uint8 first
    auto ptr = tensor->readMap<uint8_t>();
    if (ptr) {
        VIDEO_LOGV(TAG,"tensorToRgb: reading as uint8 data");
        ...
        VIDEO_LOGV(TAG,"tensorToRgb: tensor total size: %d, expected size: %d", tensorSize, expectedSize);
        
        if (tensorSize == expectedSize) {
            memcpy(rgbData.data(), ptr, expectedSize);
            VIDEO_LOGV(TAG,"tensorToRgb: successfully copied %d bytes of uint8 data", expectedSize);
            return true;
        } else {
            VIDEO_LOGV(TAG,"tensorToRgb: tensor size (%d) doesn't match expected size (%d), trying to reshape", 
            ...
            VIDEO_LOGV(TAG,"tensorToRgb: copied %d bytes, filled %d bytes with zeros", 
    ...
    if (floatPtr) {
        VIDEO_LOGV(TAG,"tensorToRgb: reading as float data");
        rgbData.resize(expectedSize);
        ...
        VIDEO_LOGV(TAG,"tensorToRgb: converted %d float values to uint8", copySize);
        return true;
    }
    
    VIDEO_LOGE(TAG,"tensorToRgb: failed to read tensor data as uint8 or float");
    return false;
}
