...
1. 图像处理:
   std::string prompt = "请描述这张图片: <img>/path/to/image.jpg</img>";
   session->Response(prompt, callback);

2. 视频处理:
   std::string prompt = "这个视频里发生了什么? <video>/path/to/video.mp4</video>";
   session->Response(prompt, callback);

3. 混合多模态:
   std::string prompt = "比较这张图片 <img>/path/to/image1.jpg</img> 和这个视频 <video>/path/to/video.mp4</video> 的内容";
   session->Response(prompt, callback);

4. 多图片:
   std::string prompt = "比较这两张图片: <img>/path/to/image1.jpg</img> 和 <img>/path/to/image2.jpg</img>";
   session->Response(prompt, callback);

注意事项:
- 如果文件路径无效或文件加载失败，系统会自动回退到纯文本模式
- 视频处理会自动提取关键帧并转换为图像
- 支持 Android 原生视频解码和 OpenCV 解码
- 错误信息会记录在日志中供调试使用
*/
