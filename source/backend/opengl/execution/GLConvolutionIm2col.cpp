...
//weight image : oc_4, ic_4 * ic4 oc4
    ...
    mKernelTexture = std::shared_ptr<GLTexture>(new GLTexture(ic_4 * UNIT*fw*fh, oc_4, 1, GL_TEXTURE_2D, false));
    ...
    OPENGL_CHECK_ERROR;
    glDispatchCompute(UP_DIV(imageWidth, 4), UP_DIV(oc_4, 4), 1);
    OPENGL_CHECK_ERROR;
...
ErrorCode GLConvolutionIm2col::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    ...
    mSrcTexture = std::shared_ptr<GLTexture>(new GLTexture(UP_DIV(ic, 4)*UNIT*fw*fh, obxohxow_4, 1, GL_TEXTURE_2D, false));
    mDstTexture = std::shared_ptr<GLTexture>(new GLTexture(obxohxow_4, UP_DIV(oc, 4) * UNIT, 1, GL_TEXTURE_2D, false));
...
//        image2col
    {
        ...
        glBindImageTexture(0, mSrcTexture->id(), 0, GL_TRUE, 0, GL_WRITE_ONLY, TEXTURE_FORMAT);
        {
            int texId = 0;
        ...
        if (mIsConv1x1) {
            ...
            glUniform4i(5, input->width(),input->height(), ic_4, 1);
            glUniform4i(6, ow, oh, oc_4, 1);
            glUniform2i(7, mCommon->dilateX(), mCommon->dilateY());
        }

        OPENGL_CHECK_ERROR;
        glDispatchCompute(UP_DIV(ow, mIm2colSize[0]), UP_DIV(oh, mIm2colSize[1]), UP_DIV(ic_4*ib, mIm2colSize[2]));
        OPENGL_CHECK_ERROR;
    }
    ...
    //gemm
    {
        ...
        glBindImageTexture(0, mDstTexture->id(), 0, GL_TRUE, 0, GL_WRITE_ONLY, TEXTURE_FORMAT);
        OPENGL_CHECK_ERROR;
        glBindImageTexture(1, mSrcTexture->id(), 0, GL_TRUE, 0, GL_READ_ONLY, TEXTURE_FORMAT);
        glBindImageTexture(2, mKernelTexture->id(), 0, GL_TRUE, 0, GL_READ_ONLY, TEXTURE_FORMAT);
        ...
        OPENGL_CHECK_ERROR;
        glDispatchCompute(UP_DIV(obxohxow_4, mGemmSize[0]), UP_DIV(oc_4, mGemmSize[1]), 1);
        OPENGL_CHECK_ERROR;
    }
    ...
    //col2image
    {
        ...
        OPENGL_CHECK_ERROR;
        glDispatchCompute(UP_DIV(ow, mCol2imSize[0]), UP_DIV(oh, mCol2imSize[1]), UP_DIV(oc_4*ob, mCol2imSize[2]));
        OPENGL_CHECK_ERROR;
    }
