...
// Measured winners for the gemm_buf transpose kernels; handing them to the tuner keeps Wide from
// sweeping the full grid on every conv-gemm resize.
LwsShortlist2D gemmTransposePadLwsShortlist(GpuType gpuType) {
    static const uint32_t adrenoPool[][2] = {{8, 4}, {16, 1}, {16, 2}, {32, 1}, {64, 2}, {256, 1}};
    static const uint32_t maliPool[][2] = {{1, 2}, {1, 4}, {8, 8}, {128, 2}};
    return makeLwsShortlist(gpuType, adrenoPool, maliPool);
}

LwsShortlist2D gemmTransposeBiasLwsShortlist(GpuType gpuType) {
    static const uint32_t adrenoPool[][2] = {{1, 128}, {2, 128}, {4, 128}, {2, 256}, {2, 512}};
    static const uint32_t maliPool[][2] = {{1, 256}, {1, 512}, {2, 64}, {2, 512}, {4, 256}, {8, 128}};
    return makeLwsShortlist(gpuType, adrenoPool, maliPool);
}
...
ConvBufCommonExecution::ConvBufCommonExecution(const Op* op, Backend* backend, bool isExtra) {
    ...
    if (isExtra) {
        conv2dParams = flatbuffers::GetRoot<Convolution2D>(
            op->main_as_Extra()->attr()->GetAs<Attribute>(0)->tensor()->uint8s()->data());
    } else {
    ...
    if (isExtra) {
        const PRelu* preluParam =
            flatbuffers::GetRoot<PRelu>(op->main_as_Extra()->attr()->GetAs<Attribute>(1)->tensor()->uint8s()->data());
        ...
        if (openclBackend->getRuntime()->hint().useCachedMmap <= 1) {
            ...
            auto slopePtrCL = openclBackend->getOpenCLRuntime()->commandQueue().enqueueMapBuffer(
                slopeBuffer, true, CL_MAP_WRITE, 0, buffer_size, nullptr, nullptr, &res);
...
void ConvBufExecution::_generateFilterConvertRegion(Tensor* virtualFilter, Tensor* originBuffer) const {
    ...
    for (int so = 0; so < 4; ++so) {
        ...
        slice.dst.stride[1] =
            mResource->mKernelWidth * mResource->mKernelHeight * UP_DIV(mResource->mOutputChannel, 4) * 4;
...
ConvBufExecution::ConvBufExecution(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
                                   const MNN::Op* op, Backend* backend, bool isExtra)
    ...
    if (inputs.size() != 1) {
        ...
        bool isConv1x1 =
            (mResource->mKernelHeight == mResource->mKernelWidth && mResource->mKernelHeight == 1 &&
             mPaddings[0] == 0 && mPaddings[1] == 0 && mResource->mStrides[0] == 1 && mResource->mStrides[1] == 1);
    ...
    if (mResource->mConv1x1Opt) {
        int buffer_size = ROUND_UP(mResource->mOutputChannel, mResource->mAlignN) *
                          ROUND_UP(mResource->mInputChannel, mResource->mAlignK);
                ...
                if (mOpenCLBackend->getPrecision() != BackendConfig::Precision_High) {
                    ...
                    for (int o = 0; o < mResource->mOutputChannel; o++) {
                        for (int i = 0; i < mResource->mInputChannel; i++) {
                            ((half_float::half*)
                                 ptrCL)[i * ROUND_UP(mResource->mOutputChannel, mResource->mAlignN) + o] =
                                (half_float::half)(mFilterDataPtr[o * mResource->mInputChannel + i]);
                        }
                    }
                } else {
                    for (int o = 0; o < mResource->mOutputChannel; o++) {
                        for (int i = 0; i < mResource->mInputChannel; i++) {
                            ((float*)ptrCL)[i * ROUND_UP(mResource->mOutputChannel, mResource->mAlignN) + o] =
                                (mFilterDataPtr[o * mResource->mInputChannel + i]);
                        }
                    }
                }
            } else {
        ...
        mResource->mFilter.reset(Tensor::createDevice<float>(
            {ROUND_UP(mResource->mOutputChannel, 4) * ROUND_UP(mResource->mInputChannel, 4) * mResource->mKernelWidth *
             mResource->mKernelHeight}));
        if (mFilterDataPtr != nullptr) {
            std::vector<int> filterImageShape{
                ROUND_UP(mResource->mInputChannel, 4),
                (UP_DIV(mResource->mOutputChannel, 4) * mResource->mKernelWidth * mResource->mKernelHeight)};
            mResource->mFilter.reset(Tensor::createDevice<float>({filterImageShape[1] * 4 * filterImageShape[0]}));
            OPENCL_CHECK_ALLOC_CTOR(mOpenCLBackend->onAcquireBuffer(mResource->mFilter.get(), Backend::STATIC));

            if (mOpenCLBackend->getRuntime()->hint().useCachedMmap <= 1) {
                std::shared_ptr<Tensor> filterBuffer(
                    Tensor::createDevice<float>({mResource->mOutputChannel, ROUND_UP(mResource->mInputChannel, 4),
                ...
                if (ptrCL != nullptr && res == CL_SUCCESS) {
                    ...
                    for (int oc = 0; oc < mResource->mOutputChannel; oc++) {
                        for (int ic = 0; ic < mResource->mInputChannel; ic++) {
                            ::memcpy((float*)ptrCL + (oc * ROUND_UP(mResource->mInputChannel, 4) + ic) *
                                                         mResource->mKernelWidth * mResource->mKernelHeight,
                                     mFilterDataPtr + (oc * mResource->mInputChannel + ic) * mResource->mKernelWidth *
                                                          mResource->mKernelHeight,
                                     copy_size);
                        }
                    }
                ...
                bool needTrans = true;
                bufferConvertor.convertToNC4HW4Buffer(filterBuffer.get(), MNN::OpenCL::CONV2D_FILTER,
                                                      mResource->mFilter.get(), mOpenCLBackend->getPrecision(),
                                                      needTrans);
            }
        }
    }
...
void ConvBufCommonExecution::submitPrebuildConvPrograms() {
    if (!mOpenCLBackend || !mResource) {
        return; // ctor early-returned (alloc failure) or backend not wired
    }
    ...
    // The runtime dedups by (programName, buildOptionsStr), so this is safe to call
    // even if multiple conv layers share the same build options. Use the resource's
    // build-options set, which is finalized by the time the subclass ctor returns.
    runtime->submitPrebuild("conv_2d_buf", mResource->mBuildOptions, precision);
    if (mResource->mConvGemmOptLevel == 1) {
        runtime->submitPrebuild("gemm_buf", mResource->mBuildOptions, precision);
    }
}
...
ErrorCode ConvBufExecution::onResize(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    // printf("nchw %d %d %d %d, cohw %d %d %d, khw %d %d  gemm:%d \n", inputs[0]->batch(), inputs[0]->channel(),
    // inputs[0]->height(), inputs[0]->width(), outputs[0]->channel(), outputs[0]->height(), outputs[0]->width(),
    // mResource->mKernelWidth, mResource->mKernelHeight, mResource->mConvGemmOptLevel);
    ...
    if (mResource->mConvGemmOptLevel == 1) {
...

        {
            ...
            mPreLocalWorkSize =
                localWS2DDefault(mPreGlobalWorkSize, maxWorkGroupSize, mOpenCLBackend->getOpenCLRuntime(),
                                 "transpose_pad", mPreKernel, mOpenCLBackend->getCLTuneLevel(), "gemm_buf",
                                 gemmTransposePadLwsShortlist(mOpenCLBackend->getOpenCLRuntime()->getGpuType()))
                    .first;
        ...
        // call gemm strassen
        {
            ...
            mStrassenComputor->onEncode(alignM, alignK, alignN, alignM, alignN, alignN,
                                        openCLBuffer(mConvGemmInpTensor.get()), openCLBuffer(mResource->mFilter.get()),
        ...
        // call output transpose
        {
            ...
            mPostLocalWorkSize =
                localWS2DDefault(mPostGlobalWorkSize, maxWorkGroupSize, mOpenCLBackend->getOpenCLRuntime(),
                                 "transpose_bias", mPostKernel, mOpenCLBackend->getCLTuneLevel(), "gemm_buf",
                                 gemmTransposeBiasLwsShortlist(mOpenCLBackend->getOpenCLRuntime()->getGpuType()))
                    .first;
        ...
        if (inputChannels >= 128 && outputShape[0] * outChannel * width * height <= 64) {
            ...
            mGlobalWorkSize = {static_cast<uint32_t>(local_size), static_cast<uint32_t>(UP_DIV(outChannel, 4) * width),
                               static_cast<uint32_t>(outputShape[0] * height)};
            ...
            mKernel[0] = mOpenCLBackend->getOpenCLRuntime()->buildKernel("conv_2d_buf", "conv_2d_1x1_local",
            ...
            mResource->mConv1x1C8Opt =
                (mResource->mOutputChannel >= 16 && M >= 16 && M * mResource->mOutputChannel >= 65536);
            ...
            for (int knl_idx = 0; knl_idx < actual_kernel; knl_idx++) {
                std::set<std::string> buildOption = mResource->mBuildOptions;
                if (itemC[knl_idx] == 8 && outputShape.at(3) % itemC[knl_idx] > 0 &&
                    outputShape.at(3) % itemC[knl_idx] <= 4) {
                ...
                kernel[knl_idx] = mOpenCLBackend->getOpenCLRuntime()->buildKernel(
                    "conv_2d_buf", kernelName[knl_idx], buildOption, mOpenCLBackend->getPrecision());
                uint32_t maxWorkGroupSize =
                    static_cast<uint32_t>(mOpenCLBackend->getOpenCLRuntime()->getMaxWorkGroupSize(kernel[knl_idx]));

                uint32_t idx = 0;
                cl_int ret = CL_SUCCESS;
                globalWorkSize[knl_idx] = {static_cast<uint32_t>(UP_DIV(outputShape.at(3), itemC[knl_idx]) *
                ...
                retTune = localWS2DDefault(globalWorkSize[knl_idx], maxWorkGroupSize,
                                           mOpenCLBackend->getOpenCLRuntime(), kernelName[knl_idx] + info,
                                           kernel[knl_idx], mOpenCLBackend->getCLTuneLevel(), "conv_2d_buf");
            ...
            mKernel[0] = mOpenCLBackend->getOpenCLRuntime()->buildKernel("conv_2d_buf", kernelName[min_index],
                                                                         buildOption, mOpenCLBackend->getPrecision());
            uint32_t idx = 0;
            cl_int ret = CL_SUCCESS;
        ...
        std::string kernelName[total_kernel] = {"conv_2d_c4h1w1", "conv_2d_c4h1w2", "conv_2d_c4h4w1", "conv_2d_c4h1w4",
                                                "conv_2d_c8h2w1", "conv_2d_c8h4w1", "conv_2d_c8h1w4"};
        ...
        auto magic_ratio = 1.0 * outputShape.at(0) * outputShape.at(1) * outputShape.at(2) / 1024.0 * inputChannels *
                           kernelShape[0] * kernelShape[1] / 1024.0 * outChannel / 1024.0;
        ...
        for (int knl_idx = 0; knl_idx < actual_kernel; knl_idx++) {
            ...
            globalWorkSize[knl_idx] = {
                static_cast<uint32_t>(each_oc * UP_DIV(outputShape.at(2), itemW[knl_idx])),
        ...
        for (int kernel_idx = 0; kernel_idx < conv_block_num; kernel_idx++) {
            mKernel[kernel_idx] = mOpenCLBackend->getOpenCLRuntime()->buildKernel(
                "conv_2d_buf", kernelName[min_index], buildOption, mOpenCLBackend->getPrecision());
...
ErrorCode ConvBufExecution::onExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    if (inputs.size() > 1) {
        ...
        if (inputs.size() > 2) {
            ...
            mOpenCLBackend->getOpenCLRuntime()->commandQueue().enqueueCopyBuffer(
                openCLBuffer(inputs[2]), openCLBuffer(mResource->mBias.get()), 0, 0, buffer_size);
        }
    }
    ...
    if (mResource->mConvGemmOptLevel == 1) {
        ...
        std::string total = std::to_string(1.0 / 1000000 * inputs[0]->batch() * inputs[0]->channel() *
        ...
        if (mResource->mConvGemmOptLevel > 0) {
            ...
            name += "-ori-b" + b + "ci" + ci + "hi" + hi + "wi" + wi + "co" + co + "ho" + ho + "wo" + wo + "kh" + kh +
                    "kw" + kw;
        }
    ...
    virtual Execution* onCreate(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
        ...
        if (static_cast<OpenCLBackend*>(backend)->getMemory() == BackendConfig::Memory_Low) {
            ...
            if (conv2dParams->quanParameter() != nullptr) {
                if (((conv2dParams->quanParameter()->type() == 4) || (conv2dParams->quanParameter()->type() == 1) ||
                     (conv2dParams->quanParameter()->type() == 2))) {
                    if ((1 == conv2dParams->quanParameter()->type() || 2 == conv2dParams->quanParameter()->type()) &&
                        conv2dParams->quanParameter()->has_scaleInt()) {
                        // Don't support IDST-int8 because of error
                        return nullptr;
                    }
                    for (int i = 0; i < inputs.size(); ++i) {
