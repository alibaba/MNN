...
void AttentionBufExecution::handleKVCache(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    if (nullptr == mMeta) {
        ...
        mKvStart = mSlidingWindow > 0 ? std::max(0, mKvSeqlen - mSlidingWindow) : 0;
        mEffectiveKvSeqlen = mKvSeqlen - mKvStart;
        mKeyValueMaxlen = ROUND_UP(kvInputLen, 4);
        mDecodeTmpMaxlen = ROUND_UP(mEffectiveKvSeqlen, 4);
        return;
    }
    ...
    mDecodeTmpMaxlen =
        mSlidingWindow > 0 ? ROUND_UP(std::min(mSlidingWindow, mKeyValueMaxlen), 4) : mKeyValueMaxlen;
...
ErrorCode AttentionBufExecution::UpdateArgs(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    if (mIsDecode == false) {
        ...
        if (false == mLongPrefill) {
            ...
            if (mHasMask) {
                if (mIsAddMask) {
                    mTempMask.reset(
                        Tensor::createDevice<float>({ROUND_UP(maskQlen, 4) * ROUND_UP(maskKvlen, 4) * batch}));
                } else {
                    mTempMask.reset(
                        Tensor::createDevice<uint32_t>({ROUND_UP(maskQlen, 4) * ROUND_UP(maskKvlen, 4) * batch}));
                }
            }
        ...
        if (mOpenCLBackend->isUseRecordQueue()) {
            ...
            if (mLongPrefill) {
                ...
                {
                    // matmul qk
                    mGlobalWorkSizeQk = {static_cast<uint32_t>(UP_DIV(seqlen, 4)),
                                         static_cast<uint32_t>(UP_DIV(mKvSeqlen, 4)),
    ...
    // Decode
    mKeyValueMaxlen = ROUND_UP(mKVCacheCLManager->maxLength(), 4);
    if (mEffectiveKvSeqlen > mDecodeTmpMaxlen) {
        mDecodeTmpMaxlen =
            mSlidingWindow > 0 ? ROUND_UP(std::min(mSlidingWindow, mKeyValueMaxlen), 4) : mKeyValueMaxlen;
    ...
    mGlobalWorkSizeQk0 = UP_DIV(mEffectiveKvSeqlen, 4);
    ...
    if (mOpenCLBackend->isUseRecordQueue()) {
        ...
        }
        {
            ...
            index++;
            ret |= mKernel_qk->get().setArg(index++, mEffectiveKvSeqlen);
            ret |= mKernel_qk->get().setArg(index++, mKeyValueMaxlen);
            ret |= mKernel_qk->get().setArg(10, mKvStart);
            mGlobalWorkSizeQk[0] = ROUND_UP(mGlobalWorkSizeQk[0], std::max((uint32_t)1, mLocalWorkSizeQk[0]));
            mGlobalWorkSizeQk[1] = ROUND_UP(mGlobalWorkSizeQk[1], std::max((uint32_t)1, mLocalWorkSizeQk[1]));
            MNN_CHECK_CL_SUCCESS(ret, "reSetArg matmul_qk_decode");
        }
        {
            ...
            index++;
            index++;
            ret |= mKernel_softmax->get().setArg(index++, mEffectiveKvSeqlen);
            MNN_CHECK_CL_SUCCESS(ret, "reSetArg softmax");
        }
        {
            uint32_t index = 4;
        ...
        // qk * value
        {
            ...
            index++;
            ret |= mKernel_qkv->get().setArg(index++, mEffectiveKvSeqlen);
            ret |= mKernel_qkv->get().setArg(index++, mKeyValueMaxlen);
            ret |= mKernel_qkv->get().setArg(10, mKvStart);
            MNN_CHECK_CL_SUCCESS(ret, "reSetArg matmul_qkv_decode");
        }
#ifndef ENABLE_OPENCL_TIME_PROFILER
    }
...
ErrorCode AttentionBufExecution::longPrefillResize(const std::vector<Tensor*>& inputs,
    ...
    // rearrange qkv
    {
        ...
        // generate cache for every option
        {
            ...
            auto kernel = runtime->buildKernel("attention_buf", "rearrange_qkv", option, mOpenCLBackend->getPrecision(),
                                               inputs[0], outputs[0]);
        }
        {
            auto option = buildOption;
        ...
        mKernel_rearrange_vec[seq_idx] = runtime->buildKernel("attention_buf", "rearrange_qkv", buildOption,
        ...
        mLwsRearrgVec[seq_idx] =
            localWS3DDefault(mGwsRearrgVec[seq_idx], maxWorkGroupSize, runtime, "rearrange_qkv",
                             mKernel_rearrange_vec[seq_idx], mOpenCLBackend->getCLTuneLevel(), "attention_buf")
                .first;
    ...
    if (mHasMask) {
        ...
        mKernel_mask_vec[seq_idx] = runtime->buildKernel("attention_buf", "rearrange_mask", buildOption,
                                                         mOpenCLBackend->getPrecision(), inputs[0], outputs[0]);
        ...
        mLwsMaskVec[seq_idx] =
            localWS3DDefault(mGwsMaskVec[seq_idx], maxWorkGroupSize, runtime, "rearrange_mask",
                             mKernel_mask_vec[seq_idx], mOpenCLBackend->getCLTuneLevel(), "attention_buf")
                .first;
    ...
    for (int seq_idx = 0; seq_idx < mQseqSplitNum; seq_idx++) {
        // qk matmul
        {
            ...
            int biasType = 0;
            std::vector<cl::Buffer> bufferVec = {openCLBuffer(mTempQ.get()), openCLBuffer(mTempK.get()),
                                                 openCLBuffer(mTempQK.get())};
            if (mHasMask) {
                bufferVec.emplace_back(openCLBuffer(mTempMask.get()));
            } else {
                bufferVec.emplace_back(openCLBuffer(mTempQK.get()));
            }
            if (mIsAddMask) {
                biasType = 2;
            } else if (mHasMask) {
            ...
            auto param = getGemmParams({(uint32_t)e_pack_piece, (uint32_t)h_pack, (uint32_t)l_pack, layout,
                                       ...
                                       bufferVec, mOpenCLBackend->getOpenCLRuntime(), mOpenCLBackend->getPrecision(),
    ...
    // transpose to output
    {
        ...
        mLwsClipVec[seq_idx] =
            localWS3DDefault(mGwsClipVec[seq_idx], maxWorkGroupSize, runtime, "qkv_transpose_output",
                             mKernel_clip_vec[seq_idx], mOpenCLBackend->getCLTuneLevel(), "attention_buf")
                .first;
...
ErrorCode AttentionBufExecution::prefillResize(const std::vector<Tensor*>& inputs,
                                               const std::vector<Tensor*>& outputs) {
    auto runtime = mOpenCLBackend->getOpenCLRuntime();
    auto query = inputs[0];
    auto key = inputs[1];
...

    {
        ...
        mGlobalWorkSizeRearrgQ = {static_cast<uint32_t>(UP_DIV(seqlen, 4)), static_cast<uint32_t>(UP_DIV(headDim, 4)),
                                  static_cast<uint32_t>(numHead * batch)};
        ...
        mLocalWorkSizeRearrgQ = localWS3DDefault(mGlobalWorkSizeRearrgQ, maxWorkGroupSize, runtime, "rearrange_q",
                                                 mKernel_rearrangeQ, mOpenCLBackend->getCLTuneLevel(), "attention_buf")
                                    .first;
        mGlobalWorkSizeRearrgQ[0] =
            ROUND_UP(mGlobalWorkSizeRearrgQ[0], std::max((uint32_t)1, mLocalWorkSizeRearrgQ[0]));
        mGlobalWorkSizeRearrgQ[1] =
            ROUND_UP(mGlobalWorkSizeRearrgQ[1], std::max((uint32_t)1, mLocalWorkSizeRearrgQ[1]));
        mGlobalWorkSizeRearrgQ[2] =
            ROUND_UP(mGlobalWorkSizeRearrgQ[2], std::max((uint32_t)1, mLocalWorkSizeRearrgQ[2]));
    ...
    }
    {
        ...
        mLocalWorkSizeRearrg = localWS3DDefault(mGlobalWorkSizeRearrg, maxWorkGroupSize, runtime, "rearrange_k",
                                                mKernel_rearrange, mOpenCLBackend->getCLTuneLevel(), "attention_buf")
                                   .first;
    ...
    if (mHasMask) {
        ...
        mKernel_rearrangeMask = runtime->buildKernel("attention_buf", "rearrange_mask_shortprefill", buildOption,
                                                     mOpenCLBackend->getPrecision(), inputs[0], outputs[0]);
        mGlobalWorkSizeRearrgM = {static_cast<uint32_t>(UP_DIV(maskQlen, 4)),
                                  static_cast<uint32_t>(UP_DIV(maskKvlen, 4)), static_cast<uint32_t>(batch)};
        ...
        mLocalWorkSizeRearrgM =
            localWS3DDefault(mGlobalWorkSizeRearrgM, maxWorkGroupSize, runtime, "rearrange_mask_shortprefill",
                             mKernel_rearrangeMask, mOpenCLBackend->getCLTuneLevel(), "attention_buf")
                .first;
        mGlobalWorkSizeRearrgM[0] =
            ROUND_UP(mGlobalWorkSizeRearrgM[0], std::max((uint32_t)1, mLocalWorkSizeRearrgM[0]));
        mGlobalWorkSizeRearrgM[1] =
            ROUND_UP(mGlobalWorkSizeRearrgM[1], std::max((uint32_t)1, mLocalWorkSizeRearrgM[1]));
        mGlobalWorkSizeRearrgM[2] =
            ROUND_UP(mGlobalWorkSizeRearrgM[2], std::max((uint32_t)1, mLocalWorkSizeRearrgM[2]));
        ...
        mOpenCLBackend->recordKernel3d(mKernel_rearrangeMask, mGlobalWorkSizeRearrgM, mLocalWorkSizeRearrgM,
                                       &mRgMUpdateInfo);
    }
    {
        // matmul qk
        ...
        mKernel_qk = runtime->buildKernel("attention_buf", "matmul_qk_div_mask_prefill", buildOption,
                                          mOpenCLBackend->getPrecision(), inputs[0], outputs[0]);
        mGlobalWorkSizeQk = {static_cast<uint32_t>(UP_DIV(seqlen, 4)), static_cast<uint32_t>(UP_DIV(mKvSeqlen, 4)),
                             static_cast<uint32_t>(numHead * batch)};
        ...
        mLocalWorkSizeQk = localWS3DDefault(mGlobalWorkSizeQk, maxWorkGroupSize, runtime, "matmul_qk_div_mask_prefill",
                                            mKernel_qk, mOpenCLBackend->getCLTuneLevel(), "attention_buf")
                               .first;
        mGlobalWorkSizeQk[0] = ROUND_UP(mGlobalWorkSizeQk[0], std::max((uint32_t)1, mLocalWorkSizeQk[0]));
        mGlobalWorkSizeQk[1] = ROUND_UP(mGlobalWorkSizeQk[1], std::max((uint32_t)1, mLocalWorkSizeQk[1]));
        mGlobalWorkSizeQk[2] = ROUND_UP(mGlobalWorkSizeQk[2], std::max((uint32_t)1, mLocalWorkSizeQk[2]));
    ...
    }
    {
        ...
        buildOption.emplace("-DSOFTMAX_LOCAL_SIZE=" + std::to_string(localSize));
        mKernel_softmax =
            runtime->buildKernel("softmax_buf", "softmax_v4_buf", buildOption, mOpenCLBackend->getPrecision());
        mGlobalWorkSizeSoftMax = {static_cast<uint32_t>(localSize), static_cast<uint32_t>(UP_DIV(inside, 4)),
        ...
        if (localSize == 1) {
            mLocalWorkSizeSoftMax = localWS3DDefault(mGlobalWorkSizeSoftMax, maxWorkGroupSize, runtime, "softmax",
                                                     mKernel_softmax, mOpenCLBackend->getCLTuneLevel(), "softmax_buf")
                                        .first;
        }
        mGlobalWorkSizeSoftMax[0] =
            ROUND_UP(mGlobalWorkSizeSoftMax[0], std::max((uint32_t)1, mLocalWorkSizeSoftMax[0]));
        mGlobalWorkSizeSoftMax[1] =
            ROUND_UP(mGlobalWorkSizeSoftMax[1], std::max((uint32_t)1, mLocalWorkSizeSoftMax[1]));
        mGlobalWorkSizeSoftMax[2] =
            ROUND_UP(mGlobalWorkSizeSoftMax[2], std::max((uint32_t)1, mLocalWorkSizeSoftMax[2]));
        mSoftMaxUpdateInfo.update_kernel_args.push_back({0, 3, sizeof(cl_mem), &openCLDeferBuffer(mTempQK.get())()});
        mSoftMaxUpdateInfo.update_kernel_args.push_back(
            {0, 4, sizeof(cl_mem), &openCLDeferBuffer(mTempSoftMax.get())()});
    ...
    }
    {
        ...
        mLocalWorkSizeRearrgV = localWS3DDefault(mGlobalWorkSizeRearrgV, maxWorkGroupSize, runtime, "rearrange_v",
                                                 mKernel_rearrangeV, mOpenCLBackend->getCLTuneLevel(), "attention_buf")
                                    .first;
    ...
    // qk * value
    {
        ...
        mKernel_qkv = runtime->buildKernel("attention_buf", "matmul_qkv_prefill", buildOption,
                                           mOpenCLBackend->getPrecision(), inputs[0], outputs[0]);
        auto maxWorkGroupSize = static_cast<uint32_t>(runtime->getMaxWorkGroupSize(mKernel_qkv));
        mGlobalWorkSizeQkv = {static_cast<uint32_t>(UP_DIV(headDim, 8)), static_cast<uint32_t>(UP_DIV(seqlen, 4)),
                              static_cast<uint32_t>(numHead * batch)};
        ...
        ret |= mKernel_qkv->get().setArg(index++, batch);
        MNN_CHECK_CL_SUCCESS(ret, "setArg matmul_qkv_prefill");

        mLocalWorkSizeQkv = localWS3DDefault(mGlobalWorkSizeQkv, maxWorkGroupSize, runtime, "matmul_qkv_prefill",
                                             mKernel_qkv, mOpenCLBackend->getCLTuneLevel(), "attention_buf")
                                .first;
...
ErrorCode AttentionBufExecution::decodeResize(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    mOpenCLBackend->onReleaseBuffer(mTempSoftMax.get(), Backend::DYNAMIC_IN_EXECUTION);
    {
        ...
        mGlobalWorkSizeRearrg = {static_cast<uint32_t>(1), static_cast<uint32_t>(UP_DIV(headDim, 4)),
                                 static_cast<uint32_t>(kvNumHead * batch)};
    ...
    }
    {
        ...
        mKernel_qk = runtime->buildKernel("attention_buf", "matmul_qk_decode", buildOption,
                                          mOpenCLBackend->getPrecision(), inputs[0], outputs[0]);
        mGlobalWorkSizeQk = {static_cast<uint32_t>(UP_DIV(mEffectiveKvSeqlen, 4)), static_cast<uint32_t>(numHead)};
        auto maxWorkGroupSize = static_cast<uint32_t>(runtime->getMaxWorkGroupSize(mKernel_qk));
        ...
        ret |= mKernel_qk->get().setArg(index++, numHead);
        ret |= mKernel_qk->get().setArg(index++, headDim);
        ret |= mKernel_qk->get().setArg(index++, mKvStart);
        MNN_CHECK_CL_SUCCESS(ret, "setArg matmul_qk_decode");

        mLocalWorkSizeQk = localWS2DDefault(mGlobalWorkSizeQk, maxWorkGroupSize, runtime, "matmul_qk_decode",
                                            mKernel_qk, mOpenCLBackend->getCLTuneLevel(), "attention_buf")
                               .first;
        mGlobalWorkSizeQk[0] = ROUND_UP(mGlobalWorkSizeQk[0], std::max((uint32_t)1, mLocalWorkSizeQk[0]));
        mGlobalWorkSizeQk[1] = ROUND_UP(mGlobalWorkSizeQk[1], std::max((uint32_t)1, mLocalWorkSizeQk[1]));
        if (nullptr != mMeta) {
            ...
            mQkUpdateInfo.update_kernel_args.push_back(
                {0, 6, sizeof(mEffectiveKvSeqlen), &mEffectiveKvSeqlen});
            mQkUpdateInfo.update_kernel_args.push_back({0, 7, sizeof(mKeyValueMaxlen), &mKeyValueMaxlen});
            mQkUpdateInfo.update_kernel_args.push_back({0, 10, sizeof(mKvStart), &mKvStart});
    ...
    }
    {
        ...
        buildOption.emplace("-DSOFTMAX_LOCAL_SIZE=" + std::to_string(localSize));
        mKernel_softmax =
            runtime->buildKernel("softmax_buf", "softmax_in1_buf", buildOption, mOpenCLBackend->getPrecision());
        mGlobalWorkSizeSoftMax = {static_cast<uint32_t>(localSize), static_cast<uint32_t>(inside),
                                  static_cast<uint32_t>(outside)};
        auto maxWorkGroupSize = static_cast<uint32_t>(runtime->getMaxWorkGroupSize(mKernel_softmax));
        ...
        if (nullptr != mMeta) {
            mSoftMaxUpdateInfo.update_kernel_args.push_back(
                {0, 3, sizeof(cl_mem), &openCLDeferBuffer(mTempQK.get())()});
            mSoftMaxUpdateInfo.update_kernel_args.push_back(
                {0, 4, sizeof(cl_mem), &openCLDeferBuffer(mTempSoftMax.get())()});
            mSoftMaxUpdateInfo.update_kernel_args.push_back(
                {0, 7, sizeof(mEffectiveKvSeqlen), &mEffectiveKvSeqlen});
            mOpRecordUpdateInfo.emplace_back(&mSoftMaxUpdateInfo);
            mOpenCLBackend->recordKernel3d(mKernel_softmax, mGlobalWorkSizeSoftMax, mLocalWorkSizeSoftMax,
                                           &mSoftMaxUpdateInfo);
        } else {
    ...
    }
    {
        ...
        mGlobalWorkSizeRearrgV = {static_cast<uint32_t>(UP_DIV(headDim, 4)), static_cast<uint32_t>(1),
                                  static_cast<uint32_t>(kvNumHead * batch)};
    ...
    // qk * value
    {
        ...
        for (int i = 0; i < actual_kernel; i++) {
            for (int j = 0; j < actual_kernel; j++) {
                ...
                if (mOutputC4) {
                    option.emplace("-DATTENTION_C4");
                }
                kernel[knl_idx] = mOpenCLBackend->getOpenCLRuntime()->buildKernel(
                    "attention_buf", kernelName[i], option, mOpenCLBackend->getPrecision());
                uint32_t maxWorkGroupSize =
                    static_cast<uint32_t>(mOpenCLBackend->getOpenCLRuntime()->getMaxWorkGroupSize(kernel[knl_idx]));
                globalWorkSize[knl_idx] = {static_cast<uint32_t>(UP_DIV(headDim, itemC[i])),
                ...
                ret |= kernel[knl_idx]->get().setArg(index++, mEffectiveKvSeqlen);
                ...
                ret |= kernel[knl_idx]->get().setArg(index++, mKvStart);
        ...
        mKernel_qkv = runtime->buildKernel("attention_buf", kernelName[min_index], buildOption,
                                           mOpenCLBackend->getPrecision(), inputs[0], outputs[0]);
        ...
        ret |= mKernel_qkv->get().setArg(index++, kvNumHead);
        ret |= mKernel_qkv->get().setArg(index++, headDim);
        ret |= mKernel_qkv->get().setArg(index++, mKvStart);
        MNN_CHECK_CL_SUCCESS(ret, "setArg matmul_qkv_decode");

        mGlobalWorkSizeQkv[0] = ROUND_UP(mGlobalWorkSizeQkv[0], std::max((uint32_t)1, mLocalWorkSizeQkv[0]));
        mGlobalWorkSizeQkv[1] = ROUND_UP(mGlobalWorkSizeQkv[1], std::max((uint32_t)1, mLocalWorkSizeQkv[1]));
        if (nullptr != mMeta) {
            mQkvUpdateInfo.update_kernel_args.push_back(
                {0, 2, sizeof(cl_mem), &openCLDeferBuffer(mTempSoftMax.get())()});
            mQkvUpdateInfo.update_kernel_args.push_back({0, 3, sizeof(cl_mem), &(*(mKVCacheCLManager->value()))()});
            mQkvUpdateInfo.update_kernel_args.push_back(
                {0, 5, sizeof(mEffectiveKvSeqlen), &mEffectiveKvSeqlen});
            mQkvUpdateInfo.update_kernel_args.push_back({0, 6, sizeof(mKeyValueMaxlen), &mKeyValueMaxlen});
            mQkvUpdateInfo.update_kernel_args.push_back({0, 10, sizeof(mKvStart), &mKvStart});
            mOpRecordUpdateInfo.emplace_back(&mQkvUpdateInfo);
...
ErrorCode AttentionBufExecution::onResize(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    mIsDecode = seqlen == 1 && (nullptr == mMeta || mMeta->add == 1);
                ...
                if (getTunedInfo(info, {static_cast<unsigned int>(seqlen)}, tuneInfo,
                    ...
                    if (mOpenCLBackend->getCLTuneLevel() != None) {
                        ...
                        std::pair<std::vector<uint32_t>, uint32_t> tuneInfoTmp =
                            std::make_pair<std::vector<uint32_t>, uint32_t>({mLongPrefill}, 0);
                        setTunedInfo(info, {static_cast<unsigned int>(seqlen)}, tuneInfoTmp,
                                     mOpenCLBackend->getOpenCLRuntime(), "attention_buf");
                        init();
                    } else {
                        if (seqlen > 512) {
                            mLongPrefill = true;
...
int AttentionBufExecution::getExecuteTime() {
    ...
    if (mLongPrefill) {
        ...
        cl::Event event0, event1, event2, event3, event4, event5, event6;
        run3DKernelDefault(mKernel_rearrangeQ, mGlobalWorkSizeRearrgQ, mLocalWorkSizeRearrgQ,
                           mOpenCLBackend->getOpenCLRuntime(), &event0);
        executeTime += runtime->getEventTime(event0);
        ...
        executeTime += runtime->getEventTime(event1);
        if (mHasMask) {
            run3DKernelDefault(mKernel_rearrangeMask, mGlobalWorkSizeRearrgM, mLocalWorkSizeRearrgM,
                               mOpenCLBackend->getOpenCLRuntime(), &event2);
            executeTime += runtime->getEventTime(event2);
        }
...
AttentionBufExecution::AttentionBufExecution(const MNN::Op* op, Backend* backend, bool outputC4)
    ...
    mSlidingWindow = op->main_as_AttentionParam()->sliding_window();
...
AttentionBufExecution::AttentionBufExecution(std::shared_ptr<KVCacheCLManager> manager, const MNN::Op* op,
    ...
    mSlidingWindow = param->sliding_window();
    auto kernel = mOpenCLBackend->getOpenCLRuntime()->buildKernel(
        "softmax_buf", "softmax_buf", {"-DSOFTMAX_LOCAL_SIZE=512"}, mOpenCLBackend->getPrecision());
    OPENCL_CHECK_KERNEL_CTOR(kernel);
    mMaxWorkGroupSize = static_cast<uint32_t>(mOpenCLBackend->getOpenCLRuntime()->getMaxWorkGroupSize(kernel));
