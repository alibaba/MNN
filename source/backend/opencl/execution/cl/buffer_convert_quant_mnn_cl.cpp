...
const char* buffer_convert_quant =
    ...
    " const int buffer_width_idx=height_width_idx % kernel_shape.y;\n"
    " const int "
    "buffer_offset=output_channel_4_idx*ic_h_w_size+input_channel_4_idx*height_width_size+buffer_height_idx*kernel_"
    "shape.y+buffer_width_idx;\n"
    " int "
    "index0=buffer_offset,index1=buffer_offset+ic_h_w_size,index2=buffer_offset+2*ic_h_w_size,index3=buffer_offset+3*"
    "ic_h_w_size;\n"
    ...
    " output_values_int4.x=((index0 % 2) == 0 ? (s0 & 0xf0) : (s0 << 4)) | ((index1 % 2) == 0 ? (s1 >> 4) : (s1 & "
    "0x0f));\n"
    " output_values_int4.y=((index2 % 2) == 0 ? (s2 & 0xf0) : (s2 << 4)) | ((index3 % 2) == 0 ? (s3 >> 4) : (s3 & "
    "0x0f));\n"
    ...
    " char16 "
    "out0=(char16)(s0.s0,s1.s0,s2.s0,s3.s0,s4.s0,s5.s0,s6.s0,s7.s0,s0.s1,s1.s1,s2.s1,s3.s1,s4.s1,s5.s1,s6.s1,s7.s1);\n"
    " char16 "
    "out1=(char16)(s0.s2,s1.s2,s2.s2,s3.s2,s4.s2,s5.s2,s6.s2,s7.s2,s0.s3,s1.s3,s2.s3,s3.s3,s4.s3,s5.s3,s6.s3,s7.s3);\n"
