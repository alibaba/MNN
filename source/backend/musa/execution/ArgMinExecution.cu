...
__global__ void ArgMinKernel(const T* input, int* output,
    ...
    if (outerIdx < outerSize) {
        T minVal = input[outerIdx * axisSize * innerSize];
        int minIdx = 0;
        ...
        output[outerIdx] = minIdx;
    }
}

ArgMinExecution::ArgMinExecution(const std::vector<Tensor*>& inputs, const MNN::Op* op, Backend* backend)
    : Execution(inputs, {}, backend) {
    mBackend = static_cast<MusaBackend*>(backend);
    mOp = op->main_as_ArgMin();
}
...
ErrorCode ArgMinExecution::onExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    ArgMinKernel<<<mDim3Grid, mDim3Block>>>(
        inputPtr, outputPtr,
