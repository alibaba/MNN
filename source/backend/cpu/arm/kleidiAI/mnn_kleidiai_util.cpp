...
void KleidiAIUtil::packNCHWToNC4HW4(float* data, size_t rowNum, size_t rowSize) {
    if(rowNum == 1) {
        return;
    }

    const size_t tmpSize = rowNum * rowSize * sizeof(float);
    uint8_t *tmpBuffer = new uint8_t[tmpSize];
    memcpy(tmpBuffer, data, tmpSize);
...
void KleidiAIUtil::packNCHWToNC4HW4(__fp16* data, size_t rowNum, size_t rowSize) {
    if(rowNum == 1) {
        return;
    }

    const size_t tmpSize = rowNum * rowSize * sizeof(__fp16);
    uint8_t *tmpBuffer = new uint8_t[tmpSize];
    memcpy(tmpBuffer, data, tmpSize);

    const __fp16 *src = (const __fp16 *)tmpBuffer;
    __fp16 *dst = (__fp16 *)data;
...
void KleidiAIUtil::packNC4HW4ToNCHW(float* data, size_t rowNum, size_t rowSize) {
    if(rowNum == 1) {
        return;
    }
...
void KleidiAIUtil::packNC4HW4ToNCHW(__fp16* data, size_t rowNum, size_t rowSize) {
    if(rowNum == 1) {
        return;
    }
...
void KleidiAIUtil::packQsi4cxpoQsu4cxs0s1(
    size_t num_groups, size_t n, size_t k, size_t nr, size_t kr, size_t sr, const uint8_t* rhs, const float* bias,
    const float* scale, void* rhs_packed, size_t extra_bytes,
    const struct KleidiAIUtil::rhsPackParamCommon* paramsCommon) {
    const size_t k_internal = kai_roundup(k, 32);
    ...
    const struct kai_rhs_pack_nxk_qsi4cxpo_qsu4cxs1s0_params *params = (kai_rhs_pack_nxk_qsi4cxpo_qsu4cxs1s0_params *)paramsCommon;
    KAI_ASSERT(params != NULL);
    KAI_ASSERT(params->lhs_zero_point == 1);
    ...
    for (size_t row_idx = 0; row_idx < n; row_idx += nr) {
        ...
        float32_t* const scaling_factors = (float32_t*)((uint8_t*)sums + (nr * kai_num_bytes_adder_rhs));
