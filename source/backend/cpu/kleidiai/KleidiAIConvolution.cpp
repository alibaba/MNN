...
void KleidiAIConvolution::runRhsPack(size_t numGroups, size_t n, size_t k, size_t rhsStride, const void* rhs,
                                     const void* scale, const void* bias, void* rhsPacked) const {
    mUkernel.runRhsPack(numGroups, n, k, getNr(), getKr(), getSr(), rhsStride, rhs, bias, scale, rhsPacked, 0, nullptr);
}
...
void KleidiAIConvolution::runMatmul(size_t m, size_t n, size_t k, const void* lhsPacked, const void* rhsPacked,
    ...
    if (m == 1) {
        ...
        mUkernel.matmulGemv(m, n, k, lhsPacked, k * mElementSize, rhsPacked, dst, dstStrideRow, dstStrideCol, scalarMin,
                            scalarMax);
    } else {
        mUkernel.matmulGemm(m, n, k, lhsPacked, rhsPacked, dst, dstStrideRow, dstStrideCol, scalarMin, scalarMax);
    }
}
...
KleidiAIConvolution::KleidiAIConvolution(const Convolution2DCommon* common, Backend* b, const float* originWeight,
    ...
    if (core->bytes == 2) {
        ...
        // Run rhs pack.
        runRhsPack(1, outputCount, mSrcCount, mSrcCount * sizeof(__fp16), tempTensor->host<void>(), nullptr,
                   tempBiasTensor->host<void>(), mResource->mWeight->host<void>());
        b->onReleaseBuffer(tempBiasTensor.get(), Backend::STATIC);
        b->onReleaseBuffer(tempTensor.get(), Backend::STATIC);
    } else {
        ...
        // Run rhs pack.
        runRhsPack(1, outputCount, mSrcCount, mSrcCount * sizeof(float), originWeight, nullptr, bias,
                   mResource->mWeight->host<void>());
    }
}

KleidiAIConvolution::KleidiAIConvolution(std::shared_ptr<CPUConvolution::Resource> resource,
...
ErrorCode KleidiAIConvolution::onExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    runMatmul(m, n, k, lhsPacked, weightPtr, outputPtr, n * elementSize, elementSize, mPostParameters[3],
              mPostParameters[2]);
