...
CPUInterp::CPUInterp(Backend* backend, int resizeType, float widthScale, float heightScale, float widthOffset,
                     float heightOffset)
...
ErrorCode CPUInterp::onExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    auto area_in = inputs[0]->width() * inputs[0]->height();
    auto area_out = outputs[0]->width() * outputs[0]->height();
    ...
    if (!interpInt8) {
        switch (mResizeType) {
            case 1:
                CPUResizeNearestneighborC4<float>(inputs, outputs, mWidthScale, mHeightScale, mWidthOffset,
                                                  mHeightOffset);
                break;
            case 2:
                CPUResizeBilinearC4<float, float>(CPUBilinearSampleC4, CPUBilinearLineC4, inputs, outputs,
                                                  mWidthPosition.host<int>(), mWidthFactor.host<float>(),
                                                  mHeightPosition.host<int>(), mHeightFactor.host<float>(),
                                                  mLineBuffer.host<float>(), ((CPUBackend*)backend())->threadNumber(),
                                                  &mInputQuantZero, &mOutputQuantZero);
                break;
            case 3:
                CPUResizeCubicC4<float>(MNNCubicSampleC4, MNNCubicLineC4, inputs, outputs, mWidthScale, mHeightScale,
                                        mWidthOffset, mHeightOffset, &mInputQuantZero, &mOutputQuantZero,
                                        mOutputQuantMIn, mOutputQuantMax);
                break;
            case 4:
                CPUResizeNearestneighborRoundC4<float>(inputs, outputs, mWidthScale, mHeightScale, mWidthOffset,
                                                       mHeightOffset);
                break;
            default:
                return NOT_SUPPORT;
    ...
    const bool nativeInt8Pack = core->pack > 16;

    // Pack
    if (nativeInt8Pack) {
        // Runtime pack can be 32 on RVV VLEN=1024. Execute that layout directly.
    } else if ((mResizeType == 1 || mResizeType == 2) && (core->pack == 4)) {
    ...
    switch (mResizeType) {
        case 1:
            if (nativeInt8Pack) {
                CPUResizeNearestneighborCUnitInt8(int8ExeInputs, int8ExeOutputs, mWidthScale, mHeightScale,
                                                  mWidthOffset, mHeightOffset, core->pack);
            } else {
                CPUResizeNearestneighborC4<int8_t>(int8ExeInputs, int8ExeOutputs, mWidthScale, mHeightScale,
                                                   mWidthOffset, mHeightOffset);
            }
            break;
        case 2:
            if (nativeInt8Pack) {
                CPUResizeBilinearCUnitInt8(
                    int8ExeInputs, int8ExeOutputs, mWidthPosition.host<int>(), mWidthFactor.host<float>(),
                    mHeightPosition.host<int>(), mHeightFactor.host<float>(), mLineBuffer.host<int16_t>(),
                    ((CPUBackend*)backend())->threadNumber(), &mInputQuantZero, &mOutputQuantZero, core->pack);
            } else {
                CPUResizeBilinearC4<int8_t, int16_t>(
                    MNNBilinearSampleC8, MNNBilinearLineC8, int8ExeInputs, int8ExeOutputs, mWidthPosition.host<int>(),
                    mWidthFactor.host<float>(), mHeightPosition.host<int>(), mHeightFactor.host<float>(),
                    mLineBuffer.host<int16_t>(), ((CPUBackend*)backend())->threadNumber(), &mInputQuantZero,
                    &mOutputQuantZero);
            }
            break;
        case 3:
            if (nativeInt8Pack) {
                CPUResizeCubicCUnitInt8(int8ExeInputs, int8ExeOutputs, mWidthScale, mHeightScale, mWidthOffset,
                                        mHeightOffset, &mInputQuantZero, &mOutputQuantZero, mOutputQuantMIn,
                                        mOutputQuantMax, core->pack);
            } else {
                CPUResizeCubicC4<int8_t>(MNNCubicSampleC16, MNNCubicLineC16, int8ExeInputs, int8ExeOutputs, mWidthScale,
                                         mHeightScale, mWidthOffset, mHeightOffset, &mInputQuantZero, &mOutputQuantZero,
                                         mOutputQuantMIn, mOutputQuantMax);
            }
            break;
        case 4:
            if (nativeInt8Pack) {
                CPUResizeNearestneighborRoundCUnitInt8(int8ExeInputs, int8ExeOutputs, mWidthScale, mHeightScale,
                                                       mWidthOffset, mHeightOffset, core->pack);
            } else {
                CPUResizeNearestneighborRoundC4<int8_t>(int8ExeInputs, int8ExeOutputs, mWidthScale, mHeightScale,
                                                        mWidthOffset, mHeightOffset);
            }
            break;
        default:
    ...
    // Unpack
    if (nativeInt8Pack) {
        // Native runtime pack path already wrote output layout.
    } else if ((mResizeType == 1 || mResizeType == 2) && (core->pack == 4)) { // pack=8 -> pack=4
        MNNUnpackInt8C2Origin(outputs[0]->host<float>(), mOutputTemp.get()->host<float>(), plane_out, depth, plane_out);
    } else if ((mResizeType == 1 || mResizeType == 2) && (core->pack != 8)) {
        repackInt8(mOutputTemp.get()->host<int8_t>(), outputs[0]->host<int8_t>(), outputs[0]->batch(), area_out,
                   outputs[0]->channel(), 8, core->pack);
        ...
        if (core->pack == 4) {
            ...
            MNNUnpackC2Origin(outputs[0]->host<double>(), mOutputTemp.get()->host<double>(), plane_out, depth,
                              plane_out);
        } else if (core->pack != 16) {
            repackInt8(mOutputTemp.get()->host<int8_t>(), outputs[0]->host<int8_t>(), outputs[0]->batch(), area_out,
                       outputs[0]->channel(), 16, core->pack);
        }
    }
#endif
    return NO_ERROR;
...
ErrorCode CPUInterp::onResize(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    bool useInt8 = (CPUBackend::getDataType(inputs[0]) == DataType_DT_INT8 || inputs[0]->getType().bytes() == 1) &&
                   (CPUBackend::getDataType(outputs[0]) == DataType_DT_INT8 || outputs[0]->getType().bytes() == 1);
    if (useInt8) {
        auto core = static_cast<CPUBackend*>(backend())->functions();
        if (core->pack > packInt8) {
            packInt8 = core->pack;
        }
    }
    if (useInt8) {
        mInputTemp.reset(Tensor::createDevice<int8_t>(
            {inputs[0]->batch(), inH, inW, UP_DIV(inputs[0]->channel(), packInt8) * packInt8}));
        mOutputTemp.reset(Tensor::createDevice<int8_t>(
            {outputs[0]->batch(), outH, outW, UP_DIV(outputs[0]->channel(), packInt8) * packInt8}));
...
public:
    virtual Execution* onCreate(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
        ...
        return new CPUInterp(backend, interp->resizeType(), interp->widthScale(), interp->heightScale(),
                             interp->widthOffset(), interp->heightOffset());
    }
};
