...
template <int SHIFT>
static inline vint32m4_t truncDivPow2(vint32m4_t x, size_t vl) {
    vbool8_t neg = __riscv_vmslt_vx_i32m4_b8(x, 0, vl);
    vint32m4_t fix = __riscv_vmerge_vxm_i32m4(__riscv_vmv_v_x_i32m4(0, vl), (1 << SHIFT) - 1, neg, vl);
...
void MNNBinaryMaxInt8_RVV(int8_t* outputRaw, const int8_t* inputRaw0, const int8_t* inputRaw1,
    ...
    for (size_t i = 0; i < elementSize; i += vl) {
        ...
        // Scalar reference: res < 0 ? (res - 32768) / 65536 : (res + 32768) / 65536
        ...
        vres = truncDivPow2<16>(__riscv_vadd_vv_i32m4(vres, offset_val, vl), vl);
