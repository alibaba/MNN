...
void MNNBinarySqdInt8_RVV(int8_t* outputRaw, const int8_t* inputRaw0, const int8_t* inputRaw1,
    ...
    for (size_t i = 0; i < elementSize; i += vl) {
        ...
        if (needBroadcast == 0) {
            ...
            vf1 = __riscv_vfmul_vf_f32m4(
                __riscv_vfcvt_f_x_v_f32m4(__riscv_vwadd_vx_i32m4(__riscv_vwsub_vx_i16m2(vi1, zp1, vl), 0, vl), vl),
                scale1, vl);
        } else if (needBroadcast == 1) {
            vf1 = __riscv_vfmv_v_f_f32m4(scalar_val1, vl);
            vint8m1_t vi0 = __riscv_vle8_v_i8m1(inputRaw0 + i, vl);
            vf0 = __riscv_vfmul_vf_f32m4(
                __riscv_vfcvt_f_x_v_f32m4(__riscv_vwadd_vx_i32m4(__riscv_vwsub_vx_i16m2(vi0, zp0, vl), 0, vl), vl),
                scale0, vl);
        } else {
            vint8m1_t vi0 = __riscv_vle8_v_i8m1(inputRaw0 + i, vl);
            vint8m1_t vi1 = __riscv_vle8_v_i8m1(inputRaw1 + i, vl);
        ...
        // ������ƽ��
        vfloat32m4_t vdiff = __riscv_vfsub_vv_f32m4(vf0, vf1, vl);
        vfloat32m4_t vres = __riscv_vfmul_vv_f32m4(vdiff, vdiff, vl);
        vres = __riscv_vfmul_vf_f32m4(vres, scaleOut, vl);

        vint32m4_t vout32 = __riscv_vfcvt_x_f_v_i32m4(vres, vl);
        vout32 = __riscv_vadd_vx_i32m4(vout32, zpOut, vl);
        vout32 = __riscv_vmax_vx_i32m4(__riscv_vmin_vx_i32m4(vout32, maxV, vl), minV, vl);

        __riscv_vse8_v_i8m1(outputRaw + i, __riscv_vncvt_x_x_w_i8m1(__riscv_vncvt_x_x_w_i16m2(vout32, vl), vl), vl);
    }
}