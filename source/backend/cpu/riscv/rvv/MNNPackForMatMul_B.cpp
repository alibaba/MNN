        ...
        for (size_t k = 0; k < kernelsize; ++k) {
            for (size_t x = 0; x < ic; ++x) {
                ...
                for (size_t yHp = 0; yHp < y_len; yHp += vl) {
                    vl = __riscv_vsetvl_e32m8(y_len - yHp);

                    vint32m8_t v_src;
                    if (!transpose) {
                        v_src = __riscv_vle32_v_i32m8(src_ptr + yHp, vl);
                    } else {
                        v_src = __riscv_vlse32_v_i32m8(src_ptr + yHp * l, l * sizeof(int32_t), vl);
                    }

                    __riscv_vse32_v_i32m8(dst_ptr + yHp, v_src, vl);
                }
            }
        }
    }
