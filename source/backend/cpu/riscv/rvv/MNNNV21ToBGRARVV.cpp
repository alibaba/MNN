...
void MNNNV21ToBGRARVV(const unsigned char* source, unsigned char* dest, size_t count) {
    ...
    while (i < count) {
        ...
        vint16m4_t y16 = __riscv_vreinterpret_v_u16m4_i16m4(__riscv_vzext_vf2_u16m4(channel8, vl));

        vuint8m1_t half8 = __riscv_vlse8_v_u8m1(uv + (i / 2) * 2, 2, vlHalf);
        channel8 = __riscv_vrgather_vv_u8m2(__riscv_vlmul_ext_v_u8m1_u8m2(half8), dupIdx, vl);
        vint16m4_t v16 =
            __riscv_vsub_vx_i16m4(__riscv_vreinterpret_v_u16m4_i16m4(__riscv_vzext_vf2_u16m4(channel8, vl)), 128, vl);
        ...
        vint32m8_t y32 = __riscv_vsll_vx_i32m8(__riscv_vwcvt_x_x_v_i32m8(y16, vl), 6, vl);
        ...
        channel8 = __riscv_vncvt_x_x_w_u8m2(__riscv_vreinterpret_v_i16m4_u16m4(res16), vl);
        __riscv_vsse8_v_u8m2(dest + 4 * i + 2, 4, channel8, vl);
        ...
        channel8 = __riscv_vncvt_x_x_w_u8m2(__riscv_vreinterpret_v_i16m4_u16m4(res16), vl);
        __riscv_vsse8_v_u8m2(dest + 4 * i + 1, 4, channel8, vl);
        ...
        res16 = __riscv_vncvt_x_x_w_i16m4(calc32, vl);
        channel8 = __riscv_vncvt_x_x_w_u8m2(__riscv_vreinterpret_v_i16m4_u16m4(res16), vl);
        __riscv_vsse8_v_u8m2(dest + 4 * i + 0, 4, channel8, vl);

        channel8 = __riscv_vmv_v_x_u8m2(255, vl);
        __riscv_vsse8_v_u8m2(dest + 4 * i + 3, 4, channel8, vl);
    ...
    for (; i < count; ++i) {
        ...
        dest[4 * i + 2] = (uint8_t)R;
        dest[4 * i + 1] = (uint8_t)G;
        dest[4 * i + 0] = (uint8_t)B;
        dest[4 * i + 3] = 255;
    }
}

void MNNNV21ToBGRA(const unsigned char* source, unsigned char* dest, size_t count) {
    MNNNV21ToBGRARVV(source, dest, count);
}
