...
AttentionExecution::AttentionExecution(Backend* backend, bool kv_cache_op_param, int op_head_dim)
    : Execution(backend),
      mCudaBackend(static_cast<CUDABackend*>(backend)),
      mIsKVCacheEnabled(kv_cache_op_param),
      mOpHeadDim(op_head_dim),
      mScale(1.0f),
      mBatch(0),
      mQuerySeqLen(0),
      mNumHead(0),
...
AttentionExecution::~AttentionExecution() {
    ...
    releaseBuffer(mC4QueryContiguous);
    releaseBuffer(mC4KeyContiguous);
...
ErrorCode AttentionExecution::ensureC4TailBuffers_gpu() {
    ...
    if (mQueryC4HasTail) {
        std::vector<int> shape = {mQuerySeqLen, mNumHead * mHeadDim, 1, 1};
        auto code = ensureBuffer(mC4QueryContiguous, shape);
    ...
    if (mKeyC4HasTail) {
        std::vector<int> shape = {mNewKvSeqLen, mKvNumHead * mHeadDim, 1, 1};
        auto code = ensureBuffer(mC4KeyContiguous, shape);
        if (code != MNN::NO_ERROR) {
            return code;
        }
    }
...
bool AttentionExecution::onClone(Backend* bn, const Op* op, Execution** dst) {
    ...
    auto exe = new AttentionExecution(bn, mIsKVCacheEnabled, mOpHeadDim);
    exe->mCache = mCache;
    exe->mMeta = mMeta;
    *dst = exe;
    return true;
...
ErrorCode AttentionExecution::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    ...
    mInputC4 = TensorUtils::getDescribe(query_tensor)->dimensionFormat == MNN_DATA_FORMAT_NC4HW4;
    if (mInputC4) {
        const bool validFormat = TensorUtils::getDescribe(key_tensor)->dimensionFormat == MNN_DATA_FORMAT_NC4HW4 &&
                                 TensorUtils::getDescribe(value_tensor)->dimensionFormat == MNN_DATA_FORMAT_NC4HW4 &&
                                 TensorUtils::getDescribe(outputs[0])->dimensionFormat == MNN_DATA_FORMAT_NC4HW4;
        if (!validFormat || mOpHeadDim <= 0 || query_tensor->dimensions() != 4 || key_tensor->dimensions() != 4 ||
            value_tensor->dimensions() != 4 || query_tensor->length(2) != 1 || query_tensor->length(3) != 1 ||
            key_tensor->length(2) != 1 || key_tensor->length(3) != 1 || value_tensor->length(2) != 1 ||
            value_tensor->length(3) != 1 || key_tensor->length(0) != value_tensor->length(0) ||
            key_tensor->length(1) != value_tensor->length(1) || query_tensor->length(1) % mOpHeadDim != 0 ||
            key_tensor->length(1) % mOpHeadDim != 0) {
            MNN_ERROR("CUDA Attention: invalid C4 q/k/v layout or head configuration.\n");
            return MNN::INVALID_VALUE;
        }
        mBatch = 1;
        mQuerySeqLen = query_tensor->length(0);
        mNumHead = query_tensor->length(1) / mOpHeadDim;
        mHeadDim = mOpHeadDim;
        mKvNumHead = key_tensor->length(1) / mOpHeadDim;
        mNewKvSeqLen = key_tensor->length(0);
        mQueryC4HasTail = query_tensor->length(1) % PACK_NUMBER != 0;
        mKeyC4HasTail = key_tensor->length(1) % PACK_NUMBER != 0;
        mValueC4HasTail = value_tensor->length(1) % PACK_NUMBER != 0;
    } else {
        mQueryC4HasTail = false;
        mKeyC4HasTail = false;
        ...
        if (TensorUtils::getDescribe(value_tensor)->dimensionFormat == MNN_DATA_FORMAT_NC4HW4) {
            ...
            if (value_tensor->dimensions() != 4 || value_tensor->length(0) != mBatch * mNewKvSeqLen ||
                ...
                MNN_ERROR("CUDA Attention: invalid legacy C4 value layout.\n");
                return MNN::INVALID_VALUE;
            }
    ...
    if (mQueryC4HasTail || mKeyC4HasTail || mValueC4HasTail || mOutputC4HasTail) {
...
ErrorCode AttentionExecution::onExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    ...
    if (mQueryC4HasTail) {
        FormatConvert(getTensorDevicePtr(mC4QueryContiguous.get()), getTensorDevicePtr(query_input_tensor),
                      MNN_DATA_FORMAT_NC4HW4, MNN_DATA_FORMAT_NCHW, runtime, 1, mQuerySeqLen,
                      mNumHead * mHeadDim, query_input_tensor, mPrecision, true, true);
        query_execution_tensor = mC4QueryContiguous.get();
    }
    if (mOutputC4HasTail) {
        output_execution_tensor = mC4OutputContiguous.get();
    }
    if (mKeyC4HasTail) {
        FormatConvert(getTensorDevicePtr(mC4KeyContiguous.get()), getTensorDevicePtr(key_input_tensor),
                      MNN_DATA_FORMAT_NC4HW4, MNN_DATA_FORMAT_NCHW, runtime, 1, mNewKvSeqLen,
                      mKvNumHead * mHeadDim, key_input_tensor, mPrecision, true, true);
        key_execution_tensor = mC4KeyContiguous.get();
    }
    if (mValueC4HasTail) {
    ...
    virtual Execution* onCreate(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
        ...
        if (op->main_type() == OpParameter_AttentionParam) {
             ...
             if (att_param) {
                 ...
                 op_head_dim = att_param->head_dim();
             }
        } else {
            MNN_PRINT("[Warning] op->main_type() != OpParameter_AttentionParam\n");
        }
        return new AttentionExecution(backend, op_kv_cache_enabled, op_head_dim);
    }
};
