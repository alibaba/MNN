...
static void createProfileHandle(const QNN_INTERFACE_VER_TYPE& QnnInterface, const Qnn_BackendHandle_t& backend_handle, Qnn_ProfileHandle_t* profile_handle_ptr) {
...
public:
    ...
    bool compileModel(const std::string& path, size_t offset, size_t size, const std::vector<std::string>& allGraphName) {
        ...
        // 2. Retrieve graphs.
        {
            auto error = QNN::gContext.QnnInterface.contextValidateBinary(QNN::gContext.backendHandle, QNN::gContext.deviceHandle, mQnnContextConfig, buffer, size);
            ...
            CALL_QNN(QNN::gContext.QnnInterface.contextCreateFromBinary(QNN::gContext.backendHandle, QNN::gContext.deviceHandle, mQnnContextConfig, buffer, size, &mQnnContextHandle, mQnnProfileHandle));
            ...
            for (int i = 0; i < mGraphCount; i++) {
                CALL_QNN(QNN::gContext.QnnInterface.graphRetrieve(mQnnContextHandle, mGraphsInfo[i]->graphName, &(mQnnGraphHandleVec[i])));
            }
        }
...
private:
    ...
    void _loadState(int stateNumber, std::vector<int> seqLen) {
        ...
        for (int i=0; i<mStateInput.size(); ++i) {
            mStateInput[i].data.reset(RPCBuffer::alloc(mStateMaxSize * mStateInput[i].inside * mStateInput[i].outside * sizeof(mnn_qnn_fp16_t)));
            mStateInput[i].update.resize(seqLen.size());
            for (int j=0; j<seqLen.size(); ++j) {
                mStateInput[i].update[j].reset(RPCBuffer::alloc(mStateInput[i].inside * mStateInput[i].outside * seqLen[j] * sizeof(mnn_qnn_fp16_t)));
            }
        }
    }
...
QnnRuntime* QnnRuntime::create(const Backend::Info& info) {
    ...
    // Create Interface.
    return new QnnRuntime(info, gContext.QnnInterface, gContext.logHandle, gContext.backendHandle, gContext.deviceHandle);
}
