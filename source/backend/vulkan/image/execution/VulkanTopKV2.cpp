...
VulkanTopKV2::VulkanTopKV2(const Op* op, Backend* bn) : VulkanBasicExecution(bn) {
    ...
    mGpuParam.reset(new VulkanBuffer(vkBn->getMemoryPool(), false, sizeof(GpuTopKV2Param), nullptr,
...
ErrorCode VulkanTopKV2::onEncode(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
    ...
    mDescriptorSet.reset(mPipeline->createSet());
    mDescriptorSet->writeImage(vkOutValue->image()->view(), vkBn->getCommonSampler()->get(), VK_IMAGE_LAYOUT_GENERAL,
                               0);
    mDescriptorSet->writeImage(vkOutIndex->image()->view(), vkBn->getCommonSampler()->get(), VK_IMAGE_LAYOUT_GENERAL,
                               1);
...
public:
    virtual VulkanBasicExecution* onCreate(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
        ...
        if (_topKFromOutput(outputs[0]) <= 0) {
            return nullptr;
        }
        return new VulkanTopKV2(op, backend);
    }
};
