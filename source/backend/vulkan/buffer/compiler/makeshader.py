...
def _build_generated_shader_path(raw, macro, precision):
    ...
    if macro:
        fileName = fileName.replace(".comp", "_" + macro + ".comp")
    if precision == 'fp16':
        fileName = fileName.replace(".comp", "_FP16" + ".comp")
...
class ShaderFile:
    ...
    def __init__(self, shader='', ref=False, raw='', precision='fp32'):
        self.shaderFile = shader  # string , source .comp file
        self.refFile = ref  # (Deprecated, always True) bool , reference file , org file not exists ,auto generated by script
    ...
    def getFileName(self):
        logicalName = self.rawRefFile

        if self.appendMacro:
            logicalName = logicalName.replace(".comp", "_" + self.appendMacro + ".comp")

        if self.precision == 'fp16':
            logicalName = logicalName.replace(".comp", "_FP16.comp")

        normalized = os.path.normpath(logicalName)
        parts = normalized.split(os.sep)
        if "glsl" in parts:
            parts = parts[parts.index("glsl"):]
        else:
            parts = [parts[-1]]
...
def genShaderFileObjs(shaders, macros):
    shaderObjs = []
    _ensure_dir(gGeneratedShaderDir)
    for fileName in shaders:
        simplename = os.path.basename(fileName)
        cfg = macros.get(simplename, {})
        if isinstance(cfg, dict):
            macro_list = cfg.get("macro", []) or []
            support_fp16 = cfg.get("useFP16", False)
        else:
            macro_list = cfg if isinstance(cfg, list) else []
            support_fp16 = False

        variants = [(None, 'fp32')]
        for macro in macro_list:
            variants.append((macro, 'fp32'))

        if support_fp16:
            variants.append((None, 'fp16'))
            for macro in macro_list:
                variants.append((macro, 'fp16'))

        for macro, precision in variants:
            generated = _build_generated_shader_path(fileName, macro, precision)
            obj = ShaderFile(shader=generated, ref=True, raw=fileName, precision=precision)
            if macro:
                obj.setMacro(macro)
    ...
    for obj in objs:
        if obj.isRefFile():
            ...
            if len(raw) > 0 and len(dst) > 0:
                header = FP16_HEADER if obj.getPrecision() == 'fp16' else FP32_HEADER
                macro_line = ''
                if len(obj.getMacro()) > 0:
                    macro_line = "#define " + obj.getMacro() + "\n"
                with open(raw) as f:
                    body = f.read()
    ...
    for obj in objs:
        if obj.isRefFile():
            ...
            if os.path.exists(s) and os.path.isfile(s):
                os.remove(s)
                removed = True
    if removed and os.path.isdir(gGeneratedShaderDir) and not os.listdir(gGeneratedShaderDir):
        os.rmdir(gGeneratedShaderDir)
