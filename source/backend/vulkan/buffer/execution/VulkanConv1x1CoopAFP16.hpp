...
public:
    VulkanConv1x1Coop(VulkanBackend* backend, const Convolution2DCommon* convOption, const float* weightPtr,
                      const float* biasPtr, int ci, int co, VulkanDevice::CoopMatInfo coopMatInfo,
                      std::shared_ptr<ConvolutionCommon::Int8Common> quantInfo = nullptr);
...
private:
    VulkanConv1x1Coop(VulkanBackend* backend, const Convolution2DCommon* convOption, int ci, int co, uint32_t coopM,
    ...
    std::shared_ptr<VulkanBuffer>
        mWeightBuffer; // float/half weight (float path) or dequantized coop weight (quant path)
    std::shared_ptr<VulkanBuffer> mBiasBuffer;
    // Quant resources (static)
    std::shared_ptr<VulkanBuffer> mQuantWeightBuffer; // int8/int4 packed in [padN, padK]
    std::shared_ptr<VulkanBuffer> mQuantMetaBuffer;   // [scale, offset] per (N, K-block)
