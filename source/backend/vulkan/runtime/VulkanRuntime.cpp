...
VulkanRuntime::VulkanRuntime(const Backend::Info& info, std::shared_ptr<VulkanDevice> device,
                             std::shared_ptr<VulkanInstance> instance) {
    ...
    static std::map<std::string, float> gFlopsMap{
        {"Mali-T860", 6.83f},        {"Mali-T880", 6.83f},        {"Mali-G51", 6.83f},
        {"Mali-G52", 6.83f},         {"Mali-G71", 31.61f},        {"Mali-G72", 31.61f},
        {"Mali-G76", 31.61f},        {"Adreno (TM) 505", 3.19f},  {"Adreno (TM) 506", 4.74f},
        {"Adreno (TM) 512", 14.23f}, {"Adreno (TM) 530", 25.40f}, {"Adreno (TM) 540", 42.74f},
        {"Adreno (TM) 615", 16.77f}, {"Adreno (TM) 616", 18.77f}, {"Adreno (TM) 618", 18.77f},
        {"Adreno (TM) 630", 42.74f}, {"Adreno (TM) 640", 42.74f},
    };
    ...
    std::vector<int> legalModeValues = {0x00000001, 0x00000002, 0x00000004, 0x00000201, 0x00000202, 0x00000204};
    ...
    mGpuMode |= MNNGpuMode::MNN_GPU_RECORD_BATCH;
}
...
std::shared_ptr<VulkanBuffer> VulkanRuntime::allocUniform(const void* src, int size) {
    ...
    if (mUniformCache.empty() || allocSize > mUniformSize) {
        res = std::shared_ptr<VulkanBuffer>(
            new VulkanBuffer(*mMemoryPool, false, allocSize, nullptr, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT));
    } else {
...
Backend* VulkanRuntime::onCreate(const BackendConfig* config, Backend* origin) const {
    if (nullptr != config) {
        ...
        MNN_ASSERT(config->memory == mMemory);
    }
...
std::pair<const void*, size_t> VulkanRuntime::onGetCache() {
    ...
    for (const auto& kvPair : mTuneMap) {
        ...
        optimalLwsTemp->x = v.optimalLws[0];
        optimalLwsTemp->y = v.optimalLws[1];
        optimalLwsTemp->z = v.optimalLws[2];
