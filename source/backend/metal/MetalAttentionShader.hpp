...
auto tA = tensor<threadgroup ftype, dextents<int32_t, 2>, tensor_inline>((threadgroup ftype*)sdata, dextents<int32_t, 2>(K, M));
auto tB = tensor<threadgroup ftype, dextents<int32_t, 2>, tensor_inline>((threadgroup ftype*)sdata + tb_offset, dextents<int32_t, 2>(K, N));
...
auto tA = tensor<threadgroup ftype, dextents<int32_t, 2>, tensor_inline>((threadgroup ftype*)sdata, dextents<int32_t, 2>(K, M));
auto tB = tensor<threadgroup ftype, dextents<int32_t, 2>, tensor_inline>((threadgroup ftype*)sdata + 128, dextents<int32_t, 2>(N, K));
mpp::tensor_ops::matmul2d<
...
int k_bound_scalar = ((tile_max_valid + 31) / 32) * 32;
if (k_bound_scalar > align_value_len) k_bound_scalar = align_value_len;
av_k_upper_v4 = k_bound_scalar / 4;
}
#endif
...
int k_bound = tile_max_q_abs + 1;
if (k_bound < 0) k_bound = 0;
int k_bound_aligned = ((k_bound + 7) / 8) * 8;
...
#endif
#define SPLITKV_C 32
#define DPT (HEAD_DIM / SIMD_GROUP_WIDTH)
#define GS_LOCAL 1
...
const int q_head_base = int(gid.y) % param.head_num;
...
const int lp = int(sgitg) + rep * SPLITKV_NSG;
