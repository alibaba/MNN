...
static inline void storeC4(device ftype* tensor, int token, int base, int offset, int seqLen, ftype value) {
    if (seqLen == 1) {
        tensor[base + offset] = value;
        return;
    }
    tensor[c4Offset(token, base + offset, seqLen)] = value;
}
...
kernel void rope_kernel(
    ...
    int yBase = actual_z * p.D;
    if (actual_z >= p.numHead) {
        xTensor = k;
        xBase = (actual_z - p.numHead) * p.D;
        yTensor = ko;
        yBase = (actual_z - p.numHead) * p.D;
        isQ = false;
    }
    ...
    for (int i = start; i < p.ropeHalfD; i += step) {
        ...
        storeC4(yTensor, gid.y, yBase, i, xSeq, evenVal * cEven - oddVal * sEven);
        storeC4(yTensor, gid.y, yBase, i + p.ropeHalfD, xSeq, oddVal * cOdd + evenVal * sOdd);
    }
...
#endif
        storeC4(yTensor, gid.y, yBase, i, xSeq, value);
    }
}
#else
kernel void rope_kernel(
    ...
    if (gid.x < (uint)p.ropeHalfD) {
        ...
        storeC4(yTensor, gid.y, yBase, gid.x, xSeq, q0);
        storeC4(yTensor, gid.y, yBase, gid.x + p.ropeHalfD, xSeq, q1);
    }
    int tail = 2 * p.ropeHalfD + gid.x;
    if (tail < p.D) {
        storeC4(yTensor, gid.y, yBase, tail, xSeq, loadC4(xTensor, gid.y, xBase, tail, xSeq));
    }
}
#endif
)metal";
