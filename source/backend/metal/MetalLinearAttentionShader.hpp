...
const int use_l2norm = param.use_l2norm;
const float q_scale = param.q_scale;
const int j_blocks = d_v / SIMDS_PER_TG;
...
const int h = bh % H;
const int k_head = h / gqa_factor;
const int j = j_block * SIMDS_PER_TG + sgitg;
...
threadgroup float sh_v[CHUNK_BT][SIMDS_PER_TG];
...
threadgroup float sh_q [CHUNK_BT][HEAD_K_DIM];
threadgroup float sh_k [CHUNK_BT][HEAD_K_DIM];
threadgroup float sh_state[HEAD_K_DIM][DV_BLOCK];
threadgroup float sh_attn[CHUNK_BT][CHUNK_BT];
threadgroup float sh_qkdm[CHUNK_BT][CHUNK_BT];
threadgroup float sh_v [CHUNK_BT][DV_BLOCK];
threadgroup float sh_K_S [CHUNK_BT][DV_BLOCK];
threadgroup float sh_out [CHUNK_BT][DV_BLOCK];
threadgroup float sh_g [CHUNK_BT];
threadgroup float sh_beta[CHUNK_BT];
threadgroup float sh_G [CHUNK_BT];
...
threadgroup_barrier(mem_flags::mem_threadgroup);
{
const int kk_tiles = (CHUNK_BT / 8) * (CHUNK_BT / 8);
...
threadgroup_barrier(mem_flags::mem_threadgroup);
{
const int ks_tiles = (CHUNK_BT / 8) * (DV_BLOCK / 8);
...
threadgroup_barrier(mem_flags::mem_threadgroup);
{
const int qs_tiles = (CHUNK_BT / 8) * (DV_BLOCK / 8);
