...
public:
    MetalSharedGather(Backend* backend, int oc, std::shared_ptr<Tensor> weight,
                      std::shared_ptr<Tensor> dequantScaleBias, int dequantBits, float scaleCoef);
...
private:
    ...
    std::shared_ptr<Tensor> mDequantScaleBias; // scale + bias
