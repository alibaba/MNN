...
auto xy_wt = wt + ((idx_n4 * cst.input_slice + 0*2+kr) * 4 + nl) * 2 + kl;
auto xy_in0 = in + ((0*2+kr) * cst.input_size * cst.batch + idx_m) * 2 + kl;
auto xy_out = out + ((uz * 2 + kr) * cst.output_size * cst.batch + rx * 8 + ml) * 2 + kl;
...
auto xy_wt = wt + ((idx_n4 * cst.input_slice + 0*2+kl) * 4 + rcl % 4);
auto xy_in0 = in + ((0*2+rcl/8) * cst.input_size * cst.batch + idx_m) * 2 + kl;
auto xy_out = out + (4 * uz + rcl / 4) * cst.output_size * cst.batch + (rx * 8 + (rcl%4) * 2 + kl);
int block = (cst.input_slice + cst.block_size - 1) / cst.block_size;
for (int bi=0; bi<cst.block_size; ++bi) {
...
int idx_m = (rx * 8 + ml) < cst.input_size * cst.batch ? (rx * 8 + ml) : (cst.input_size * cst.batch - 1);
auto xy_wt = wt + ((idx_n4 * cst.input_slice + 0*2+kr) * 4 + kl * 2);
auto xy_in0 = in + ((0*2+kr) * cst.input_size * cst.batch + idx_m) * 2 + kl;
auto xy_out = out + (uz * 8 + ml) * cst.output_size * cst.batch + (rx * 8 + (kr*2+kl) * 2);
int block = (cst.input_slice + cst.block_size - 1) / cst.block_size;
for (int bi=0; bi<cst.block_size; ++bi) {
FLOAT4 scale = FLOAT4(dequantScale[2 * (idx_n4 * cst.block_size + bi) + 0]) / (FLOAT)cst.scale_coef;
FLOAT4 dequant_bias = FLOAT4(dequantScale[2 * (idx_n4 * cst.block_size + bi) + 1]) / (FLOAT)cst.scale_coef;
...
{
#ifdef W_QUANT_4
uchar2 w_int40 = xy_wt[4 * z + 0];
FLOAT4 w40 = FLOAT4((float)(w_int40[0] >> 4) - 8, (float)(w_int40[0] & 15) - 8, (float)(w_int40[1] >> 4) - 8, (float)(w_int40[1] & 15) - 8);
#elif defined(W_QUANT_8)
char4 w_int40 = xy_wt[4 * z + 0];
FLOAT4 w40 = FLOAT4((float)w_int40[0], (float)w_int40[1], (float)w_int40[2], (float)w_int40[3]);
#endif
...
{
#ifdef W_QUANT_4
uchar2 w_int40 = xy_wt[4 * z + 1];
FLOAT4 w40 = FLOAT4((float)(w_int40[0] >> 4) - 8, (float)(w_int40[0] & 15) - 8, (float)(w_int40[1] >> 4) - 8, (float)(w_int40[1] & 15) - 8);
#elif defined(W_QUANT_8)
char4 w_int40 = xy_wt[4 * z + 1];
FLOAT4 w40 = FLOAT4((float)w_int40[0], (float)w_int40[1], (float)w_int40[2], (float)w_int40[3]);
#endif
...
auto xy_wt = wt + (idx_n4 * cst.input_slice + 0) * 4 + rcl % 4;
auto xy_in0 = in + idx_m + cst.input_size * cst.batch * kl;
auto xy_out = out + (4 * uz + 2 * kl) * cst.output_size * cst.batch + idx_m;
int block = (cst.input_slice + cst.block_size - 1) / cst.block_size;
for (int bi=0; bi<cst.block_size; ++bi) {
FLOAT4 scale = FLOAT4(dequantScale[2 * (idx_n4 * cst.block_size + bi) + 0]) / (FLOAT)cst.scale_coef;
FLOAT4 dequant_bias = FLOAT4(dequantScale[2 * (idx_n4 * cst.block_size + bi) + 1]) / (FLOAT)cst.scale_coef;
...
for(int i = 0; i < 4; i++) {
#ifdef W_QUANT_4
uchar2 w_int40 = xy_wt[4 * (z + 2*i)];
FLOAT4 w40 = FLOAT4((float)(w_int40[0] >> 4) - 8, (float)(w_int40[0] & 15) - 8, (float)(w_int40[1] >> 4) - 8, (float)(w_int40[1] & 15) - 8);
#elif defined(W_QUANT_8)
char4 w_int40 = xy_wt[4 * (z + 2*i)];
FLOAT4 w40 = FLOAT4((float)w_int40[0], (float)w_int40[1], (float)w_int40[2], (float)w_int40[3]);
#endif
FLOAT4 res = w40 * scale[rcl % 4] + dequant_bias[rcl % 4];
...
auto xy_wt = wt + (idx_n4 * cst.input_slice + 0) * 4 + rcl % 4;
auto xy_in0 = in + idx_m0 + cst.input_size * cst.batch * kl;
auto xy_in1 = in + idx_m1 + cst.input_size * cst.batch * kl;
auto xy_out0 = out + (4 * uz + 2 * kl) * cst.output_size * cst.batch + idx_m0;
auto xy_out1 = out + (4 * uz + 2 * kl) * cst.output_size * cst.batch + idx_m1;
int block = (cst.input_slice + cst.block_size - 1) / cst.block_size;
for (int bi=0; bi<cst.block_size; ++bi) {
FLOAT4 scale = FLOAT4(dequantScale[2 * (idx_n4 * cst.block_size + bi) + 0]) / (FLOAT)cst.scale_coef;
FLOAT4 dequant_bias = FLOAT4(dequantScale[2 * (idx_n4 * cst.block_size + bi) + 1]) / (FLOAT)cst.scale_coef;
...
int idx_m = (16 * rx + rcl) < cst.input_size * cst.batch ? (16 * rx + rcl) : (cst.input_size * cst.batch - 1);
auto xy_wt0 = wt + (idx_n40 * cst.input_slice + 0) * 4 + (rcl % 4);
auto xy_wt1 = wt + (idx_n41 * cst.input_slice + 0) * 4 + (rcl % 4);
auto xy_in0 = in + idx_m + cst.input_size * cst.batch * kl;
auto xy_out = out + (4 * uz + 2 * kl) * cst.output_size * cst.batch + idx_m;
int block = (cst.input_slice + cst.block_size - 1) / cst.block_size;
for (int bi=0; bi<cst.block_size; ++bi) {
...
{
#ifdef W_QUANT_4
uchar2 w_int4 = xy_wt0[4*z];
FLOAT4 w4 = FLOAT4((float)(w_int4[0] >> 4) - 8, (float)(w_int4[0] & 15) - 8, (float)(w_int4[1] >> 4) - 8, (float)(w_int4[1] & 15) - 8);
#elif defined(W_QUANT_8)
char4 w_int4 = xy_wt0[4*z];
FLOAT4 w4 = FLOAT4((float)w_int4[0], (float)w_int4[1], (float)w_int4[2], (float)w_int4[3]);
#endif
...
{
#ifdef W_QUANT_4
uchar2 w_int4 = xy_wt1[4*z];
FLOAT4 w4 = FLOAT4((float)(w_int4[0] >> 4) - 8, (float)(w_int4[0] & 15) - 8, (float)(w_int4[1] >> 4) - 8, (float)(w_int4[1] & 15) - 8);
#elif defined(W_QUANT_8)
char4 w_int4 = xy_wt1[4*z];
FLOAT4 w4 = FLOAT4((float)w_int4[0], (float)w_int4[1], (float)w_int4[2], (float)w_int4[3]);
#endif
...
auto xy_in0 = in + idx_k4 * cst.input_size * cst.batch + idx_m20;
auto xy_in1 = in + idx_k4 * cst.input_size * cst.batch + idx_m21;
int idx_wk4 = 0 * 8 + kwl * 4 + 0;
int idx_n4 = (uz * 16 + no) < cst.output_slice ? (uz * 16 + no) : (cst.output_slice - 1);
auto xy_wt = wt + (idx_n4 * cst.input_slice + idx_wk4) * 4 + nl;
...
#pragma unroll(16)
for (int i = 0; i < 16; ++i) {
((threadgroup ftype*)sdata)[idx_sb + 64*i] = ftype(w_dequant[i/4][i%4]);
}
...
threadgroup_barrier(mem_flags::mem_threadgroup);
auto xy_out = out + ((uz * 4 + ko) * 4 + 0) * cst.output_size * cst.batch + (rx * 2 + kl) * 16 + ml;
...
auto xy_wt0 = wt + ((idx_n40 * cst.input_slice / 2 + 0) * 2 + kl) * 4 + ni;
auto xy_in0 = in + ((0 * 2 + ko) * cst.input_size * cst.batch + idx_m) * 2 + kl;
auto xy_out = out + ((4 * uz + 2 * ko + kl) * 4 + 0) * cst.output_size * cst.batch + idx_m;
...
#else
const device ftype4 *wi [[buffer(0)]],
#endif
device ftype4 *wf [[buffer(1)]],
constant conv1x1_constants& cst [[buffer(2)]],
...
#endif
auto xy_wf = wf + ((idx_n4 * ((cst.input_slice+3)/4) + idx_k16) * 4 + idx_nl) * 4;
#ifdef W_QUANT_2
...
auto tW = tensor<threadgroup ftype, dextents<int32_t, 2>, tensor_inline>((threadgroup ftype*)sdata + 2048, dextents<int32_t, 2>(K, N));
...
int idx_k4 = 0 * 16 + kl;
auto xy_in0 = in + idx_k4 * cst.input_size * cst.batch + idx_m40;
auto xy_in1 = in + idx_k4 * cst.input_size * cst.batch + idx_m41;
auto xy_in2 = in + idx_k4 * cst.input_size * cst.batch + idx_m42;
auto xy_in3 = in + idx_k4 * cst.input_size * cst.batch + idx_m43;
int idx_wk16 = (0 * 2 + kwl) * 2 + 0;
int idx_n4 = (uz * 16 + no) < cst.output_slice ? (uz * 16 + no) : (cst.output_slice - 1);
auto xy_wt = wt + (idx_n4 * ((cst.input_slice+3)/4) + idx_wk16) * 4 + nl;
int idx_sa = (ml * 4 + 0) * 16 + kl;
int idx_sb = 512 + ((no * 4 + nl) * 2 + kwl) * 8 + 0;
int block = (cst.input_slice + cst.block_size - 1) / cst.block_size;
for (int bi=0; bi<cst.block_size; ++bi) {
int zmin = bi * block;
int zmax = min(zmin + block, cst.input_slice);
for (int z = zmin; z < zmax; z += 16) {
FLOAT4x4 w_dequant_0;
{
auto w = xy_wt[z];
w_dequant_0 = FLOAT4x4((FLOAT4)w[0], (FLOAT4)w[1], (FLOAT4)w[2], (FLOAT4)w[3]);
}
FLOAT4x4 w_dequant_1;
{
auto w = xy_wt[z + 4];
...
#pragma unroll(4)
for (int i = 0; i < 4; ++i) {
((threadgroup ftype4*)sdata)[idx_sb + i] = ftype4(w_dequant_0[i]);
}
#pragma unroll(4)
for (int i = 0; i < 4; ++i) {
((threadgroup ftype4*)sdata)[idx_sb + 4 + i] = ftype4(w_dequant_1[i]);
}
((threadgroup ftype4*)sdata)[idx_sa] = (ftype4)*(xy_in0);
...
int idx_sb = 256 + ((no * 4 + nl) * 2 + kwl) * 4 + 0;
int block = (cst.input_slice + cst.block_size - 1) / cst.block_size;
for (int bi=0; bi<cst.block_size; ++bi) {
#if defined(W_QUANT_4) || defined(W_QUANT_8)
FLOAT scale0 = FLOAT(dequantScale[((idx_n4 * cst.block_size + bi) * 2 + 0) * 4 + nl]) / (FLOAT)cst.scale_coef;
...
int idx_k4 = ko * 2 + kl;
auto xy_in0 = in + idx_k4 * mBound + idx_m40;
auto xy_in1 = in + idx_k4 * mBound + idx_m41;
auto xy_in2 = in + idx_k4 * mBound + idx_m42;
auto xy_in3 = in + idx_k4 * mBound + idx_m43;
int idx_n4 = (uz * 16 + no) < cst.output_slice ? (uz * 16 + no) : (cst.output_slice - 1);
auto xy_wt = wt + (idx_n4 * ((cst.input_slice + 3) / 4) + kwl) * 4 + nl;
int idx_sa = (ko * 64 + ml * 4 + 0) * 2 + kl;
int idx_sb = 2048 + (kwl * 16 + 0) * 64 + no * 4 + nl;
...
FLOAT s0 = in40[1];
FLOAT s1 = in40[0] * FLOAT(0.0625);
FLOAT s2 = in40[3] * FLOAT(0.00390625);
FLOAT s3 = in40[2] * FLOAT(0.000244140625);
...
FLOAT4x4 w_fp32 = FLOAT4x4(FLOAT4(w[0]), FLOAT4(w[1]), FLOAT4(w[2]), FLOAT4(w[3]));
FLOAT4x4 w_dq;
FLOAT4 s = scale[oc] + adjusted_bias[oc] + FLOAT(8.0) * scale[oc];
