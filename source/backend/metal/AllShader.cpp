...
"}\n"
;
const char* shader_MetalROIAlign_metal = 
...
"};\n"
"static inline M4 bilinear_interpolate(const device M4 *data,\n"
...
" M4 val=bilinear_interpolate(z_in,s.input_height,s.input_width,py,px);\n"
" sum += float4(val);\n"
" }\n"
" }\n"
" sum /= (float)count;\n"
...
" out[out_idx]=M4(sum);\n"
"}\n"
"kernel void roi_align_max(const device M4 *in [[buffer(0)]],\n"
" const device M *roi [[buffer(1)]],\n"
" device M4 *out [[buffer(2)]],\n"
" constant roi_align_shape &s [[buffer(3)]],\n"
" const device int *batch_idx_buf [[buffer(4)]],\n"
...
" if(channelSize>1) {\n"
" /* true */ {xy_out[cst.outputSize*cst.batch +0]=activate(M4(result4),cst.activation); }\n"
" if (widthSize>1) {xy_out[cst.outputSize*cst.batch +1]=activate(M4(result5),cst.activation); }\n"
" }\n"
"}\n"
...
" int channelSize=min(cst.outputDepthQuad-idx_c,2);\n"
" if(channelSize>1) {\n"
" /* true */ xy_out[cst.outputSize*cst.batch]=activate(M4(result2),cst.activation);\n"
" if (widthSize>1) {xy_out[cst.outputSize*cst.batch +1]=activate(M4(result3),cst.activation); }\n"
" if (widthSize>2) {xy_out[cst.outputSize*cst.batch +2]=activate(M4(result6),cst.activation); }\n"
" if (widthSize>3) {xy_out[cst.outputSize*cst.batch +3]=activate(M4(result7),cst.activation); }\n"
" }\n"
"}\n"
;
