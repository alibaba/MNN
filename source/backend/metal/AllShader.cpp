...
" }\n"
"}\n"
"kernel void layernorm_x16_rms_sg(const device M4 *in [[buffer(0)]],\n"
" device M4 *out [[buffer(1)]],\n"
" constant layernorm_constants& cst [[buffer(2)]],\n"
" const device float4 *gamma [[buffer(3)]],\n"
" const device float4 *beta [[buffer(4)]],\n"
...
" if(cst.has_gamma_beta) {\n"
" out_data[in_idx+1]=(M4)(norm*gamma[in_idx+1]+beta[in_idx+1]);\n"
" } else {\n"
" out_data[in_idx+1]=(M4)(norm);\n"
" }\n"
" }\n"
" if(tiisg == 2 && in_idx+2<cst.inside/4) {\n"
" float4 norm=var*((float4)in_data[in_idx+2]);\n"
" if(cst.has_gamma_beta) {\n"
" out_data[in_idx+2]=(M4)(norm*gamma[in_idx+2]+beta[in_idx+2]);\n"
" } else {\n"
" out_data[in_idx+2]=(M4)(norm);\n"
" }\n"
" }\n"
" if(tiisg == 3 && in_idx+3<cst.inside/4) {\n"
" float4 norm=var*((float4)in_data[in_idx+3]);\n"
" if(cst.has_gamma_beta) {\n"
" out_data[in_idx+3]=(M4)(norm*gamma[in_idx+3]+beta[in_idx+3]);\n"
" } else {\n"
" out_data[in_idx+3]=(M4)(norm);\n"
" }\n"
" }\n"
"}\n"
;
...
" if (computeSize>3) {xy_out[3]=activate(M4(result3),cst.activation); }\n"
"}\n"
"kernel void conv1x1_gemm_16x16_w4(const device M4 *in [[buffer(0)]],\n"
" device M4 *out [[buffer(1)]],\n"
" constant conv1x1_constants& cst [[buffer(2)]],\n"
...
" xy_out[0]=activate(M4(sdata[(kl*16+rcl)*2+0]+FLOAT4(biasTerms[4*uz+2*kl+0])),cst.activation);\n"
" }\n"
" if((4*uz+2*kl+1)<cst.output_slice) {\n"
" xy_out[cst.output_size*cst.batch]=activate(M4(sdata[(kl*16+rcl)*2+1]+FLOAT4(biasTerms[4*uz+2*kl+1])),cst.activation);\n"
" }\n"
" }\n"
"}\n"
"kernel void conv1x1_gemm_32x16_w4(const device M4 *in [[buffer(0)]],\n"
" device M4 *out [[buffer(1)]],\n"
" constant conv1x1_constants& cst [[buffer(2)]],\n"
" const device uchar2 *wt [[buffer(3)]],\n"
" const device M4 *biasTerms [[buffer(4)]],\n"
...
" if((16*rx+rcl)<size_m) {\n"
" if((4*uz+2*kl)<cst.output_slice) {\n"
" xy_out0[0]=activate(M4(sdata[(kl*32+rcl)*2+0]+FLOAT4(biasTerms[4*uz+2*kl+0])),cst.activation);\n"
" }\n"
" if((4*uz+2*kl+1)<cst.output_slice) {\n"
" xy_out0[cst.output_size*cst.batch]=activate(M4(sdata[(kl*32+rcl)*2+1]+FLOAT4(biasTerms[4*uz+2*kl+1])),cst.activation);\n"
" }\n"
" }\n"
" if((16*rx+rcl)+size_m/2<size_m) {\n"
" if((4*uz+2*kl)<cst.output_slice) {\n"
" xy_out1[0]=activate(M4(sdata[(kl*32+16+rcl)*2+0]+FLOAT4(biasTerms[4*uz+2*kl+0])),cst.activation);\n"
" }\n"
" if((4*uz+2*kl+1)<cst.output_slice) {\n"
" xy_out1[cst.output_size*cst.batch]=activate(M4(sdata[(kl*32+16+rcl)*2+1]+FLOAT4(biasTerms[4*uz+2*kl+1])),cst.activation);\n"
" }\n"
" }\n"
"}\n"
"kernel void conv1x1_gemm_16x32_w4(const device M4 *in [[buffer(0)]],\n"
" device M4 *out [[buffer(1)]],\n"
" constant conv1x1_constants& cst [[buffer(2)]],\n"
" const device uchar2 *wt [[buffer(3)]],\n"
" const device M4 *biasTerms [[buffer(4)]],\n"
...
" if((16*rx+rcl)<cst.input_size*cst.batch) {\n"
" if(4*uz+2*kl<cst.output_slice) {\n"
" xy_out[0]=activate(M4(sdata[(kl*16+rcl)*2+0]+FLOAT4(biasTerms[4*uz+2*kl])),cst.activation);\n"
" }\n"
" if(4*uz+2*kl+1<cst.output_slice) {\n"
" xy_out[cst.output_size*cst.batch]=activate(M4(sdata[(kl*16+rcl)*2+1]+FLOAT4(biasTerms[4*uz+2*kl+1])),cst.activation);\n"
" }\n"
" if(cst.output_slice/2+4*uz+2*kl<cst.output_slice) {\n"
" xy_out[cst.output_slice/2*cst.output_size*cst.batch]=activate(M4(sdata[((kl+2)*16+rcl)*2+0]+FLOAT4(biasTerms[cst.output_slice/2+4*uz+2*kl])),cst.activation);\n"
" }\n"
" if(cst.output_slice/2+4*uz+2*kl+1<cst.output_slice) {\n"
" xy_out[(cst.output_slice/2+1)*cst.output_size*cst.batch]=activate(M4(sdata[((kl+2)*16+rcl)*2+1]+FLOAT4(biasTerms[cst.output_slice/2+4*uz+2*kl+1])),cst.activation);\n"
" }\n"
" }\n"
"}\n"
"kernel void conv1x1_gemm_32x64_w4(const device M2 *in [[buffer(0)]],\n"
" device M4 *out [[buffer(1)]],\n"
" constant conv1x1_constants& cst [[buffer(2)]],\n"
" const device uchar2 *wt [[buffer(3)]],\n"
" const device M4 *biasTerms [[buffer(4)]],\n"
...
" xy_out[0]=activate(M4(sdata[(((ml/16*4+2*ko+kl)*2+0)*16+ml % 16)*2]+FLOAT4(biasTerms[(4*uz+2*ko+kl)*4])),cst.activation);\n"
" }\n"
" if((4*uz+2*ko+kl)*4+1<cst.output_slice) {\n"
" xy_out[cst.output_size*cst.batch]=activate(M4(sdata[(((ml/16*4+2*ko+kl)*2+0)*16+ml % 16)*2+1]+FLOAT4(biasTerms[(4*uz+2*ko+kl)*4+1])),cst.activation);\n"
" }\n"
" if((4*uz+2*ko+kl)*4+2<cst.output_slice) {\n"
" xy_out[cst.output_size*cst.batch*2]=activate(M4(sdata[(((ml/16*4+2*ko+kl)*2+1)*16+ml % 16)*2]+FLOAT4(biasTerms[(4*uz+2*ko+kl)*4+2])),cst.activation);\n"
" }\n"
" if((4*uz+2*ko+kl)*4+3<cst.output_slice) {\n"
" xy_out[cst.output_size*cst.batch*3]=activate(M4(sdata[(((ml/16*4+2*ko+kl)*2+1)*16+ml % 16)*2+1]+FLOAT4(biasTerms[(4*uz+2*ko+kl)*4+3])),cst.activation);\n"
" }\n"
" }\n"
"}\n"
...
" }\n"
"}\n"
"kernel void conv1x1_gemm_16x16(const device M4 *in [[buffer(0)]],\n"
" device M4 *out [[buffer(1)]],\n"
" constant conv1x1_constants& cst [[buffer(2)]],\n"
" const device M4 *wt [[buffer(3)]],\n"
" const device M4 *biasTerms [[buffer(4)]],\n"
...
" }\n"
"}\n"
"kernel void conv1x1_gemm_32x16(const device M4 *in [[buffer(0)]],\n"
" device M4 *out [[buffer(1)]],\n"
" constant conv1x1_constants& cst [[buffer(2)]],\n"
" const device M4 *wt [[buffer(3)]],\n"
" const device M4 *biasTerms [[buffer(4)]],\n"
