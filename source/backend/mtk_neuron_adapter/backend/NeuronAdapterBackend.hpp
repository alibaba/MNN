...
#ifndef MNN_NeuronAdapterBACKEND_H
#define MNN_NeuronAdapterBACKEND_H
...
namespace MNN {
    ...
    class NeuronAdapterRuntime : public Runtime {
    public:
        NeuronAdapterRuntime(const Backend::Info& info);
        virtual ~NeuronAdapterRuntime();
    ...
    private:
        ...
        friend class NeuronAdapterBackend;
    };

    class NeuronAdapterBackend : public Backend {
    public:

        NeuronAdapterBackend(const NeuronAdapterRuntime* runtime);
        virtual ~NeuronAdapterBackend();
    ...
    public:
        ...
        uint32_t buildOperand(const void* data, size_t size, int code, std::vector<uint32_t> dims = {}, const float* scales = nullptr, int zero = 0);
        ErrorCode buildOperation(NeuronOperationType op, const std::vector<uint32_t> &inputs, const std::vector<uint32_t> &outputs, const char* name = nullptr);
    ...
    private:
        ...
        const NeuronAdapterRuntime* mNPURuntime;
        BackendConfig::PrecisionMode mPrecision;
        ...
        std::vector<uint32_t> mOutputTensorIndexes;
        ...
        // NeuronAdapter resource
        struct NeuronAdapterDevice {
            NeuronDevice* device;
            const char* name;
        };
        std::vector<NeuronAdapterDevice> mNeuronAdapterDevices;
        NeuronModel *mNeuronAdapterModel = nullptr;
        NeuronCompilation *mNeuronAdapterCompilation = nullptr;
    };

    template <class T>
    class NeuronAdapterCreatorRegister {
    public:
        NeuronAdapterCreatorRegister(OpType type) {
            T *t = new T;
            NeuronAdapterBackend::addCreator(type, t);
        }
        ~NeuronAdapterCreatorRegister() = default;
    };

    template <typename T>
    class TypedCreator : public NeuronAdapterBackend::Creator {
    public:
...
#define REGISTER_NeuronAdapter_OP_CREATOR(name, opType)     \
    void ___##name##__##opType##__() {            \
        static TypedCreator<name> _temp;\
        NeuronAdapterBackend::addCreator(opType, &_temp); \
    }

}

#endif //MNN_NeuronAdapterBACKEND_H
