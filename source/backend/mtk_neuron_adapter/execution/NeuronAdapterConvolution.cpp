...
#include "NeuronAdapterConvolution.hpp"

namespace MNN {


NeuronAdapterConvolution::NeuronAdapterConvolution(MNN::Backend *b, const MNN::Op *op, const std::vector<Tensor *> &inputs, const std::vector<MNN::Tensor *> &outputs) : NeuronAdapterCommonExecution(b, op) {
...
ErrorCode NeuronAdapterConvolution::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    ...
    if (inputs.size() < 3) {
        ...
        if (isQuantInt8) {
            ...
            quanCommon = ConvolutionCommon::load(mOp, backend(), false, true);
            weightPtr = quanCommon->weight.get();
        ...
        if (isQuantInt8) {
            ...
            inputIdxs.push_back(buildConstant(weightPtr, weightSize, NEURON_TENSOR_QUANT8_SYMM_PER_CHANNEL, weightDims, alpha, 0));
            inputIdxs.push_back(buildConstant(biasPtr, biasSize * sizeof(int), NEURON_TENSOR_INT32, biasDims));
        } else {
            ...
            inputIdxs.push_back(buildConstant(nhwcWeight.get(), weightSize * sizeof(float), NEURON_TENSOR_FLOAT32, weightDims));
            inputIdxs.push_back(buildConstant(biasPtr, biasSize * sizeof(float), NEURON_TENSOR_FLOAT32, biasDims));
        }
    }
    // pad
    ...
    // fusecode
    NeuronAdapterFuseCode code = NEURON_FUSED_NONE;
    if (relu) code = NEURON_FUSED_RELU;
    if (relu6) code = NEURON_FUSED_RELU6;
    ...
    auto op = NEURON_CONV_2D;
    if (mOp->type() == OpType_ConvolutionDepthwise) {
        op = NEURON_DEPTHWISE_CONV_2D;
    } else if (mOp->type() == OpType_Deconvolution){
        op = NEURON_TRANSPOSE_CONV_2D;
    }
    auto tmpIdx = getTensorIdxs(outputs);
    auto errorcode = buildOperation(op, inputIdxs, tmpIdx);
    return errorcode;
}

REGISTER_NeuronAdapter_OP_CREATOR(NeuronAdapterConvolution, OpType_Convolution)
REGISTER_NeuronAdapter_OP_CREATOR(NeuronAdapterConvolution, OpType_ConvolutionDepthwise)
REGISTER_NeuronAdapter_OP_CREATOR(NeuronAdapterConvolution, OpType_Deconvolution)
} // namespace MNN
