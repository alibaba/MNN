...
#include "NeuronAdapterPool.hpp"

namespace MNN {


NeuronAdapterPool::NeuronAdapterPool(MNN::Backend *b, const MNN::Op *op, const std::vector<Tensor *> &inputs, const std::vector<MNN::Tensor *> &outputs) : NeuronAdapterCommonExecution(b, op) {
}

ErrorCode NeuronAdapterPool::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    ...
    // NeuronAdapter Pool inputs: [input, pad_left, pad_right, pad_top, pad_bottom, stride_w, stride_h, kernel_w, kernel_h, fusecode, NCHW/NHWC]
    ...
    // fusecode
    inputIdxs.push_back(buildScalar(NEURON_FUSED_NONE));
    // NCHW/NHWC
    inputIdxs.push_back(buildScalar(mNCHW));
    auto op = NEURON_MAX_POOL_2D;
    if (pool->type() == PoolType_AVEPOOL) {
        op = NEURON_AVERAGE_POOL_2D;
    }
    return buildOperation(op, inputIdxs, getTensorIdxs(outputs));
}

REGISTER_NeuronAdapter_OP_CREATOR(NeuronAdapterPool, OpType_Pooling)
} // namespace MNN
