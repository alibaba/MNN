...
HEXAGON_BUILD_ARGS=(hexagon "DSP_ARCH=$1" "HTP_OPS_PWL_VARIANT=${HTP_OPS_PWL_VARIANT:-learned8}")
...
if [[ -x "${HEXAGON_STRIP_BIN}" ]]; then
    "${HEXAGON_STRIP_BIN}" --strip-debug outputs/libMNN_htpops_skel.so
else
...
if ! ./check_so_symbols.sh outputs/libMNN_htpops_skel.so; then
