...
static inline int compute_store_tiles_w8a16_sym_per_channel(uint8_t* dst, const uint8_t* bias,
        ...
        for (int oy = oy_start; oy < oy_end; ++oy) {
            ...
            if (pack == 64 && oy + 1 < oy_end && ((oy * 32) & 63) == 0) {
                ...
                int storeRet = store_output_tile_pair_fp16_scaled(dst, vtcm_output, vtcm_output_next,
                                                                  nullptr,
                                                                  nullptr,
                                                                  vtcm_scales, nullptr, nullptr, M, ox, oy,
            ...
            int storeRet = store_output_tile_fp16_scaled(dst, vtcm_output, nullptr,
                                                         vtcm_scales, nullptr, M, ox, oy, pack,
                                                         params->relu, params->relu6,
                                                         params->outputBytes);
            if (storeRet != AEE_SUCCESS) {
                return storeRet;
        ...
        for (int ox_start = 0; ox_start < mp; ox_start += mp_chunk) {
            ...
            for (int oy_start = 0; oy_start < np; oy_start += np_chunk) {
                ...
                for (int chunk_idx = 0; chunk_idx < async.valid_chunks; ++chunk_idx) {
                    ...
                    int ret = compute_store_tiles_w8a16_sym_per_channel(dst, bias, params, M, pack, kp,
                                                                        ox_start, ox_end,
