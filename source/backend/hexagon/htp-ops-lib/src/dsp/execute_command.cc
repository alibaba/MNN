...
extern AEEResult htp_ops_pool2d_fp16(uint8_t* output, uint8_t* input,
        ...
        int32 kernelY, int32 kernelX, int32 strideY, int32 strideX,
        int32 padY, int32 padX, int32 padType, int32 countType, int32 poolType);

extern AEEResult htp_ops_conv_depthwise2d_fp16(uint8_t* output, uint8_t* input, uint8_t* weight, uint8_t* bias, int32 batch, int32 ih, int32 iw, int32 oh, int32 ow, int32 c4, int32 kernelY, int32 kernelX, int32 strideY, int32 strideX, int32 padY, int32 padX, int32 dilateY, int32 dilateX, int32 relu, int32 relu6);

extern AEEResult htp_ops_raster_blit(uint8_t* dst, uint8_t** src, int src_number, uint8_t* region, int32_t regionCount, int32_t bytes);
...
extern AEEResult htp_ops_loop_blit(uint8_t* dst, uint8_t* src0, uint8_t* src1, uint8_t* iter0, uint8_t* iter1, uint8_t* iter2, int32 cmdKind, int32 type, int32 bytes, uint8_t* param);
extern AEEResult htp_ops_batch_matmul(uint8_t* dst, uint8_t* src0, uint8_t* src1, uint8_t* iter0, uint8_t* iter1, uint8_t* iter2, int32 bytes, uint8_t* param);

extern AEEResult htp_ops_tensor_convert(uint8_t* dst, uint8_t* src, int32 batch, int32 area, int32 channel, int32 bytes, int32 type);

extern AEEResult htp_ops_layer_norm(uint8_t* dst, uint8_t* src, uint8_t* gamma, uint8_t* beta, int32 outter, int32 inner, float epsilon, int32 rmsNorm);

extern AEEResult htp_ops_layer_norm_packed(uint8_t* dst, uint8_t* src, uint8_t* gamma, uint8_t* beta, int32 batch, int32 channels, float epsilon, int32 rmsNorm);
...
extern AEEResult htp_ops_add_fuse_layernorm(uint8_t* dst, uint8_t* addOut, uint8_t* src0, uint8_t* src1, uint8_t* gamma, uint8_t* beta, int32 batch, int32 channels, float epsilon, int32 rmsNorm);

extern AEEResult htp_ops_flash_attn(uint8_t* o, uint8_t* q, uint8_t* k, uint8_t* v, uint8_t* mask, uint8_t* workspace, uint8_t* pastK, uint8_t* pastV, int32 qo_len, int32 seq_current, int32 seq_add, int32 n_heads, int32 n_kv_heads, int32 head_dim, float scale, int32 mask_stride, int32 max_kv_len, int32 value_c4);
extern AEEResult htp_ops_flash_attn_pages(uint8_t* o, uint8_t* q, uint8_t* k, uint8_t* v, uint8_t* mask, uint8_t* workspace,
                                          uint8_t** pastKPages, uint8_t** pastVPages,
                                          int32 qo_len, int32 seq_current, int32 seq_add, int32 n_heads,
                                          int32 n_kv_heads, int32 head_dim, float scale, int32 mask_stride,
                                          int32 max_kv_len, int32 page_count, int32 page_size, int32 value_c4);
extern AEEResult htp_ops_flash_attention_block(uint8_t* m_new, uint8_t* l_new, uint8_t* out_new,
                                                uint8_t* q, uint8_t* k, uint8_t* v, uint8_t* mask,
                                                uint8_t* running_m, uint8_t* running_l, uint8_t* running_out,
                                                uint8_t* workspace,
                                                int32 batch, int32 heads, int32 tokens, int32 chunk, int32 head_dim,
                                                float scale);
...
extern AEEResult htp_ops_binary_elementwise(uint8_t* dst, uint8_t* src0, uint8_t* src1,
                                            int32 outSize, int32 in0Size, int32 in1Size,
                                            int32 type, int32 bytes, int32 inputBytes,
...
extern AEEResult htp_ops_relu6(uint8_t* dst, const uint8_t* src, int32 size, int32 bytes, float minValue, float maxValue);
...
extern AEEResult htp_ops_prelu(uint8_t* dst, const uint8_t* src, const uint8_t* slope, int32 size, int32 bytes,
                               int32 plane, int32 channel, int32 slopeCount, int32 pack, int32 batch);
...
static void sync_group_tensors(MmapManager* mmap_manager, int32 syncGroupFd, int32 syncGroupOffset, int32 syncGroupSize, bool before_execute) {
...
int htp_execute_command(MmapManager* mmap_manager, const DSPCOMMAND::Command* command) {
    ...
    if (inputs->size() + outputs->size() > 64) {
        FARF(ERROR, "htp_execute_command: too many tensors inputs=%d outputs=%d",
    ...
    switch(opType) {
        ...
        case DSP_OP_TMAC_A16W1: {
            ...
            int output_bytes = intParams[5];
            ret = htp_ops_tmac_a16w1_fp16(
                                        mapped_ptrs[inputs->size()], // output
                                        mapped_ptrs[0], // activation (FP16)
                                        mapped_ptrs[1], // packed W1 weights
                                        mapped_ptrs[2], // FP32 block scales
                                        mapped_ptrs[3], // bias (FP16, optional)
                                        m, ic, oc, scale_block_num, scale_asymmetric, output_bytes);
            break;
        }
        case DSP_OP_POOL2D_FP16: {
            ...
            ret = htp_ops_pool2d_fp16(mapped_ptrs[inputs->size()],
                                       mapped_ptrs[0],
                                       batch, ih, iw, oh, ow, c4,
                                       kernelY, kernelX, strideY, strideX,
                                       padY, padX, padType, countType, poolType);
            break;
        }
        case DSP_OP_CONV_DEPTHWISE2D_FP16: {
            ret = htp_ops_conv_depthwise2d_fp16(mapped_ptrs[inputs->size()],
                                                 mapped_ptrs[0],
                                                 mapped_ptrs[1],
                                                 mapped_ptrs[2],
                                                 intParams[0], intParams[1], intParams[2], intParams[3], intParams[4], intParams[5],
                                                 intParams[6], intParams[7], intParams[8], intParams[9], intParams[10], intParams[11],
                                                 intParams[12], intParams[13], intParams[14], intParams[15]);
            break;
        }
        case DSP_OP_RASTER_BLIT: {
            ...
            ret = htp_ops_raster_blit(mapped_ptrs[src_number],
                                       src_ptrs,
                                       src_number,
                                       regionPtr,
                                       regionCount,
        ...
        case DSP_OP_CAST: {
            ret = htp_ops_cast(mapped_ptrs[inputs->size()],
                               mapped_ptrs[0],
        ...
        case DSP_OP_BINARY_BLIT: {
            ...
            uint8_t* regionPtr = (uint8_t*)&intParams[3];
            ret = htp_ops_binary_blit(mapped_ptrs[inputs->size()],
                                      mapped_ptrs[0],
                                      mapped_ptrs[1],
                                      regionPtr,
                                      regionCount,
        ...
        case DSP_OP_LOOP_BLIT: {
            ret = htp_ops_loop_blit(mapped_ptrs[inputs->size()],
                                     mapped_ptrs[0],
                                     mapped_ptrs[1],
                                     mapped_ptrs[2],
                                     mapped_ptrs[3],
        ...
        case DSP_OP_BATCH_MATMUL: {
            ret = htp_ops_batch_matmul(mapped_ptrs[inputs->size()],
                                       mapped_ptrs[0],
                                       mapped_ptrs[1],
                                       mapped_ptrs[2],
                                       mapped_ptrs[3],
        ...
        case DSP_OP_TENSOR_CONVERT: {
            ret = htp_ops_tensor_convert(mapped_ptrs[inputs->size()],
                                          mapped_ptrs[0],
                                          intParams[0], intParams[1], intParams[2], intParams[3], intParams[4]);
            break;
        }
        case DSP_OP_LAYER_NORM: {
            ret = htp_ops_layer_norm(mapped_ptrs[3],
                                      mapped_ptrs[0],
                                      mapped_ptrs[1],
                                      mapped_ptrs[2],
                                      intParams[0], intParams[1], floatParams[2], intParams[3]);
            break;
        }
        case DSP_OP_LAYER_NORM_PACKED: {
            ret = htp_ops_layer_norm_packed(mapped_ptrs[3],
                                             mapped_ptrs[0],
                                             mapped_ptrs[1],
                                             mapped_ptrs[2],
                                             intParams[0], intParams[1], floatParams[2], intParams[3]);
        ...
        case DSP_OP_SCALE: {
            ret = htp_ops_scale(mapped_ptrs[2],
                                 mapped_ptrs[0],
                                 mapped_ptrs[1],
                                 intParams[0], intParams[1],
    ...
    case DSP_OP_ROPE_FUSE_LAYERNORM:
      {
        ...
        case DSP_OP_ADD_FUSE_LAYERNORM: {
            ret = htp_ops_add_fuse_layernorm(mapped_ptrs[4],
                                              mapped_ptrs[5],
                                              mapped_ptrs[0],
                                              mapped_ptrs[1],
                                              mapped_ptrs[2],
        ...
        case DSP_OP_FLASH_ATTN: {
            ...
            if (page_count > 0 && page_table_capacity > 0) {
                if (inputs->size() <= 4 || mapped_ptrs[4] == NULL || page_count > page_table_capacity) {
                    FARF(ERROR, "DSP_OP_FLASH_ATTN page table mismatch: page_count=%d capacity=%d inputs=%d",
                         page_count, page_table_capacity, (int)inputs->size());
                    ret = AEE_EBADPARM;
                    break;
                }
                ...
                for (int i = 0; i < page_count; ++i) {
                    ...
                    if (kFd < 0 || vFd < 0 || kMap == NULL || vMap == NULL) {
                        FARF(ERROR, "DSP_OP_FLASH_ATTN page table map failed: page=%d kfd=%d vfd=%d",
                             i, kFd, vFd);
                        ret = AEE_EBADPARM;
                        break;
                    }
                ...
                if (page_count > 0) {
                    FARF(ERROR, "DSP_OP_FLASH_ATTN page inputs mismatch: page_count=%d inputs=%d",
                         page_count, (int)inputs->size());
                    ret = AEE_EBADPARM;
                    break;
                }
                uint8_t* pastKPtr = mapped_ptrs[4];
                uint8_t* pastVPtr = mapped_ptrs[5];
                ret = htp_ops_flash_attn(outPtr,
                                          qPtr, kPtr, vPtr,
                                          maskPtr,
                                          workspacePtr,
                                          pastKPtr, pastVPtr,
                                          intParams[0], seq_current, seq_add, intParams[3], intParams[4],
                                          intParams[5], floatParams[6], intParams[7], intParams[8], value_c4);
            }
            break;
        }
        case DSP_OP_FLASH_ATTENTION_BLOCK: {
            ret = htp_ops_flash_attention_block(mapped_ptrs[inputs->size()],
                                                  mapped_ptrs[inputs->size() + 1],
                                                  mapped_ptrs[inputs->size() + 2],
                                                  mapped_ptrs[0],
                                                  mapped_ptrs[1],
                                                  mapped_ptrs[2],
                                                  inputs->size() > 3 ? mapped_ptrs[3] : NULL,
                                                  mapped_ptrs[4],
                                                  mapped_ptrs[5],
                                                  mapped_ptrs[6],
                                                  outputs->size() > 3 ? mapped_ptrs[inputs->size() + 3] : NULL,
                                                  intParams[0], intParams[1], intParams[2],
                                                  intParams[3], intParams[4], floatParams[5]);
            break;
        }
        case DSP_OP_BINARY_ELEMENTWISE: {
            ret = htp_ops_binary_elementwise(mapped_ptrs[2],
                                              mapped_ptrs[0],
                                              mapped_ptrs[1],
                                              intParams[0], intParams[1], intParams[2], intParams[3], intParams[4],
                                              intParams[5], intParams[6], intParams[7],
                                              params && params->size() > 8 ? intParams + 8 : nullptr,
                                              params ? (int32_t)params->size() - 8 : 0);
            break;
        }
        case DSP_OP_SELECT: {
            ret = htp_ops_select(mapped_ptrs[3],
                                 mapped_ptrs[0],
                                 mapped_ptrs[1],
                                 mapped_ptrs[2],
                                 intParams[0], intParams[1], intParams[2], intParams[3], intParams[4], intParams[5],
                                 intParams[6], intParams[7]);
            break;
        }
        ...
        case DSP_OP_REDUCTION: {
            ret = htp_ops_reduction(mapped_ptrs[inputs->size()],
                                    mapped_ptrs[0],
                                    intParams[0], intParams[1], intParams[2], intParams[3], intParams[4]);
            break;
        }
        case DSP_OP_MASKED_REDUCTION: {
            ret = htp_ops_masked_reduction(mapped_ptrs[inputs->size()],
                                           mapped_ptrs[0],
                                           mapped_ptrs[1],
                                           intParams[0], intParams[1], intParams[2], intParams[3], intParams[4]);
            break;
        ...
        case DSP_OP_RELU6: {
            ret = htp_ops_relu6(mapped_ptrs[inputs->size()],
                                mapped_ptrs[0],
                                intParams[0], intParams[1], floatParams[2], floatParams[3]);
            break;
        }
        case DSP_OP_RELU: {
            ret = htp_ops_relu(mapped_ptrs[inputs->size()],
                               mapped_ptrs[0],
                               intParams[0], intParams[1], floatParams[2]);
            break;
        }
        case DSP_OP_PRELU: {
            ret = htp_ops_prelu(mapped_ptrs[inputs->size()],
                                mapped_ptrs[0],
                                mapped_ptrs[1],
                                intParams[0], intParams[1], intParams[2],
                                intParams[3], intParams[4], intParams[5],
                                params && params->size() > 6 ? intParams[6] : 1);
            break;
        }
        case DSP_OP_LSTM: {
            ...
            ret = htp_ops_lstm(mapped_ptrs[outputBase], yh, yc,
                               mapped_ptrs[0], mapped_ptrs[1], mapped_ptrs[2], mapped_ptrs[3],
                               mapped_ptrs[4], mapped_ptrs[5], packedW, packedR,
                               intParams[0], intParams[1], intParams[2], intParams[3],
        ...
        case DSP_OP_WEIGHT_REORDER: {
            ...
            ret = htp_ops_weight_reorder(mapped_ptrs[1],
                                          mapped_ptrs[0],
                                          reorderParams);
            break;
        }
        case DSP_OP_WEIGHT_REORDER_INT4: {
            ret = htp_ops_weight_reorder_int4(mapped_ptrs[1],
                                               mapped_ptrs[0],
                                               intParams[0], intParams[1], intParams[2]);
            break;
        }
...
AEEResult htp_ops_execute_command_group(remote_handle64 handle, int32 groupFd, int32 groupOffset, int32 count, int32 syncGroupFd, int32 syncGroupOffset, int32 syncGroupSize) {
    ...
    LocalVtcmGuard vtcmGuard(count);
    if (vtcmGuard.status() != AEE_SUCCESS) {
        return vtcmGuard.status();
    }
