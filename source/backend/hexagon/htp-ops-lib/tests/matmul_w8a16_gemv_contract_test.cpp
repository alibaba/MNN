...
void gemv_portable(int K, int N, const std::vector<int8_t>& aq, float sa,
                   const std::vector<int8_t>& blob, const std::vector<float>& scales,
                   const std::vector<float>& bias, int nblk, std::vector<float>& out) {
    const int kp = K / 32;
    const int np = N / 32;
...
void gemv_naive(int K, int N, const std::vector<int8_t>& aq, float sa,
                const std::vector<int8_t>& wq, const std::vector<float>& scale,
                const std::vector<float>& bias, int nblk, std::vector<float>& out) {
    out.assign(N, 0.0f);
    for (int o = 0; o < N; ++o) {
        float acc = 0.0f;
...
int run_case(int K, int N) {
    ...
    for (int k = 0; k < K; ++k) {
        a[k] = std::sin(static_cast<float>(k) * 17.0f * 0.013f) * 1.7f +
               (static_cast<float>(k % 7) - 3.0f) * 0.09f;
    }
