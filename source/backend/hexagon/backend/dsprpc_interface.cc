...
using rpcmem_cache_flush_t       = decltype(rpcmem_cache_flush);
using rpcmem_cache_invalidate_t  = decltype(rpcmem_cache_invalidate);
...
struct dsprpc_interface {
    ...
    rpcmem_cache_flush_t *      rpcmem_cache_flush_fn       = reinterpret_cast<rpcmem_cache_flush_t *>(load_fn("rpcmem_cache_flush"));
    rpcmem_cache_invalidate_t * rpcmem_cache_invalidate_fn  = reinterpret_cast<rpcmem_cache_invalidate_t *>(load_fn("rpcmem_cache_invalidate"));
...
int rpcmem_cache_flush(void * po, int size) {
    auto fn = dsprpc_interface::instance()->rpcmem_cache_flush_fn;
    return fn ? fn(po, size) : -1;
}

int rpcmem_cache_invalidate(void * po, int size) {
    auto fn = dsprpc_interface::instance()->rpcmem_cache_invalidate_fn;
    return fn ? fn(po, size) : -1;
}
