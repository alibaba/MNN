...
def build_deps():
    ...
    if IS_WINDOWS:
        os.system('cmake -G "Ninja" -DMNN_BUILD_TRAIN=ON -DMNN_BUILD_CONVERTER=on\
            ...
            -DMNN_AAPL_FMWK=OFF -DMNN_SEP_BUILD=OFF -DMNN_FMA_ENABLE=ON .. && ninja MNN MNNTrain MNNConvert')
    elif IS_LINUX:
        os.system('cmake -DMNN_BUILD_CONVERTER=on -DMNN_BUILD_TRAIN=ON -DCMAKE_BUILD_TYPE=Release\
            ...
            -DMNN_USE_THREAD_POOL=OFF -DMNN_FMA_ENABLE=ON .. && make MNN MNNTrain MNNConvert  -j4')
    else:
        os.system('cmake -DMNN_BUILD_CONVERTER=on -DMNN_BUILD_TRAIN=ON -DCMAKE_BUILD_TYPE=Release\
            -DMNN_BUILD_SHARED_LIBS=OFF -DMNN_AAPL_FMWK=OFF -DMNN_SEP_BUILD=OFF\
            -DMNN_FMA_ENABLE=ON .. && make MNN MNNTrain MNNConvert  -j4')
################################################################################
