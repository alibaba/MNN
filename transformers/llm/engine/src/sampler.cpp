...
LocalSampler::LocalSampler(Llm* llm, int max_new_tokens, struct LocalSamplerConfig config) {
    mLlm = llm;
    std::vector<int> history_ids_;
    mCandidates.emplace_back(history_ids_); // for LocalSampler, reference have never been modified manually.
...
void LocalSampler::penalty(MNN::Express::VARP logits, float penalty, bool penalizeNgram, int ngram, float ngram_factor) {
    ...
    // initialization
    std::vector<int>& prev = mCandidates[0];
...
std::string LocalSampler::sample(const std::vector<int>& input_ids, std::ostream* os, const char* end_with, struct TimePerformance* time_perf) {
    ...
    mCandidates[0].insert(mCandidates[0].end(), input_ids.begin(), input_ids.end());
    ...
    while (getGenLength(0, output_str.size()) < mMaxNewTokens) {
        ...
        decode_time.decode_prev_token_ = mCandidates[0].size();
        st = std::chrono::system_clock::now();
        // next token
        logits = mLlm->forward({mCandidates[0].back()}, false);
        if (nullptr == logits.get()) {
            return output_str;
        }
