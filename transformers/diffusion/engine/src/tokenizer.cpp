...
#if defined(MNN_DIFFUSION_WITH_LLM_TOKENIZER)
    delete mMtokTokenizer;
    mMtokTokenizer = nullptr;
#endif
}
...
bool CLIPTokenizer::loadMtok(const std::string& filePath) {
#if defined(MNN_DIFFUSION_WITH_LLM_TOKENIZER)
    ...
    input.close();
    delete mMtokTokenizer;
    mMtokTokenizer = MNN::Transformer::Tokenizer::createTokenizer(mtokPath);
    mUseMtok = (mMtokTokenizer != nullptr);
    if (!mUseMtok) {
        MNN_ERROR("Failed to create tokenizer from %s\n", mtokPath.c_str());
    } else {
        MNN_PRINT("[Tokenizer][CLIP] using tokenizer.mtok: %s\n", mtokPath.c_str());
    }
    return mUseMtok;
#else
    (void)filePath;
...
std::vector<int> CLIPTokenizer::encodeMtok(const std::string& sentence, int maxlen) {
#if defined(MNN_DIFFUSION_WITH_LLM_TOKENIZER)
    std::vector<int> ids(maxlen * 2, 0);
    auto normalizeClipTokens = [&](const std::string& text) {
        std::vector<int> tokens = mMtokTokenizer->encode(text);
        if (tokens.empty() || tokens[0] != mStartIdx) {
            tokens.insert(tokens.begin(), mStartIdx);
        }
        if ((int)tokens.size() >= maxlen) {
            tokens.resize(maxlen);
            tokens[maxlen - 1] = mEndIdx;
            return tokens;
        }
        if (tokens.empty() || tokens.back() != mEndIdx) {
            tokens.push_back(mEndIdx);
        }
        if ((int)tokens.size() > maxlen) {
            tokens.resize(maxlen);
            tokens[maxlen - 1] = mEndIdx;
        }
        while ((int)tokens.size() < maxlen) {
            tokens.push_back(0);
        }
        return tokens;
    };
    auto uncond = normalizeClipTokens("");
    auto cond = normalizeClipTokens(sentence);
    std::copy(uncond.begin(), uncond.end(), ids.begin());
    std::copy(cond.begin(), cond.end(), ids.begin() + maxlen);
    return ids;
#else
    (void)sentence;
    return std::vector<int>(maxlen * 2, 0);
#endif
}

bool T5Tokenizer::loadMtok(const std::string& filePath) {
#if defined(MNN_DIFFUSION_WITH_LLM_TOKENIZER)
    auto mtokPath = BuildMtokPath(filePath);
    std::ifstream input(mtokPath.c_str(), std::ios::binary);
    if (!input.good()) {
        MNN_PRINT("[Tokenizer][T5] tokenizer.mtok not found at %s, fallback to spiece.model\n", mtokPath.c_str());
        return false;
    }
    input.close();
    delete mMtokTokenizer;
    ...
    if (!mUseMtok) {
        ...
        MNN_PRINT("[Tokenizer][T5] using tokenizer.mtok: %s\n", mtokPath.c_str());
    }
    return mUseMtok;
#else
    (void)filePath;
...
std::vector<int> T5Tokenizer::encodeMtok(const std::string& sentence, int maxlen) {
#if defined(MNN_DIFFUSION_WITH_LLM_TOKENIZER)
    std::vector<int> ids = mMtokTokenizer->encode(sentence);

    // Keep mtok behavior aligned with legacy path: ensure EOS is present.
    if (ids.empty() || ids.back() != mEosId) {
...
#else
    (void)sentence;
    return std::vector<int>(maxlen, 0);
#endif
}
...
std::vector<int> T5Tokenizer::encode(const std::string& sentence, int maxlen) {
    if (mUseMtok && mMtokTokenizer) {
        return encodeMtok(sentence, maxlen);
    }
