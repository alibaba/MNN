...
bool SchedulerConfig::loadFromFile(const std::string& modelPath) {
    for (const auto& path : {modelPath + "/scheduler/scheduler_config.json",
                              modelPath + "/scheduler_config.json"}) {
        ...
        // Read basic scheduler parameters
        if (doc.HasMember("num_train_timesteps") && doc["num_train_timesteps"].IsInt())
            trainTimestepsNum = doc["num_train_timesteps"].GetInt();
        if (doc.HasMember("shift") && (doc["shift"].IsFloat() || doc["shift"].IsDouble()))
            shift = static_cast<float>(doc["shift"].GetDouble());
        if (doc.HasMember("use_dynamic_shifting") && doc["use_dynamic_shifting"].IsBool())
            useDynamicShifting = doc["use_dynamic_shifting"].GetBool();
        
        // Read dynamic shifting parameters
        if (doc.HasMember("base_image_seq_len") && doc["base_image_seq_len"].IsInt())
            baseImageSeqLen = doc["base_image_seq_len"].GetInt();
        if (doc.HasMember("max_image_seq_len") && doc["max_image_seq_len"].IsInt())
            maxImageSeqLen = doc["max_image_seq_len"].GetInt();
        if (doc.HasMember("base_shift") && (doc["base_shift"].IsFloat() || doc["base_shift"].IsDouble()))
            baseShift = static_cast<float>(doc["base_shift"].GetDouble());
        if (doc.HasMember("max_shift") && (doc["max_shift"].IsFloat() || doc["max_shift"].IsDouble()))
            maxShift = static_cast<float>(doc["max_shift"].GetDouble());
        
        MNN_PRINT("[SchedulerConfig] Loaded from %s: use_dynamic=%d, shift=%.2f, base_shift=%.2f, max_shift=%.2f\n",
                  path.c_str(), useDynamicShifting, shift, baseShift, maxShift);
        return true;
    }
    MNN_PRINT("[SchedulerConfig] No scheduler_config.json found, using defaults\n");
    return false;
}
