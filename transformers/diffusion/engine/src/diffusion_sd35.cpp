...
DiffusionSD35* DiffusionSD35::createDiffusionSD35(std::string modelPath, DiffusionModelType modelType,
                                                  MNNForwardType backendType, int memoryMode) {
    return new DiffusionSD35(modelPath, modelType, backendType, memoryMode);
}

DiffusionSD35::DiffusionSD35(std::string modelPath, DiffusionModelType modelType, MNNForwardType backendType,
                             int memoryMode)
...
bool DiffusionSD35::load() {
    ...
    // Keep runtime hints compatible with branches that do not expose QKV_QUANT_OPTIONS.
    ...
    // Load tokenizers
    if (!mTokenizer1 || !mTokenizer2 || !mTokenizer3) {
        MNN_ERROR("Tokenizer is not initialized\n");
        return false;
    }
    if (!mTokenizer1->load(mModelPath + "/tokenizer")) {
        MNN_ERROR("Failed to load tokenizer at %s/tokenizer\n", mModelPath.c_str());
        return false;
    }
    if (!mTokenizer2->load(mModelPath + "/tokenizer_2")) {
        MNN_ERROR("Failed to load tokenizer at %s/tokenizer_2\n", mModelPath.c_str());
        return false;
    }
    if (!mTokenizer3->load(mModelPath + "/tokenizer_3")) {
        MNN_ERROR("Failed to load tokenizer at %s/tokenizer_3\n", mModelPath.c_str());
        return false;
    }
...
bool DiffusionSD35::loadModule(int index) {
    ...
    std::vector<std::string> modelNames = {"text_encoder.mnn", "text_encoder_2.mnn", "text_encoder_3.mnn",
                                           "transformer.mnn", "vae_decoder.mnn"};
    ...
    if (mModules[index])
        mModules[index]->traceOrOptimize(MNN::Interpreter::Session_Resize_Fix);
    return true;
}
...
VARP DiffusionSD35::transformer(VARP hidden_states, VARP encoder_hidden_states, VARP pooled_projections,
                                VARP timestep) {
...
bool DiffusionSD35::run(const std::string prompt, const std::string imagePath, int iterNum, int randomSeed,
