...
int main(int argc, const char* argv[]) {
    ...
    if (memory_mode == 1) {
        MNN_PRINT(
            "(Memory Enough) All Diffusion models will be initialized when application enter. with fast "
            "initialization\n");
    } else {
        MNN_PRINT(
            "(Memory Lack) Each diffusion model will be initialized when using, freed after using. with slow "
            "initialization\n");
    }
    ...
    std::shared_ptr<DiffusionSD35> diffusion(
        DiffusionSD35::createDiffusionSD35(resource_path, STABLE_DIFFUSION_1_5, backend_type, memory_mode));
