...
本仓库已有可抄的范例：`test/speed/GemvBWTest.cpp` 就是**对标外部参照实现的 GEMV roofline 微基准**建的，
默认形状 4096×14336 取自 Llama-3-8B 风格 FFN。它显式声明了口径：**W bytes 只算权重 + per-block
（alpha + zp，fp16）元数据，不算输入向量与输出**——两边用同一个分母，`eff GB/s` 才能直接比。

于是能得到这类可交付的结论（`docs/perf/arm_low_bit_gemm.md`）：

| | MNN eff GB/s | vs 对手基准 |
|---|---|---|
| W8 | 109.4 | 超过对手同位宽档（109） |
| W4 | 100.7 | **+29%** vs 对手同位宽档（78） |
| W3 | 50.2 | 与对手同位宽档持平 |
| W2 | 64.5 | **+58%** vs 对手同位宽档（41） |

要逐项对齐的：分母口径（算不算输入/输出/元数据）、block size、量化方案是否等价
（对手的某个 w4 方案 ≠ 任意 w4）、线程数与绑核、cache 状态（冷/热）、迭代次数与取值方式（best-of vs 均值）、
