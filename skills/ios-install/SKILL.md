...
- **Model/runtime schema compatibility**: a valid checksum only proves asset integrity; it does not prove that an externally supplied `.mnn` file matches the framework's generated schema. Build `MNNConvert` and `MNN.framework` from the same source revision. When source ONNX is available, re-export the model with that converter instead of carrying a `.mnn` binary across branches. Always run an on-device model-load or silent inference smoke before declaring the install complete.
