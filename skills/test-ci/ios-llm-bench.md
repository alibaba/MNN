...
- **Diffusion framework 还需要 tokenizer 支持**：即使只跑 SD1.5，iOS framework 也要同时启用 `MNN_BUILD_DIFFUSION=ON`、`MNN_BUILD_OPENCV=ON`、`MNN_IMGCODECS=ON` 和 `MNN_BUILD_LLM=ON`；缺少最后一项时 `tokenizer.mtok` 会在 `Diffusion::load()` 直接失败，尚未进入被测推理路径。
