...
8. **业务完成状态不要依赖展示层动画回调**：异步推理的按钮解锁、任务结束和性能数据提交，应由引擎的权威完成事件驱动；打字机、Markdown、列表 cell 等展示层可能因视图重建或离屏而丢失回调，只能影响动画本身。验收时至少覆盖未闭合代码块、长文本、滚动离屏和连续多次请求，并确认每次都能恢复交互。
9. **设备侧计算结果用 `readMap()`，不要用 `writeMap()` 读取**：CPU 输出位于主存时，错误使用 `writeMap()` 可能碰巧可用；Metal 等 GPU 输出可能是只读设备变量，`writeMap()` 会返回空指针。需要修改 logits 等输出时，先用 `readMap()` 同步到主机，再复制到可写 scratch；对映射结果做空指针检查，并在真机 GPU 回归中覆盖这条路径。
10. **跨前端对拍先证明模型身份一致**：UI 显示名、自定义名和目录名都不能代表模型身份；先比较 `llm.mnn`、权重、`llm_config.json`、tokenizer 的哈希及导出参数，再对齐 effective config（sampler、max tokens、backend、env）。把设备产物与配置搬到 host 复现；若同样退化，先查模型/导出，避免误判为 UI、历史或设备后端。
