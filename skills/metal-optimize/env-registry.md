...
| `MNN_METAL_DISABLE_GATEUP_SILU` | **开融合**（unset/=0 时启用）| 让 `MUL_SILU` 保持独立 dispatch（gate/up 先落内存再相乘），而不是折进 fused gate/up GEMV 的 epilogue | Gate/Up fusion 已建立的 decode GEMV；`GATE_UP_SILU` 宏把双行机制从"一矩阵两相邻 slice"改成"gate/up 同一 slice"，epilogue 直接写 `up * silu(gate)`。非 tensor-API 设备（ROW_2）：grid.x 翻倍、grid.z 2→1（TG 数与每 TG 的 4 个 slice-dot 均不变）；**tensor-API 设备（M5+）该管线单独强制带 ROW_2 编译**，grid 与原单行 fused grid 相同、z 2→1。无 LN 折叠时若 SwiGLU 输出与投影输入同址，先 re-home 到 STATIC，失败则保持 MUL_SILU 独立 | 实验：每层省 1 个 MUL_SILU dispatch + gate/up 两个中间张量的显存往返；M4 Pro 0.6B tg128 配对 +1.9%（3/3 同向）；M5 待标定（双行流对宽 SM 寄存器压力未知，若实测负收益设 =1 回退） |
...
| `MNN_METAL_DISABLE_QKV_PACKED_GRID` | **开 packing**（unset/=0 时启用）| 恢复矩形 grid：`grid.x` 按最大投影取 `UP_DIV(maxSlice, slicesPerTG)`、z 选投影 | QKV fusion 已建立且成员 output_channel 不等（`packedGridX < numProj * maxGridX`，等宽 q/k/v 组自动走矩形）；`QKV_PACKED_GRID` 宏把各投影的 TG 区间沿 x 首尾相接（grid.z 1），shader 用 `qkv_seg[6..8]` 基址级联反解 (projection, local x)，seg buffer 6→9 floats | 实验：消除小投影上只做 early-return 的 TG（Qwen3.5 linear 层 qkv 6144 / z 2048 / b 16 / a 16，矩形 grid 67% TG 空转）；本机 M4 Pro Qwen3.5-2B tg128 +0.7%（噪声内偏正），窄小投影模型收益更大 |
| `MNN_METAL_DISABLE_LN_SPLIT_SG` | **开 split**（unset/=0 时启用）| 恢复旧行为：threadgroup 里每个 simdgroup 各自重扫整个 input+residual 重算 inv_rms | LN_FUSED pipeline（64 线程 = 2 simdgroup）；`LN_SPLIT_SG` 宏让两个 simdgroup 各扫一半 input（z+=64），经 `threadgroup float ln_sq_partial[2]` + barrier 归约一次；配套把 LN 前导段整体上提到 per-simdgroup 出界检查之前（`tg_first_uz` 全 TG 一致检查可提前退出，掉队 simdgroup 压到归约之后再退，保证 barrier 全员可达） | 实验：LN 前导的 input+residual 读带宽减半；与 QKV_PACKED_GRID/GEMV_MIDDLE_STEP 三项合并实测 M4 Pro 0.6B tg128 **+2.0%**（340.9→347.7，4/4 轮同向） |
| `MNN_METAL_SPLITK_LEGACY_LANES` | **关**（新 lane 划分 + 编译期常量，默认） | =1 回退 `block/4` 且不下发 `GEMV_MIDDLE_STEP` 宏 | SPLIT_K_2 decode GEMV pipeline；host 侧 `gemvMiddleStep()` 按 simdgroup 实际覆盖的 quant block 数定 middle_step（每 lane 恰好扫 1 个 block、内层归约不低于 2 lane、取 2 的幂），以 `GEMV_MIDDLE_STEP` 编译期常量注入（cache key 追加 `ms<N>`），编译器可对热路径 %/÷ 做强度削减 | 实验：修复 SPLIT_K_2 下少而肥的 block 形状上 per-block scale/bias 重复加载；三项合并 +2.0% 见上行 |
