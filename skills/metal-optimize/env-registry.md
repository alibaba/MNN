...
| `MNN_METAL_W4W8_OUTER_DEQUANT_GEMM_TENSORAPI` | **关**（fused in-kernel unpack 默认启用）| =1 走 outer-dequant + fp GEMM（A/B 基线 / 紧急回滚）| unset/=0 走 fused | tensor-API 设备 + Q4/Q8 + area≥64（prefill）；非 M5 设备本开关为 no-op | 定型；fused 路径含 M64 tile 自动策略（Q4 + area≥128，M5 pp512 +5.9% / pp2048 +6.8%）。**2026-07-31 由 `MNN_METAL_DISABLE_FUSED_Q4_GEMM` 改名**（正向表述 + W4W8 位宽 + TENSORAPI 标志，旧名已失效）|
| `MNN_METAL_GEMV_SPLITK` | **开** | (无效，默认开) | =0 恢复旧 2sg GEMV（64 线程，每行 1 个 simdgroup 流式读整行） | decode GEMV (area==1) + Q4/Q8 + oc%8==0 + 偶数 quant block 数 | 实验（2026-07-28）：`SPLIT_K_2` = 同一 2sg kernel 4 simdgroup/tg（每行 2 个 K 半段 + tg 归约），行内在途读加倍；M4 Pro Qwen3-0.6B tg 12/512/2048 +3.9%/+3.8%/+3.3%。注意先走 g8 kernel 的方案（nibble-unpack 内循环）反而 -5%，勿回退到那个方向 |
| `MNN_METAL_LINEAR_ATTN_SGMM` | **开** | (无效，默认开) | =0 回退标量 `fused_chunk_sg` 长 prefill 路径 | LinearAttention (gated_delta_rule) prefill L≥16 + 非 tensor-API 设备 + supportSimdGroupMatrix + **dk==128** + dv%16==0 | 实验（2026-08-03）：`linear_attn_flash_chunk_sgmm` = flash_chunk 分块算法的 simdgroup_float8x8 (8x8 MMA) 移植，SIMDS_PER_TG=16。M4 Pro 双向配对 rep3：Qwen3.5-0.8B pp512 **+14.7%** / pp2048 **+15.8%**，2B pp2048 **+8.2%**；dk=64 保留标量路径。同批合入：linear-attn decode encode-replay 与短 prefill 2≤L<16 路由 fused_sg_align。|
...
|---|---|---|---|---|
| `MNN_METAL_DISABLE_LN_FUSION` | **开融合**（unset/=0 时启用）| 关掉 LayerNorm fusion，恢复独立 dispatch | Metal 后端；merged proj 场景 | 定型 A/B；日常不动。2026-07-31 起按值判断（仅 =1 关闭；旧行为是设任意值即关）|
| `MNN_METAL_DISABLE_GATE_UP_FUSION` | **开融合**（unset/=0 时启用）| 关掉 Gate/Up leader/follower fusion | Metal 后端；MUL_SILU 匹配 | 定型 A/B。2026-07-31 起按值判断（仅 =1 关闭）；**同日由 `MNN_DISABLE_GATE_UP_FUSION` 改名补上 METAL 前缀，旧名已失效**|
| `MNN_METAL_DISABLE_QKV_FUSION` | **开融合**（unset/=0 时启用）| 关掉 Q/K/V leader/follower fusion，恢复 3 个独立 GEMV dispatch | decode GEMV + 同一输入恰有 3 个 conv1x1 消费者（q/k/v 投影）；input-LN 会进一步融进 QKV leader（受 `MNN_METAL_DISABLE_LN_FUSION` 控制）| 实验（2026-07-28）：M4 Pro Qwen3-0.6B tg 12/512 QKV 融合 +2.7%/+1.6%，叠加 LN 融合再 +1.9%/+1.7%；k/v 输出 re-home 到 STATIC 内存避免提前写被动态池复用覆写（详见 MetalConvolution1x1::setupQKVFusion 注释）。2026-07-31 起按值判断（仅 =1 关闭）|
...
| ~~`MNN_METAL_GEMV_ROW2`~~ | 已删除（2026-07-28 收敛）| M4 正（p12 +3.9% / p512 +2.3%）/ M5 中性偏负 双端标定完成，ROW_2 收敛为 non-tensor-API 设备门控（`MetalConvolution1x1.mm` 三处 setup 统一），三态覆盖开关删除 |
