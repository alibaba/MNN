...
|---|---|---|---|---|
| `MNN_METAL_DISABLE_LN_FUSION` | **开融合**（unset/=0 时启用）| 关掉 LayerNorm fusion，恢复独立 dispatch | Metal 后端；merged proj 场景 | 定型 A/B；日常不动。2026-07-31 起按值判断（仅 =1 关闭；旧行为是设任意值即关）。**2026-08-03 覆盖面扩大**：原先只在 gate/up leader（group=2）与 sole-consumer 上生效，现已可挂载 Qwen3.5 linear-attention 层的 4 投影 QKV leader（group=4，2B/0.8B 各 18 层）。前置条件是 `matchLNFusions` 里的**分配器别名检测**——融合 dispatch 写的张量（组内全部投影输出 + `residualOutput`）与它读的张量（`hiddenInput`/`residualInput`）若字节区间重叠，把冲突输出 re-home 到 `Backend::STATIC`；re-home 失败则跳过该层融合。此别名（`out_q` 与 `ln_res_in` 同址）正是此前 LN×P4 逐次非确定的根因，详见 [`graph-fusion.md`](./graph-fusion.md) §4.3。收益：M4 Pro 双向配对 0.8B tg128@p512 **+1.6%** / 2B **+1.4%**（5 对全同向）；确定性 5 连跑 hash 全同，run_test 384/392 与基线一致。**2026-08-04 起 `MetalLayerNorm` 还会注册单输入 NC4HW4 RMSNorm**（gated norm），供 linear-attention **输出门控段折叠**匹配（7 dispatch→1，0.8B decode +6.4% / 2B +2.9%，见 [`graph-fusion.md`](./graph-fusion.md) §5）——该折叠**无独立 env 开关**，回退为 commit 级；`MNN_METAL_DISABLE_LN_FUSION=1` 不影响它（走的是另一条注册表） **2026-08-06 该注册已删除**：输出门控段折叠上移为导出图的 `GatedRMSNorm` op（导出开关 `--disable_fuse_gated_norm`，默认开；见 [`graph-fusion.md`](./graph-fusion.md) §5），`matchLinearAttnGatedNormFolds` 与 RasterCopy/Cast/RmsNorm/Elemwise 四张注册表全部删除，Metal 后端至此**零图匹配**；旧模型失去该折叠（输出不变，重导出拿回收益）。 **2026-08-05 语义变更**：融合决策已上移到导出图。`matchQKVFusions`/`matchLNFusions` 两个运行时匹配器连同注册表（`mInputToConv1x1Group`/`mLayernormMap`）已删除，改由 `MetalFusedProj::setupFusion()` 按导出的 `FusedLinearParam` 成员顺序确定性建立（backend 在 `onResizeEnd` 的 allocator `compute()` 之后回调）。本开关现在控制该函数内的对应分支。**未带 `--fuse_qkv_proj`/`--fuse_gate_up_proj`/`--fuse_ln_proj` 导出的旧模型在 Metal 上不再享有这两项融合**（实测 Qwen3.5-0.8B decode dispatch 246→365），需重新导出。 |
