...
| `MNN_METAL_DISABLE_GATEUP_SILU` | **开融合**（unset/=0 时启用）| 让 `MUL_SILU` 保持独立 dispatch（gate/up 先落内存再相乘），而不是折进 fused gate/up GEMV 的 epilogue | Gate/Up fusion 已建立 + ROW_2（非 tensor-API 设备）decode GEMV；`GATE_UP_SILU` 宏把双行机制从"一矩阵两相邻 slice"改成"gate/up 同一 slice"，epilogue 直接写 `up * silu(gate)`，grid.x 翻倍、grid.z 2→1（TG 数与每 TG 的 4 个 slice-dot 均不变）；无 LN 折叠时若 SwiGLU 输出与投影输入同址，先 re-home 到 STATIC，失败则保持 MUL_SILU 独立 | 实验：每层省 1 个 MUL_SILU dispatch + gate/up 两个中间张量的显存往返；融合只在 leader epilogue 支持的形状上生效，需读回 `gateUpSiluFused()` 决策 |
