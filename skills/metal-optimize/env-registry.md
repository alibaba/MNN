...
| `MNN_METAL_GEMV_SPLITK` | **1 = SPLIT_K_2（默认开）** | =2 切 `SPLIT_K_SHUFFLE`（A/B 实验，2026-08-18）：K 半段收进单 simdgroup（lane 0-15 下半 / 16-31 上半，`simd_sum` 顺带合并两半，无 tg mem/barrier），64 线程/tg，`gemvMiddleStep` 按 16 lane 定 `GEMV_MIDDLE_STEP` | =0 恢复旧 2sg GEMV（64 线程，每行 1 个 simdgroup 流式读整行） | decode GEMV (area==1) + Q4/Q8 + oc%8==0 + 偶数 quant block 数（三档共用同一门控） | 实验（2026-07-28）：`SPLIT_K_2` = 同一 2sg kernel 4 simdgroup/tg（每行 2 个 K 半段 + tg 归约），行内在途读加倍；M4 Pro Qwen3-0.6B tg 12/512/2048 +3.9%/+3.8%/+3.3%。注意先走 g8 kernel 的方案（nibble-unpack 内循环）反而 -5%，勿回退到那个方向。=2 检验 SPLIT_K_2 的收益来自「每行 64 在途 lane」还是「省 barrier」：**已判定（2026-08-18 M4 Pro 0.6B W4 双向配对 rep5）**——tg128 纯 decode：=0 286.6 / =1 **294.6** / =2 291.8 tok/s，即 shuffle 版比 legacy +1.8% 但比 SPLIT_K_2 **-1.0%**（4/4 样本序一致）；pp2048+tg128 两者持平（噪声内）。结论：收益主体是在途 lane 加倍，barrier 非瓶颈，=1 保持默认；=2 保留作 tensor-API/M5 设备待标定（barrier 成本可能不同）|
| `MNN_METAL_ENABLE_LMHEAD_SPLITK` | **关**（实验开关，默认走 g16）| =1 把超大 oc 的 lm_head decode GEMV（oc>16384，原 g16 路由）改走 SPLIT_K_2 kernel；=2 在 g16 kernel 内部做 split-K（`G16_SPLIT_K`：128 线程/tg，每两个 simdgroup 结对拆同一 dual-row 的 K 半段 + tg mem 归约，grid 不变、oc%16==0 保证 barrier 前无早退） | unset/=0 走 g16 | decode + area==1；=1 需 gemvSplitK>0 + oc%8==0 + blockSize 偶数；=2 需 oc%16==0 + blockSize 偶数 | 实验（2026-08-18，M4 Pro Qwen3.5-2B b64 ic=2048/oc=248320）：**=1** kernel 级 2310us→87us（-96%，profiler 数字受 encode 开销失真，仅诊断用），e2e decode 仅 **+1.3%**（100.7→102.0，10 轮交替配对正反序同向，greedy byte-identical）⇒ **decode 尾部已是 lm_head→采样 GPU→CPU sync-bound**。**=2（g16 内 split-K）已证伪**：greedy byte-identical，但 kernel 时间与基线持平（2257us vs 2270us，71/72 calls）——lm_head 每 token 流 228MB 权重，早已是带宽瓶颈（~108GB/s 已是该形状实测上限），加倍在途 lane 不减少搬运字节数，split-K 无收益；e2e 配对 A 均值 102.04 vs B 100.89（6 轮正反序，B 三次跌到 ~100，中性偏负，barrier 开销）。结论：lm_head 方向停止 kernel 级优化；下一个杠杆是设备端采样 / 消除该同步点。M5/iPad 未标定 |
...
| ~~`MNN_METAL_DISABLE_GATEUP_SILU`~~ | 已删除（2026-08-17）| SwiGLU epilogue 折叠转为 fused gate/up dispatch 的**唯一形态**（`mGateUpSilu = siluOutput != nullptr`），env 门控删除；SwiGLU 别名 re-home 与 LN 别名检测保留。M4 Pro 0.6B tg128 配对 +1.9%（3/3 同向）；M5 尚未上机标定，若 A/B 判负从 git 历史恢复 env 即可 |
| ~~`MNN_METAL_DISABLE_QKV_PACKED_GRID`~~ | 已删除（2026-08-17）| Packed grid 转为 shape 门（`packedGridX < numProj * maxGridX`）**唯一形态**，env 删除；等宽 q/k/v 组自然走矩形（packed 命中条件不成立）。M4 Pro Qwen3.5-2B tg128 +0.7%（噪声内偏正），窄小投影模型收益更大 |
| ~~`MNN_METAL_DISABLE_LN_SPLIT_SG`~~ | 已删除（2026-08-17）| LN_FUSED pipeline 恒带 `LN_SPLIT_SG` 宏，env 删除；两个 simdgroup 各扫一半 input + threadgroup 归约成为唯一路径。与 QKV_PACKED_GRID / GEMV_MIDDLE_STEP 合并 M4 Pro 0.6B tg128 +2.0%（340.9→347.7，4/4 同向）|
| ~~`MNN_METAL_SPLITK_LEGACY_LANES`~~ | 已删除（2026-08-17）| SPLIT_K_2 的 host 侧 `gemvMiddleStep()` lane 划分 + `GEMV_MIDDLE_STEP` 编译期常量转为唯一形态，env 删除；cache key 追加 `ms<N>` 保留（多形状复用同 kernel）。收益并入上一行 |
