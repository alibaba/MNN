...
VARP _Input(INTS dims, Dimensionformat format, halide_type_t type) {
    ...
    input->main.AsInput()->dtype   = _convertDataType(type);
    MNN_ASSERT(input->main.AsInput()->dtype != DataType_DT_INVALID);
    input->main.AsInput()->dims    = dims;
    input->main.AsInput()->dformat = _convertFormat(format);
    return (Variable::create(Expr::create(std::move(input), {})));
}
VARP _Const(const void* ptr, INTS dims, Dimensionformat format, halide_type_t type) {
    ...
    auto blob        = new BlobT;
    blob->dataFormat = _convertFormat(format);
    blob->dims       = dims;
    int length       = 1;
    ...
    op->main.value = blob;
    return (Variable::create(Expr::create(std::move(op), {})));
}
...
VARP _Const(float value, INTS dims, Dimensionformat format) {
    ...
    constOp->main.AsBlob()->dataFormat = _convertFormat(format);
    ...
    return (Variable::create(Expr::create(std::move(constOp), {})));
}
...
VARP _Conv(std::vector<float>&& weight, std::vector<float>&& bias, VARP x, INTS channel, INTS kernelSize,
           PaddingMode pad, INTS stride, INTS dilate, int group) {
    std::unique_ptr<OpT> convOp(new OpT);
    convOp->type = OpType_Convolution;
    if (channel[0] == channel[1] && channel[0] == group) {
        convOp->type = OpType_ConvolutionDepthwise;
    ...
    return (Variable::create(Expr::create(std::move(convOp), {x})));
}
...
static VARP _Pool(VARP x, INTS kernel, INTS stride, PoolType type, PaddingMode pad, INTS pads) {
    ...
    static std::map<PaddingMode, PoolPadType> padmap{
        {CAFFE, PoolPadType_CAFFE},
        {VALID, PoolPadType_VALID},
        {SAME, PoolPadType_SAME},
    };
    ...
    pool->main.AsPool()->padType = padmap[pad];
    ...
    return (Variable::create(Expr::create(std::move(pool), {x})));
}
...
VARP _Reshape(VARP x, INTS dim) {
    ...
    return (Variable::create(Expr::create(std::move(reshape), {x})));
}
VARP _Reshape(VARP x, VARP shape) {
    ...
    reshape->main.value                = new ReshapeT;
    reshape->main.AsReshape()->dimType = MNN_DATA_FORMAT_NCHW;
    return (Variable::create(Expr::create(std::move(reshape), {x, shape})));
}
...
VARP _Relu(VARP x, float slope) {
    ...
    return (Variable::create(Expr::create(std::move(relu), {x})));
}
VARP _Relu6(VARP x) {
    std::unique_ptr<OpT> relu(new OpT);
...
VARP _Softmax(VARP x, int axis) {
    ...
    return (Variable::create(Expr::create(std::move(softmax), {x})));
}
VARP _Concat(VARPS xs, int axis) {
    ...
    return (Variable::create(Expr::create(std::move(concat), xs)));
}
...
VARP _Convert(VARP x, Dimensionformat dest) {
    ...
    convert->main.AsTensorConvertInfo()->source = _convertFormat(source);
    convert->main.AsTensorConvertInfo()->dest   = _convertFormat(dest);
    return (Variable::create(Expr::create(std::move(convert), {x})));
}
...
VARP _ChannelShuffle(VARP x, int group) {
    x = _Convert(x, NHWC);
    x = _Reshape(x, {0, 0, 0, group, -1});
    x = _Transpose(x, {0, 1, 2, 4, 3});
    x = _Reshape(x, {0, 0, 0, -1});
    x = _Convert(x, NC4HW4);
    return x;
}
...
VARP _Pack(VARPS xs, halide_type_t dtype, int axis) {
    ...
    pack->main.AsPackParam()->dataType = _convertDataType(dtype);
