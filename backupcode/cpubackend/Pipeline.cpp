...
ErrorCode Pipeline::execute() {
    ...
    for (auto& info : mInfo.second) {
        ...
        for (int cmdIndex=0; cmdIndex<buffer.command.size(); ++cmdIndex) {
            ...
            // // print only GatedMLP convolution operator results
            // if (cmd.workInputs.size() == 1 && cmd.workOutputs.size() == 1) {
            //     if (cmd.workInputs[0]->elementSize()>20 && cmd.workOutputs[0]->elementSize()>20) {
            //         if (cmd.workInputs[0]->elementSize() != cmd.workOutputs[0]->elementSize() \
            //             || cmd.workInputs[0]->host<float>()[0] != cmd.workOutputs[0]->host<float>()[0]){
            //             if (cmd.workOutputs[0]->shape()[1] > 2000) {
            //                 std::cout << "convolution input shape: " << std::endl;
            //                 for (auto s : cmd.workInputs[0]->shape()) {
            //                     std::cout << s << " ";
            //                 }
            //                 std::cout << std::endl;
            //                 auto ptr = cmd.workInputs[0]->host<float>();
            //                 for (int i = 0; i<cmd.workInputs[0]->elementSize(); ++i) {
            //                     std::cout << ptr[i] << " ";
            //                 }
            //                 std::cout << std::endl;
            //                 std::cout << "convolution output shape: " << std::endl;
            //                 for (auto s : cmd.workOutputs[0]->shape()) {
            //                     std::cout << s << " ";
            //                 }
            //                 std::cout << std::endl;
            //                 ptr = cmd.workOutputs[0]->host<float>();
            //                 for (int i = 0; i<cmd.workOutputs[0]->elementSize(); ++i) {
            //                     std::cout << ptr[i] << " ";
            //                 }
