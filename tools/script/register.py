...
def generateNeuronAdapterFile(rootDir):
    coremlDir = os.path.join(rootDir, "source", "backend", "mtk_neuron_adapter")
    coremlExeDir = os.path.join(coremlDir, "execution")
    coremlRegFile = os.path.join(coremlDir, "backend", "NeuronAdapterOPRegister.cpp")
    fileNames = os.listdir(coremlExeDir)
    print(fileNames)
    if len(fileNames) <= 1:
        # Error dirs
    ...
    for fi in fileNames:
        ...
        with open(f) as fileC:
            c = fileC.read().split('\n')
            c = list(filter(lambda l:l.find('REGISTER_NeuronAdapter_OP_CREATOR')>=0, c))
            c = list(filter(lambda l:l.find('OpType')>=0, c))
            for l in c:
                l = l.split('(')[1]
                l = l.split(')')[0]
    ...
    with open(coremlRegFile, 'w') as f:
        ...
        for l in funcNames:
            f.write("extern void " + l + '();\n')
        f.write('\n')
        f.write('void registerNeuronAdapterOps() {\n')
        for l in funcNames:
            f.write(l+'();\n')
        f.write("}\n}\n")
...
generateNeuronAdapterFile(sys.argv[1])
