...
private:
    ...
    bool fuseAttentionInputsC4() {
        rebuildMaps();
        std::set<int> removeIndexes;
        for (int idx = 0; idx < (int)mOps.size(); ++idx) {
            auto attention = mOps[idx].get();
            if (attention->type != OpType_Attention || attention->main.type != OpParameter_AttentionParam ||
                attention->main.AsAttentionParam() == nullptr || !attention->main.AsAttentionParam()->output_c4 ||
                attention->inputIndexes.size() < 3) {
                continue;
            }
            int ropeIdx = producerOf(attention->inputIndexes[0]);
            if (ropeIdx < 0 || ropeIdx != producerOf(attention->inputIndexes[1]) ||
                !sameConsumers(mConsumers, attention->inputIndexes[0], idx) ||
                !sameConsumers(mConsumers, attention->inputIndexes[1], idx)) {
                continue;
            }
            ...
            auto qMatch = matchRoPEInput(rope->inputIndexes[0], ropeIdx);
            auto kMatch = matchRoPEInput(rope->inputIndexes[1], ropeIdx);
            auto vMatch = matchAttentionValue(attention->inputIndexes[2], idx);
            int headDim = ropeConfig->head_dim > 0 ? ropeConfig->head_dim : ropeConfig->rope_cut_head_dim;
            if (!qMatch.valid || !kMatch.valid || !vMatch.valid || headDim <= 0 || headDim % 4 != 0 ||
                qMatch.channel % headDim != 0 || kMatch.channel % headDim != 0 ||
                vMatch.channel != kMatch.channel) {
                continue;
            }
            int numHead = qMatch.channel / headDim;
            int kvNumHead = kMatch.channel / headDim;
            auto attentionConfig = attention->main.AsAttentionParam();
            if ((ropeConfig->num_head > 0 && ropeConfig->num_head != numHead) ||
                (ropeConfig->kv_num_head > 0 && ropeConfig->kv_num_head != kvNumHead) ||
                (attentionConfig->head_dim > 0 && attentionConfig->head_dim != headDim)) {
                continue;
            }

            rope->inputIndexes[0] = qMatch.input;
            rope->inputIndexes[1] = kMatch.input;
            rope->defaultDimentionFormat = MNN_DATA_FORMAT_NC4HW4;
            ropeConfig->num_head = numHead;
            ropeConfig->kv_num_head = kvNumHead;
            ropeConfig->head_dim = headDim;
            attention->inputIndexes[2] = vMatch.input;
            attention->defaultDimentionFormat = MNN_DATA_FORMAT_NC4HW4;
            attentionConfig->head_dim = headDim;
            ...
            removeIndexes.insert(vMatch.removeIndexes.begin(), vMatch.removeIndexes.end());
        }
        bool changed = !removeIndexes.empty();
        removeOps(removeIndexes);
        return changed;
    ...
    bool ensureRoPEInputsC4() {
        ...
        for (int idx = 0; idx < (int)mOps.size(); ++idx) {
            ...
            if (param == nullptr || rope->defaultDimentionFormat == MNN_DATA_FORMAT_NC4HW4 ||
                rope->inputIndexes.size() < 4 || rope->outputIndexes.size() != 2 || param->num_head <= 0 ||
                param->kv_num_head <= 0 || param->head_dim <= 0 || param->head_dim % 4 != 0) {
                continue;
            }
            ...
            for (int output : rope->outputIndexes) {
                auto consumers = mConsumers.find(output);
                if (consumers == mConsumers.end()) {
                    continue;
                }
                for (int consumer : consumers->second) {
                    if (mOps[consumer]->type == OpType_Attention) {
                        mOps[consumer]->defaultDimentionFormat = MNN_DATA_FORMAT_NHWC;
                    }
                }
            }
            ...
            auto restoreOutput = [&](int outputPosition, int numHead, const std::string& prefix) {
                int originalOutput = rope->outputIndexes[outputPosition];
                int packedOutput = buildUniqueTensor(prefix + "_packed");
                int contiguousOutput = buildUniqueTensor(prefix + "_convert");
                rope->outputIndexes[outputPosition] = packedOutput;
                insertAfter[idx].push_back(makeConvert(mTensors[contiguousOutput], packedOutput, contiguousOutput,
                                                       MNN_DATA_FORMAT_NC4HW4, MNN_DATA_FORMAT_NCHW));
                insertAfter[idx].push_back(makeReshape(mTensors[originalOutput], contiguousOutput, originalOutput,
                                                       {1, -1, numHead, param->head_dim}));
            };
            restoreOutput(0, param->num_head, "FuseTransformerC4_rope_q_output");
            restoreOutput(1, param->kv_num_head, "FuseTransformerC4_rope_k_output");
            rope->defaultDimentionFormat = MNN_DATA_FORMAT_NC4HW4;
            changed = true;
        }
        if (!changed) {
        ...
        std::vector<std::unique_ptr<OpT>> newOps;
        newOps.reserve(mOps.size() + insertBefore.size() * 4 + insertAfter.size() * 4);
        for (int idx = 0; idx < (int)mOps.size(); ++idx) {
        ...
        for (auto& plans : regions) {
            ...
            for (auto& plan : plans) {
                ...
                if (plan.linearAttention.valid) {
                    ...
                    auto attention = mOps[plan.attentionIdx].get();
                    auto rope = mOps[plan.ropeIdx].get();
                    auto attentionParam = attention == nullptr ? nullptr : attention->main.AsAttentionParam();
                    auto ropeConfig = ropeParam(rope);
                    if (attentionParam == nullptr || ropeConfig == nullptr) {
                        return false;
                    }
                    attentionParam->head_dim = ropeConfig->head_dim;
                    attention->defaultDimentionFormat = MNN_DATA_FORMAT_NC4HW4;
                    rope->defaultDimentionFormat = MNN_DATA_FORMAT_NC4HW4;
                }
        ...
        for (auto& opPtr : mOps) {
            ...
            if (op->type == OpType_Attention && op->main.type == OpParameter_AttentionParam &&
                ...
                if (op->defaultDimentionFormat == MNN_DATA_FORMAT_NC4HW4) {
                    auto ropeConfig = attentionRoPEParam(op);
                    int valueProducer = op->inputIndexes.size() < 3 ? -1 : producerOf(op->inputIndexes[2]);
                    if (param->head_dim <= 0 || ropeConfig == nullptr || ropeConfig->num_head <= 0 ||
                        ropeConfig->kv_num_head <= 0 || param->head_dim != ropeConfig->head_dim ||
                        ropeConfig->num_head % ropeConfig->kv_num_head != 0 || !param->output_c4 ||
                        valueProducer < 0 || !isConvolution(mOps[valueProducer].get()) ||
                        convolutionOutputCount(mOps[valueProducer].get()) !=
                            ropeConfig->kv_num_head * ropeConfig->head_dim) {
                        return false;
                    }
                }
                if (!param->output_c4) {
