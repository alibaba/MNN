...
template <typename T>
static void ResizeAndCopyData(std::vector<uint8_t> *vec_data, const T *src,
...
public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    MNN_CHECK(1 == inputs.size(),
              "Onnx ConstantOfShape should have one input!");
    ...
    for (int i = 0; i < extraParam->attr()->size(); ++i) {
      ...
      if (key == "value") {
        ...
        if (blob->dataType() == DataType_DT_INT32) {
          ...
          ResizeAndCopyData<int>(&tensor_data, blob->int32s()->data(),
                                 tensor_shape);
        } else if (blob->dataType() == DataType_DT_FLOAT) {
          data_type = halide_type_of<float>();
          ResizeAndCopyData<float>(&tensor_data, blob->float32s()->data(),
                                   tensor_shape);
        } else {
    ...
    auto const_shape = _Const(static_cast<const void *>(tensor_data.data()),
                              tensor_shape, NHWC, data_type);
...
static auto gRegister = []() {
  OnnxExtraManager::get()->insert("ConstantOfShape",
                                  std::shared_ptr<OnnxExtraManager::Transform>(
                                      new OnnxConstantOfShapeTransform));
  return true;
}();
