...
public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    if (inputs.size() == 1 && nullptr == attrs) {
      MNN_PRINT(
          "Attrs of Slice in ONNX must not be null when inputs.size == 1\n");
      return nullptr;
    }
    std::vector<int> starts, ends, axes, strides;
    if (inputs.size() == 1) {
      auto copyFunction = [](std::vector<int> &dst,
                             const MNN::Attribute *attr) {
        MNN_ASSERT(nullptr != attr->list());
        MNN_ASSERT(nullptr != attr->list()->i());
        dst.resize(attr->list()->i()->size());
        ::memcpy(dst.data(), attr->list()->i()->data(),
                 dst.size() * sizeof(int));
      };
      ...
      MNN_ERROR("Onnx slice must have 1 or 4 inputs for Slice1, \
    ...
    auto MakeConstVecVar = [](const std::vector<int> &array) {
      ...
      return _Const(array.data(), {static_cast<int>(array.size())}, NCHW,
                    data_type);
    };
    ...
    if (strides.size() && !IsAllOne(strides)) {
      ...
      std::vector<int> tf_begin(ndim, 0);
      std::vector<int> tf_end = inputInfo->dim;
      std::vector<int> tf_strides(ndim, 1);
      ...
      for (int i = 0; i < axes.size(); ++i) {
        ...
        if (ends[i] == (1 << 30)) {
          tf_end[axis] = inputInfo->dim[axis];
        } else if (ends[i] == -(1 << 30)) {
          tf_end[axis] = 0;
        } else {
          tf_end[axis] = ends[i];
        }
        tf_strides[axis] = strides[i];
      }
      auto beginVar = MakeConstVecVar(tf_begin);
      auto EndVar = MakeConstVecVar(tf_end);
      auto StridesVar = MakeConstVecVar(tf_strides);
      sliceOp->type = OpType_StridedSlice;
      return Expr::create(sliceOp.get(), {input, beginVar, EndVar, StridesVar},
                          expr->outputSize());
    } else {
      ...
      return Expr::create(sliceOp.get(), {input, beginVar, sizeVar},
                          expr->outputSize());
    }
  }
};
...
static auto gRegister = []() {
  OnnxExtraManager::get()->insert(
      "Slice",
      std::shared_ptr<OnnxExtraManager::Transform>(new OnnxSliceTransform));
  return true;
}();
