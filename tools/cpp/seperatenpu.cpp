...
static std::unique_ptr<MNN::OpT> _compileSubModule(const SubModuleIO& io, SubModuleInfo& info, const void* buffer, size_t bufferSize, const std::string& path, std::string srcpath, const std::string& targetNpuPath, float& cpuTotal, float& npuTotal, int shapeIndex) {
    std::vector<std::string> inputNames(info.inputs.size());
    std::vector<std::string> outputNames(info.outputs.size());
    auto net = flatbuffers::GetRoot<Net>(buffer);
    for (int i=0; i<info.inputs.size(); ++i) {
    ...
    for (int i = 0; i < io.inputs.size(); i++) {
        ...
        for (int j = 0; j < io.inputs[i]->getInfo()->dim.size(); j++) {
            ...
            inputsShapeStr += std::to_string(io.inputs[i]->getInfo()->dim[j]);
        }
    }
    attr.reset(new MNN::AttributeT);
...
int main(int argc, const char* argv[]) {
    if (argc < 3) {
        MNN_PRINT("Usage: ./seperatenpu src.mnn dst.mnn npu.json\n");
        return 0;
    }
    ...
    if (argc >= 4) {
        ...
        if (document.HasMember("offline")) {
            gNeedOffline = true;
            gOfflieSrc = document["offline"]["src"].GetString();
            gOfflieDst = document["offline"]["dst"].GetString();
        } else {
            gOfflieSrc = gNPUName;
            gOfflieDst = gNPUName;
        }
        ...
        gNPUName = document["name"].GetString();
    ...
    for (int inputIndex=0; inputIndex < inputs.size(); ++inputIndex) {
        ...
        for (int i=0; i<subModulesInfo.size(); ++i) {
            if (!subModulesInfo[i].isBreak) {
                auto path = std::string("res/") + std::to_string(npuIndex) + "." + gOfflieDst;
                auto srcPath = path;
                if (0 != inputIndex) {
                    srcPath = std::string("res/") + std::to_string(inputIndex) + "_" +  std::to_string(npuIndex) + "." + gOfflieSrc;
                } else {
                    srcPath = std::string("res/") +  std::to_string(npuIndex) + "." + gOfflieSrc;
                }
                ...
                npuOps[i] = std::move(_compileSubModule(moduleIO[i], subModulesInfo[i], bufferPair.first, bufferPair.second, srcPath, srcMNN, path, cpuTotal, npuTotal, inputIndex));
                npuIndex++;
            }
        }
