...
bool equalValues(const std::vector<float>& a, const std::vector<float>& b, const char* label, size_t dk, size_t dv) {
    for (size_t i = 0; i < a.size(); ++i) {
        ...
        if (!(error <= 2e-5f * std::max(1.0f, std::fabs(b[i])))) {
        ...
        for (size_t dv : {size_t(0), size_t(1), size_t(3), size_t(4), size_t(7), size_t(16), size_t(31), size_t(32),
            ...
            for (float decay : {0.0f, 0.73f, 1.0f}) {
                ...
                for (float beta : {0.0f, 0.37f, 1.0f}) {
                    expected = state;
                    actual = state;
                    // Bound the recurrent update as in the normalized-key inference path.
                    // Unnormalized keys are already exercised by the single-step tests above.
                    auto recurrentK = k;
                    auto recurrentQ = q;
                    const float normalization = 1.0f / std::sqrt(float(std::max(size_t(1), dk)));
                    for (size_t i = 0; i < dk; ++i) {
                    ...
                    for (size_t step = 0; step < 8; ++step) {
                        for (size_t j = 0; j < dv; ++j) {
                            ...
                            for (size_t i = 0; i < dk; ++i) {
                                sk += expected[1 + i * dv + j] * recurrentK[i];
                                sq += expected[1 + i * dv + j] * recurrentQ[i];
                            }
                            const float correction = beta * (v[j] - decay * sk);
                            refOut[j + 1] = decay * sq + kq * correction;
                            for (size_t i = 0; i < dk; ++i)
                                expected[1 + i * dv + j] =
