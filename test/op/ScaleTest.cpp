...
static Interpreter *create(int b, int c, int h, int w, bool tensorflow) {
    ...
    }
    {
        ...
        flatbuffers::uoffset_t scale;
        if (b) {
            auto data = fbb.CreateVector(scales);
            auto bs   = fbb.CreateVector(bias);
            auto sb   = ScaleBuilder(fbb);
            sb.add_channels(c);
            sb.add_scaleData(data);
            sb.add_biasData(bs);
            scale = sb.Finish().o;
        } else {
            auto data = fbb.CreateVector(scales);
        ...
        auto name = fbb.CreateString("scale");
        auto iv   = fbb.CreateVector(std::vector<int>({0}));
        auto ov   = fbb.CreateVector(std::vector<int>({1}));
        OpBuilder builder(fbb);
        builder.add_type(OpType_Scale);
        builder.add_name(name);
        builder.add_inputIndexes(iv);
        builder.add_outputIndexes(ov);
        builder.add_main_type(OpParameter_Scale);
        builder.add_main(flatbuffers::Offset<void>(scale));
        vec.push_back(builder.Finish());
    }
...
MNNTestSuiteRegister(ScaleTensorflowTest, "op/scale/tensorflow");
