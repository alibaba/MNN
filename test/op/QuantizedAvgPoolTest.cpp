...
static Interpreter *create(PoolPadType type, int kernel, int stride, int pad, int min, int max, int w, int h, int c,
    ...
    {
        auto pb = QuantizedAvgPoolBuilder(fbb);
        ...
        OpBuilder builder(fbb);
        builder.add_type(OpType_QuantizedAvgPool);
        builder.add_name(name);
        builder.add_inputIndexes(iv);
        builder.add_outputIndexes(ov);
        builder.add_main_type(OpParameter_QuantizedAvgPool);
                                    ...
                                    for (int p = 0; p <= 2; p++) {
                                        dispatch([&](MNNForwardType backend) -> void {
                                            if (backend == MNN_FORWARD_CPU)
                                                return;
                                            // nets
                                            auto net = create(type, k, s, p, 1, 99, w, h, c, b);
