...
static Interpreter *create(float slope, int b, int c, int h, int w, bool tensorflow) {
    ...
    {
        auto rb = ReluBuilder(fbb);
        rb.add_slope(slope);
        auto relu = rb.Finish();
        auto name = fbb.CreateString("relu");
        auto iv   = fbb.CreateVector(std::vector<int>({0}));
        auto ov   = fbb.CreateVector(std::vector<int>({1}));

        OpBuilder builder(fbb);
        builder.add_type(OpType_ReLU);
        builder.add_name(name);
        builder.add_inputIndexes(iv);
        builder.add_outputIndexes(ov);
        builder.add_main_type(OpParameter_Relu);
        builder.add_main(flatbuffers::Offset<void>(relu.o));
        vec.push_back(builder.Finish());
    }
...
class ReLUCaffeTest : public MNNTestCase {
public:
    virtual ~ReLUCaffeTest() = default;
    virtual bool run() {
        for (int b = 1; b <= 2; b++) {
            for (int c = 1; c <= 8; c *= 2) {
                for (int h = 1; h <= 8; h *= 2) {
                        ...
                        dispatch([&](MNNForwardType backend) -> void {
                            ...
                            auto input = new Tensor(4);
                            {
                                ...
                                for (int i = 0; i < b * c * h * w; i++) {
                                    input->host<float>()[i] = rand() % 255 - 127;
                                }
                            }
...
class ReLUTensorflowTest : public MNNTestCase {
public:
    virtual ~ReLUTensorflowTest() = default;
    virtual bool run() {
        for (int b = 1; b <= 2; b++) {
            for (int c = 1; c <= 8; c *= 2) {
                for (int h = 2; h <= 8; h *= 2) { // CPU relu take at least 4 input -        -|||
                    for (int w = 2; w <= 8; w *= 2) {
                        dispatch([&](MNNForwardType backend) -> void {
                            if (backend == MNN_FORWARD_CPU)
                                return;
...
MNNTestSuiteRegister(ReLUCaffeTest, "op/relu/caffe");
MNNTestSuiteRegister(ReLUTensorflowTest, "op/relu/tensorflow");
