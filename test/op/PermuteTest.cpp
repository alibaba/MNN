...
public:
    virtual bool run() {
        ...
        {
            // NC4HW4
            auto input    = _Input({10, 8, 37, 37}, NC4HW4, halide_type_of<float>());
            auto output   = _Permute(input, {0, 3, 2, 1});
            auto outputC4 = (37 + 3) / 4;
            std::vector<float> targetOutputs(10 * 8 * 37 * outputC4 * 4, 0.f);
            float maxValue = 1.0f;
            {
                auto ptr = input->writeMap<float>();
                ::memset(ptr, 0, input->getInfo()->size * sizeof(float));
                ...
                for (int b = 0; b < 10; ++b) {
                    auto ptrB    = ptr + b * 8 * 37 * 37;
                    auto targetB = targetOutputs.data() + b * 8 * 37 * 4 * outputC4;
                    for (int c = 0; c < 8; ++c) {
                        auto ptrC    = ptrB + (c / 4) * 4 * 37 * 37 + (c % 4);
                        auto targetC = targetB + c * 4;
                        for (int y = 0; y < 37; ++y) {
                            auto ptrY    = ptrC + y * 37 * 4;
                            auto targetY = targetC + y * 8 * 4;
                            for (int x = 0; x < 37; ++x) {
                                auto ptrX    = ptrY + x * 4;
                                auto targetX = targetY + (x / 4) * 8 * 4 * 37 + (x % 4);
                                *ptrX        = index;
                                *targetX     = index;
                                index += 0.01f;
                            }
