...
VARP applyUnary(VARP input, TestUnaryType type) {
  ...
  case TestUnaryType::LOG:
    return _Log(input);
  case TestUnaryType::SIN:
    return _Sin(input);
  case TestUnaryType::COS:
    return _Cos(input);
  }
  return nullptr;
}
...
public:
  virtual bool run(int precision) override {
    ...
    for (int i = 0; i < kSize; ++i) {
      inputData[i] = -12.0f + 24.0f * static_cast<float>(i) /
    ...
    const float specialValues[] = {-100.0f, -12.0f, -8.0f, -4.0f, -0.0f,
                                   0.0f,    4.0f,   8.0f,  12.0f, 100.0f};
    ...
    const Case cases[] = {
        {TestUnaryType::SIGMOID, 0.005f}, {TestUnaryType::TANH, 0.009f},
        {TestUnaryType::SILU, 0.008f},    {TestUnaryType::GELU, 0.009f},
        {TestUnaryType::LOG, 0.02f},      {TestUnaryType::SIN, 0.012f},
        {TestUnaryType::COS, 0.012f},
    };
    ...
    for (const auto &testCase : cases) {
      ...
      MNN_PRINT("Hexagon Unary %s max abs error: %.8f at x=%.8f\n",
                unaryName(testCase.type), maxError, inputData[maxIndex]);
      if (maxError > testCase.maxAbsoluteError) {
        ...
        for (int i = begin; i < end; ++i) {
          MNN_ERROR("  index=%d x=%.8f expected=%.8f actual=%.8f\n", i,
                    inputData[i], referenceUnary(inputData[i], testCase.type),
                    value[i]);
        }
        return false;
      }
