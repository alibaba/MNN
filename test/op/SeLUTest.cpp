...
static Interpreter *create(float scale, float alpha, int b, int c, int h, int w, bool tensorflow) {
    ...
    {
        auto sb = SeluBuilder(fbb);
        sb.add_scale(scale);
        sb.add_alpha(alpha);
        auto selu = sb.Finish();
        auto name = fbb.CreateString("selu");
        auto iv   = fbb.CreateVector(std::vector<int>({0}));
        auto ov   = fbb.CreateVector(std::vector<int>({1}));

        OpBuilder builder(fbb);
        builder.add_type(OpType_Selu);
        builder.add_name(name);
        builder.add_inputIndexes(iv);
        builder.add_outputIndexes(ov);
        builder.add_main_type(OpParameter_Selu);
        builder.add_main(flatbuffers::Offset<void>(selu.o));
        vec.push_back(builder.Finish());
    }
                ...
                for (int h = 1; h <= 16; h *= 2) {
                    for (int w = 1; w <= 16; w *= 2) {
                        dispatch([&](MNNForwardType backend) -> void {
                            ...
                            auto net    = create(scale, alpha, b, c, h, w, false);
                            ...
                            auto input = new Tensor(4);
                            {
                                ...
                                for (int i = 0; i < b * c * h * w; i++) {
                                    input->host<float>()[i] = (rand() % 255 - 512) / 256.f;
                                }
                ...
                for (int h = 1; h <= 4; h++) {
                    for (int w = 1; w <= 4; w++) {
                        dispatch([&](MNNForwardType backend) -> void {
                            ...
                            float scale = rand() % 255 / 255.f;
                            float alpha = rand() % 255 / 255.f;
                            auto net    = create(scale, alpha, b, c, h, w, true);
                            auto CPU    = createSession(net, MNN_FORWARD_CPU);
                            auto GPU    = createSession(net, backend);
                            if (!CPU || !GPU) {
                                delete net;
...
MNNTestSuiteRegister(SeLUTensorflowTest, "op/selu/tensorflow");
