...
public:
    virtual bool run(int precision) override {
        if (MNNTestSuite::get()->pStaus.forwardType != MNN_FORWARD_HEXAGON) {
            MNN_PRINT("Skip HexagonMulSiluPWLTest: Hexagon backend is not selected\n");
            return true;
        }
        ...
        for (int i = 0; i < kSize; ++i) {
            gateData[i] = -12.0f + 24.0f * static_cast<float>(i) / static_cast<float>(kSize - 1);
            upData[i] = -8.0f + 16.0f * static_cast<float>((i * 37) % kSize) / static_cast<float>(kSize - 1);
        }
        ...
        int maxIndex = 0;
        for (int i = 0; i < kSize; ++i) {
            const float expected = upData[i] * gateData[i] / (1.0f + std::exp(-gateData[i]));
