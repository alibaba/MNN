...
public:
    virtual bool run() {
        ...
        interp->getSessionInfo(session, MNN::Interpreter::SessionInfoCode::MEMORY, &initMemory);
        for (int i = 0; i < 100; ++i) {
            if (i % 2 == 0) {
        ...
        interp->getSessionInfo(session, MNN::Interpreter::SessionInfoCode::MEMORY, &lastMemory);
